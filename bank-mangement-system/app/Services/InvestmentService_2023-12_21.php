...
class InvestmentService
{
    ...
    public function storeInvestment($request, $repo)
    {
       // dd($request->all());
        ...
        try {
            ...
            $sAccount = $this->getMemberId($request->input('memberAutoId'))->with(['savingAccount' => function ($q) use ($companyId) {
    ...
    public function registerSSbRequiredData($request, $memberId, $transtype = NULL)
    {
        ...
        try {
            ...
            $createAccount = CommanController::createSavingAccountDescriptionModify($memberId, $branch_id, $branchCode, $ssbAmountTrans, 0, $res->id, $miCode, $investmentAccount, $is_primary, $faCode, $description, $request['associatemid'], 0, $daybookRefRDrr, $request['company_id'], $request['memberAutoId'], $passbook, 1);
            ...
            // ---------------------------  HEAD IMPLEMENT --------------------------
            if ($checkSSbExistinAllCompany == 0) {
                $this->investHeadCreateSSB($ssbAmountTrans, $globaldate, $ssbAccountId, $planId, NULL, NULL, NULL, NULL, NULL, NULL, NULL, $branch_id, $request['associatemid'], $memberId, $ssbId, $createDayBook, $request['payment-mode'], $investmentAccount, $companyId, $daybookRefRDrr);
            }
            ...
            if ($transtype == 'Loan') {
                ...
                if ($memberInvestment > 1 && $ssbAmountTrans > 0) {
                    $this->loanStationaryCharges($request, $investmentId, $memberId, $investmentAccount);
                }
            }
    ...
    {  
        ...
        try {
            ...
            if ($gstAmount > 0) {
                ...
                Memberinvestments::where('id', $investmentId)->update($rec);
            }
            $response = [
                'status' => true,
                'data'   => '50'
