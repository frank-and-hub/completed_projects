...
class MonthlyEcsTransactionGroupLoan extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'command:MonthlyEcsTransactionGroupLoan';
     ...
     * @var string
     */
    protected $description = 'Monthly ECS Transaction Group Loan cron run sucessfully';
    ...
    {
		$t = false;
        $logName = 'loan/monthly_ESC_transaction_for_group_loan'. '-'.date('Y-m-d',strtotime(now())).'.log';		
        ...
        Log::channel('ssbEcsCronGroupLoan')->info('Transaction  Start'); 
		// try {
			...
			foreach ($branches as $branch) {
				...
				$loans = $loanModel
					...
					->where('status', 4)
					// ->where('emi_due_date', '<' ,'2024-03-01')
					// ->where('emi_due_date','>' ,'2024-02-28')
					// ->whereIn('emi_due_date', ['2024-04-10','2024-04-11'])
					->where('emi_due_date', $sysDate)
					->where('ecs_type', 2)
					->get();
				//  p($loans->count());
                Log::channel('ssbEcsCronGroupLoan')->info('total account ='.$loans->count()); 
				foreach ($loans as $loan) {
                    ...
                    Log::channel('ssbEcsCronGroupLoan')->info('interestCalculate  a/c no ='.$loan->account_number.', date ='.$currentDate); 
					...
					if ($loan->newLoanSSB && $amount > 0 || $loan->newLoanSSB && $closerAmount > 0   ) {
						...
						Log::channel('ssbEcsCronGroupLoan')->info('  amount >0   a/c no ='.$loan->account_number); 
					} else {
                         Log::channel('ssbEcsCronGroupLoan')->info('no due amount   a/c no ='.$loan->account_number.', date ='.$currentDate); 
						// Change next emi due date
						$mlData = \App\Models\Grouploans::find($loan->id);
                        ...
                        Log::channel('ssbEcsCronGroupLoan')->info('no due amount   a/c no ='.$loan->account_number.', next emi due ='.$emiDueDate); 
			...
			$sms_text = "Software Cron - MonthlyEcsTransactionGroupLoan completed with status - success on $ldate Samraddh Bestwin";
	...
	public function callCronController($value,$systemDate,$closerAmount)
	{
		...
		$res = [];
		// dd($req->all());
		$response = \App\Http\Controllers\Admin\Cron\CronController::cronDepositeGroupLoanEmi($req);
