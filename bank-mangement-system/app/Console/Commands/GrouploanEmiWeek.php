...
class GrouploanEmiWeek extends Command
{
    use EmiDatesTraits;
     ...
     * @var string
     */
    protected $signature = 'groupemiUpdateWeekly:generate';
    ...
    public function handle()
    {
        ...
        $getLoans = Grouploans::where('loan_type','=',3)->where('emi_option',2)->whereIn('status',[4])->whereNotNull('approve_date')->get();
    ...
    try{
        foreach($getLoans as $loan)
        {
            ...
            $CurrentDate= date('d');
            // $CurrentDate = date('d',strtotime('2022-06-21'));
            // $CurrentDatess = date('Y-m-d',strtotime('2022-06-21'));
            $CurrentDateYear = date('Y');
            $CurrentDateMonth = date('m');
            $CurrentDatess = $currentDate;

            // $CurrentDateYear = date('Y',strtotime('2022-06-21'));;
            // $CurrentDateMonth = date('m',strtotime('2022-06-21'));;


            //$prioremistartDate = $checkDateYear.'-'.  $checkDateMonth.'-01';

            $daysDiff =today()->diffInDays($LoanCreatedDate)/7;
            ...
            if(array_key_exists($CurrentDate.'_'.$CurrentDateMonth.'_'.$CurrentDateYear,$nextEmiDates))
            {
                ...
                if($preDate == $CurrentDatess)
                {
                    ...
                    $aqmount = LoanEmisNew::where('loan_id',$loan->id)->where('is_deleted','0')->whereBetween('emi_date',[$pDate,$CurrentDatess])->sum('roi_amount');
                }
