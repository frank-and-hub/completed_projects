...
class CommissionDetailReportController extends Controller
{
    ...
    if ($request->ajax()) {
      ...
      if (isset($arrFormData['is_search']) && $arrFormData['is_search'] == 'yes') {
        $data = AssociateMonthlyCommission::with('member:id,associate_id,first_name,last_name,associate_no')
          ->with('carderName:name,id,short_name')->where('is_deleted', '0')->where('type', 1)
        ...
        $data = $data->orderby('id', 'DESC')->get(['id', 'total_amount', 'qualifying_amount', 'commission_amount', 'percentage', 'cadre_from', 'cadre_to', 'commission_for_month', 'company_id', 'assocaite_id', 'commission_for_year']);
        $sno = $_POST['start'];
        $rowReturn = array();
        //dd($data->toArray());
        foreach ($data as $row) {
  ...
  public function loanCommissionList(Request $request)
  {
    if ($request->ajax()) {
      ...
      if (isset($arrFormData['is_search']) && $arrFormData['is_search'] == 'yes') {
        $data = AssociateMonthlyCommission::with('member:id,associate_id,first_name,last_name,associate_no')
          ->with('carderName:name,id,short_name')->where('is_deleted', '0')->where('type',2)->where('type_id',$arrFormData['loan_id']);
  ...
  public function loanCommissionExport(Request $request)
  {
    ...
    $data = AssociateMonthlyCommission::with('carderName:name,id,short_name')->where('is_deleted', '0')->where('type',2)->where('type_id',$request['loan_id']);
    ...
    $results = $data->orderby('id', 'DESC')->offset($start)->limit($limit)->get(['id', 'total_amount', 'qualifying_amount', 'commission_amount', 'percentage', 'cadre_from', 'cadre_to', 'commission_for_month', 'company_id', 'assocaite_id', 'commission_for_year']);
      ...
      foreach ($results as $row) {
        ...
        $sno++;
        $val['DT ROWINDEX'] = $sno;
        $val['MONTH'] = date("M", strtotime($row->commission_for_month)) . '-' . date("Y", strtotime($row->commission_for_year));
        $val['ASSOCIAT ID'] = $row->member->associate_no;
        $val['ASSOCIAT NAME'] = !empty($row->member->first_name) && !empty($row->member->last_name) ? $row->member->first_name . ' ' . $row->member->last_name : '';
        $val['CARDER NAME'] = $row->carderName->name . '(' . $row->carderName->short_name . ')';
        $val['TOTAL NAME'] = number_format((float) $row->total_amount, 2, '.', '');
      ...
      $data = $data->orderby('id', 'DESC')->get();
      $pdf = PDF::loadView('templates.branch.investment_management.exportinvestmentcommission', compact('data'))->setPaper('a4', 'landscape')->setWarnings(false);
      $pdf->save(storage_path() . '_filename.pdf');
      return $pdf->download('branch_loancommissionexports.pdf');
    }
  }
}
