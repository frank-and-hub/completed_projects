...
use URL;
use App\Models\{Form15G,MemberCompany};
...
{    
    ...
    {		
		if($id){	
			...
			if($first_id_type_id == 5 || $second_id_type_id == 5){
				...
				return view('templates.branch.form_g.index', $data);
		    }else{		
				$previousUrl = URL::previous();
				return redirect('https://uatsamraddh.host4india.in/branch/dashboard');			
		   }
		} 
    }
	...
	public function delete(Request $request)
	  ...
	  public function datacheck(Request $request){
		$data = Form15G::whereCompanyId($request->selectedcustomerId)->whereStatus(1)->whereMemberId($request->memberId)->whereCustomerId($request->customerId)->where('year',$request->year)->exists();
		$response = [
			'data' => $data?1:0,
		];
