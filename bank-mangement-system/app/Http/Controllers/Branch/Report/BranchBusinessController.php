...

{
    ...
    public function branch_business()
    {
        ...
        return view('templates.branch.report.branch_business', $data);
    }
          ...
          //  dd($company);
            if (isset($request['is_search']) && $request['is_search'] == 'yes') {
                ...
                if ($request['start_date'] != '') {
                    ...
                    // $account_head=$account_head->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate]); 
                }
                ...
                $val['total_case_current_daily_new_ac'] = branchBusinessTotalCaseCollectionCount($startDate, $endDate, $branch_id, array($planDaily));

                $val['current_daily_new_deno_ac'] = branchBusinessInvestNewDenoSum($startDate, $endDate, $branch_id, $planDaily);
                $val['total_case_current_daily_new_deno_ac'] = branchBusinessTotalCaseCollectionAmountSum($startDate, $endDate, $branch_id, array($planDaily));
                ...
                $val['total_case_current_monthly_new_ac'] = branchBusinessTotalCaseCollectionCount($startDate, $endDate, $branch_id, array($monthlyId));

                $val['current_monthly_deno_sum'] = branchBusinessInvestNewDenoSumType($startDate, $endDate, $monthlyId, $branch_id);
                $val['total_case_current_monthly_deno_sum'] = branchBusinessTotalCaseCollectionAmountSum($startDate, $endDate, $branch_id, array($monthlyId));
                ...
                $val['total_case_current_fd_new_ac'] = branchBusinessTotalCaseCollectionCount($startDate, $endDate, $branch_id, array($fdId));

                $val['current_fd_deno_sum'] = branchBusinessInvestNewDenoSumType($startDate, $endDate, $fdId, $branch_id);
                $val['total_case_current_fd_deno_sum'] = branchBusinessTotalCaseCollectionAmountSum($startDate, $endDate, $branch_id, array($fdId));
                ...
                */
                ...
                $val['total_loan_recovery_ac'] = LoanDayBooks::where('branch_id', $branch_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->count();
                $val['total_loan_recovery_amt'] = LoanDayBooks::where('branch_id', $branch_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('deposit');
                ...
                $val['file_chrg_total_ac'] = Daybook::whereIn('transaction_type', ['6,10'])->where('branch_id', $branch_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->count();
                $val['file_chrg_amount_total'] = Daybook::whereIn('transaction_type', ['6,10'])->where('branch_id', $branch_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('amount');;
                $val['file_chrg_total_ac_case_mode'] = Daybook::whereIn('transaction_type', ['6,10'])->where('branch_id', $branch_id)->where('payment_mode', '0')->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->count();
                $val['file_chrg_amount_total_cash_mode'] = Daybook::whereIn('transaction_type', ['6,10'])->where('branch_id', $branch_id)->where('payment_mode', '0')->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('amount');
            ...
            return \Response::json(['view' => view('templates.branch.report.filtered_branch_business_report', ['data' => $val, 'account_head' => $account_head, 'branchToHoTotal' => $branchToHoTotal, 'end_date' => $endDate, 'branch_id' => $branch_id, 'loans' => $loans, 'micros' => $micros, 'total_amounts' => $totalAmounts, 'receivedChequeMicro' => $receivedChequeMicro, 'receivedChequeLoan' => $receivedChequeLoan, 'total_received_cheque_amount' => $total_received_cheque_amount, 'closing_cash_in_hand_samraddh_micro' => $closing_cash_in_hand_samraddh_micro, 'closing_cash_in_hand_samraddh_loan' => $closing_cash_in_hand_samraddh_loan, 'start_date' => $startDate])->render(), 'msg_type' => 'success', 'startDate' => $startDate, 'endDate' => $endDate]);
        }
    }
}
