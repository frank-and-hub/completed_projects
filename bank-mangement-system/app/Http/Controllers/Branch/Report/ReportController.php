...
class ReportController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('templates.branch.report.index', $data);
    }
    /**
    ...
    public function associateBusinessReport()
    {
        ...
        return view('templates.branch.report.associate_business', $data);
    }
    /**
    ...
    public function associateBusinessList(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = Member::select(['id', 'associate_join_date', 'associate_no', 'first_name', 'last_name', 'current_carder_id', 'is_associate', 'associate_branch_id'])
                ->with(['associate_branch' => function ($q) {
            ...
            foreach ($data as $row) {
                $sno++;
                $associate_id = $row->id;
                //echo investNewAcCountType($associate_id,$startDate,$endDate,$monthlyId,$branch_id); 
                //dd($investNewAcCountType);
                $val['DT_RowIndex'] = $sno;
                ...
                $val['cadre'] = $row['getCarderNameCustom']->name; //getCarderName($row->current_carder_id);
                ...
                //     $val['daily_new_ac'] = $investNewAcCount[$associate_id.'_'.$planDaily];
                // }else{
                //     $val['daily_new_ac'] = '0';
                // }
                ...
                //     $val['daily_deno_sum'] = $investNewDenoSum[$associate_id.'_'.$planDaily];
                // }else{
                //     $val['daily_deno_sum'] = '0';
                // }
                ...
                $val['monthly_new_ac'] = investNewAcCountType($associate_id, $startDate, $endDate, $monthlyId, $branch_id); //Monthly N.I. - No. A/C
                ...
                $val['fd_new_ac'] = investNewAcCountType($associate_id, $startDate, $endDate, $fdId, $branch_id); //FD N.I. - No. A/C
                ...
                //echo $associate_id.','.$startDate.','.$endDate.','.$branch_id;
    ...
    public function associateBusinessSummaryReport()
    {
        ...
        $data['branch'] = Branch::where('status', 1)->get();
        $data['zone'] = Branch::where('status', 1)->select('zone')->groupBy('zone')->get();
        return view('templates.branch.report.associate_business_summary', $data);
    }
    /**
    ...
    public function associateBusinessCompareReport()
    {
        ...
        return view('templates.branch.report.associate_business_compare', $data);
    }
    /**
    ...
    public function maturity()
    {
        ...
        return view('templates.branch.report.new_maturity', $data);
    }
    public function planmaturity(Request $request)
    {
        $plans = Plans::where('status', 1)->where('plan_category_code', '!=', 'S')->where('company_id', $request['company_id'])->get(['name', 'id']);
        $data = '';
    ...
    public function maturityReportListing(Request $request)
    {
        if ($request->ajax()) {
            ...
            foreach ($data as $row) {
                ...
                if ($row['demandadvice']) {
                    if ($row['demandadvice']->payment_mode == 2) {
                        $transaction = (isset($row['demandadvice']['demandAmountHead'][0])) ? $row['demandadvice']['demandAmountHead'][0] : 0;
                        if ($transaction) {
    ...
    public function loan()
    {
        ...
        $data['branch'] = Branch::where('status', 1)->get();
        return view('templates.branch.report.loan', $data);
    }
    public function companyIdToLoan(Request $request)
    {
        $company_id = $request->company_id;
        $data['loan'] = Loans::where('company_id', $company_id)->pluck('name', 'id');
    ...
    public function loanListing(Request $request)
    {
        if ($request->ajax()) {
            ...
            $loan = Memberloans::select('id', 'customer_id', 'applicant_id', 'status', 'loan_type', 'applicant_id', 'group_loan_common_id', 'branch_id', 'account_number', 'amount', 'deposite_amount', 'approve_date', 'approved_date', 'emi_amount', 'emi_period', 'emi_option', 'created_at', 'credit_amount', 'ROI', 'due_amount', 'company_id','closing_date','transfer_amount')
                ->with([
                    'loan:id,name',
                    'company:id,name',
            ...
            $grpLoan  = Grouploans::select('id', 'branch_id', 'created_at', 'loan_type', 'status', 'account_number', 'member_loan_id', 'applicant_id', 'member_id', 'group_loan_common_id', 'deposite_amount', 'amount', 'approve_date', 'emi_period', 'emi_option', 'due_amount', 'ROI', 'emi_amount', 'credit_amount', 'company_id','closing_date','transfer_amount')
                ->with([
                    'loan:id,name',
                    'company:id,name',
                    'member:id,member_id,first_name,last_name,mobile_no,address',
            ...
            if (isset($arrFormData['is_search']) && $arrFormData['is_search'] == 'yes') {
                ...
                if ($arrFormData['company_id'] != '' && $arrFormData['company_id'] != '0') {
                    $application_number = $arrFormData['company_id'];
                    $loan = $loan->where('account_number', '=', $application_number);
                    $grpLoan = $grpLoan->where('account_number', '=', $application_number);
                }
                if ($arrFormData['member_id'] != '') {
                    ...
                    $grpLoan = $grpLoan->whereHas('loanMemberCompany', function ($query) use ($member_id) {
                        $query->where('member_id', 'LIKE', '%' . $member_id . '%');
                    });
                }
            }
            ...
            foreach ($data as $row) {
                ...
                if ($row->loan_type == 3) {
                    if (customGetMemberData($row['loanMemberCompany']->customer_id)) {
                        $val['applicant_name'] = customGetMemberData($row['loanMemberCompany']->customer_id)->first_name . ' ' . customGetMemberData($row['loanMemberCompany']->customer_id)->last_name;
                    } else {
                ...
                if ($row->loan_type == 3) {
                    ...
                    if (isset($row->applicant_id)) {
                        $member_id = Member::where('id',$row->customer_id)->first(['id','member_id']);
                        $val['applicant_id'] = ($member_id)??'N/A';
                    } else {
                ...
                if ($row->loan_type == 3) {
                    // if (isset($row->member_id)) {
                        if (customGetMemberData($row['loanMemberCompany']->customer_id)) {
                            $val['applicant_phone_number'] = customGetMemberData($row['loanMemberCompany']->customer_id)->mobile_no;
                        } else {
                    ...
                    // if (isset($row->applicant_id)) {
                        if (customGetMemberData($row->customer_id)) {
                            $val['applicant_phone_number'] = customGetMemberData($row->customer_id)->mobile_no;
                        } else {
                            $val['applicant_phone_number'] = 'N/A';
                        }
                    // } else {
                ...
                $val['customer_id'] = isset($row['loanMemberCompany']['member']['member_id']) ? $row['loanMemberCompany']['member']['member_id'] : 'N/A';
                $val['company'] = isset($row['company']['name']) ? $row['company']['name'] : 'N/A';
                if (isset($row->branch_id)) {
                    $branch = getBranchDetail($row->branch_id)->name;
                } else {
                    $branch = 'N/A';
                }
                $val['branch'] = $branch;
                ...
                if ($row->loan_type == 3) {
                    $val['member_id'] = (isset($row['loanMemberCompany'])) ? $row['loanMemberCompany']['member_id'] : 'N/A';
                } else {
                    // $val['member_id'] = (isset($row['loanMemberCompany'])) ? $row['loanMemberCompany']['member_id'] : 'N/A';
                    if (isset($row->applicant_id)) {
                        // $member_id = Member::where('id',$row->applicant_id)->first(['id','member_id']);
                        $val['member_id'] = getMemberCompanyDataNew($row->applicant_id)?getMemberCompanyDataNew($row->applicant_id)->member_id:'N/A';
                    } else {
                        $val['member_id'] = 'N/A';
                    }
                }
                $val['sanctioned_amount'] = $row->amount . ' &#8377';
                $val['transfer_amnt'] = $row->deposite_amount . ' &#8377';
                $val['sanction_date'] =($row->approve_date) ? date("d/m/Y", strtotime(convertDate($row->approve_date))) : 'N/A';                
                ...
                // pd($row->id);
                $mode = \App\Models\Daybook::whereIn('transaction_type', [3, 8])->where('loan_id', $row->id)->first(['payment_mode','cheque_dd_no']);
                if (isset($mode)) {
                ...
                $amount = \App\Models\LoanDayBooks::where('loan_type', $row->loan_type)->where('loan_id', $row->id)->whereIn('loan_sub_type',[0,1])->sum('deposit');
                $val['total_recovery_amount'] = $amount . ' &#8377';
                // $val['total_recovery_emi_till_date'] = $row->credit_amount . ' &#8377';
                if ($row->emi_option == 1) {
                ...
                $secondMonth = $d2[1];
                $monthDiff = $secondMonth - $firstMonth;
                $ccamount  = $monthDiff * $row->emi_amount;
                $val['emi_should_be_received_till_date'] = $ccamount . ' &#8377';
                $d1 = explode('-', $row->created_at);
                $d2 = explode('-', date("Y-m-d"));
                $firstMonth = $d1[1];
                $secondMonth = $d2[1];
                $monthDiff = $secondMonth - $firstMonth;
                $caamount  = $monthDiff * $row->emi_amount;
                $val['future_emi_due_till_date'] = $caamount . ' &#8377';
                $val['date'] = date("d/m/Y", strtotime($row->created_at));

                $val['co_applicant_name'] = (isset($row['LoanCoApplicants']) && count($row['LoanCoApplicants']) > 0) ? (customGetMemberData($row['LoanCoApplicants'][0]->member_id)) ? customGetMemberData($row['LoanCoApplicants'][0]->member_id)->first_name . ' ' . customGetMemberData($row['LoanCoApplicants'][0]->member_id)->last_name : 'N/A' : 'N/A';
                
                $val['co_applicant_number'] = (isset($row['LoanCoApplicants']) && count($row['LoanCoApplicants']) > 0) ? (customGetMemberData($row['LoanCoApplicants'][0]->member_id)) ? customGetMemberData($row['LoanCoApplicants'][0]->member_id)->mobile_no : 'N/A' : 'N/A';

                $val['gurantor_name'] = (isset($row['LoanGuarantor']) && count($row['LoanGuarantor']) > 0) ? (customGetMemberData($row['LoanGuarantor'][0]->member_id)) ? customGetMemberData($row['LoanGuarantor'][0]->member_id)->first_name . ' ' . customGetMemberData($row['LoanGuarantor'][0]->member_id)->last_name : $row['LoanGuarantor'][0]->name??'N/A' : 'N/A'; 
                
                $val['gurantor_number'] = (isset($row['LoanGuarantor']) && count($row['LoanGuarantor']) > 0) ? (customGetMemberData($row['LoanGuarantor'][0]->member_id)) ? customGetMemberData($row['LoanGuarantor'][0]->member_id)->mobile_no : $row['LoanGuarantor'][0]->mobile_number??'N/A' : 'N/A' ; 
             
                if (isset($row['LoanApplicants']) && count($row['LoanApplicants']) > 0) {
                ...
                //     $val['loan_end_date'] = date('d/m/Y', strtotime($row->closing_date));
                // } else {
                //     $val['loan_end_date'] = 'N/A';
                // }
                $val['loan_end_date'] = isset($row->closing_date) ? date('d/m/Y', strtotime($row->closing_date)) : $last_recovery_date ;
            ...
            // if(isset($loannew)){
                $token = session()->get('_token');
                  //Set value on caches
                Cache::put('loanReportListbranch'.$token, $data->toArray());
                Cache::put('loanReportListCountbranch'.$token, $totalCount);
    ...
    public function groupLoan()
    {
        ...
        $data['branch'] = Branch::where('status', 1)->get();
        return view('templates.branch.report.grouploan', $data);
    }
