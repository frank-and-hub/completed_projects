...
class ExpenseController extends Controller
{
    ...
    public function bill_expense_report_listing(Request $request)
    {
       ...
       if($request->ajax()){
        ...
        $data = \App\Models\BillExpense::with('expenses')->where('branch_id',$branch_id)->where('is_deleted',0);
         ...
         $data = $data->orderBy('created_at','DESC')->get();
          return Datatables::of($data)
           ...
           ->addColumn('action', function($row){
                $detailExpenseurl =  URL::to("branch/report/expense/".$row->bill_no."");  
                $deleteurl= URL::to("branch/bill_delete");
                    $btn = '';
                    if($row->status == 0)
                    {

                        $btn .= '<a href="branch/expense/edit/'.$row->bill_no.'"   title="Edit Expense"><i class="fa fa-edit mr-3"></i></a>'; 
                       
                        $btn .= '<a class="delete_expense mr-4" href="#" onclick="return false;" data-row-id="'.$row->bill_no.'" title="delete_expense"><i class="fa fa-trash"></i></a>';       
    ...
    public function edit($id){
        ...
        $billExpense = \App\Models\BillExpense::with('expenses')->select('bill_no','party_name','branch_balance','payment_mode','cheque_id','utr_no','neft_charge','party_bank_name','party_bank_ac_no','party_bank_ifsc','branch_id','bank_balance','bank_id','account_id')->where('bill_no',$id)->where('is_deleted',0)->first();
		...
		$expenseData = \App\Models\Expense::select('id','bill_no','bill_date','particular','account_head_id','sub_head1','sub_head2','amount','receipt')->where('bill_no',$id)->where('is_deleted',0)->get();
    ...
    **/
    ...
    public function export_bill(Request $request)
    {
        ...
        $getBranchId=getUserBranchId(Auth::user()->id);
        $branch_id=$getBranchId->id;
         $data = \App\Models\BillExpense::with('expenses')->where('is_deleted',0)->where('branch_id',$branch_id);
        ...
        {  
            $sno++;
            $val['SR_NO']=$sno;
            $val['BRANCH NAME']=getBranchDetail($row->branch_id)->name;
