        ...
        $member = Member::select('id','associate_app_status')->where('associate_no',$request->associate_no)->where('associate_status',1)->where('is_block',0)->first();
            ...
            if ($member) {
                ...
                if($token == $request->token){
                    $data = array();
                    $branchData=\App\Models\Branch::where('status',1)->get();
                    if(count($branchData))
                    {
    ...
    {   
        ...
        $member = Member::select('id','associate_app_status')->where('associate_no',$request->associate_no)->where('associate_status',1)->where('is_block',0)->first();
            ...
            if ($member) {
                ...
                if($token == $request->token){
                    $data = array();
                    $branchData=\App\Models\Plans::where('status',1)->get();
                    if(count($branchData))
                    {
                        foreach ($branchData as $key => $value) 
                        {
            ...
            if ($member) {
                ...
                if($token == $request->token){ 
                    ...
                    foreach ($invest_data as $key => $row) 
                    {
                        ...
                        $idProofDetail= \App\Models\MemberIdProof::where('member_id',$row['member']->id)->first();
    ...
    {   
        ...
        $member = Member::select('id','associate_app_status','first_name','last_name')->where('associate_no',$request->associate_no)->where('associate_status',1)->where('is_block',0)->first();
            ...
            if ($member) {
                ...
                if($token == $request->token){
                    $data = array();
                    $plan=\App\Models\Plans::where('status',1)->get();
                    if(count($plan))
                    {
                    ...
                    $dataRelation = array();
                    $relationData=\App\Models\Relations::get();
                            ...
                            if($member_data)
                            {
                              ...
                              //  $ssb = SavingAccount::where('member_id',$member->id)->first();
                               // $ssb_detail='';
                                ...
                                $dataSSB = array();
                                $ssb=SavingAccount::where('member_id',$member_data->id)->first();
                                if($ssb)
                                {
                                    $dataSSB['id'] = $ssb->id;  
                                    $dataSSB['account_no'] = $ssb->account_no;
            ...
            if ($member) {
                ...
                if($token == $request->token){
                    ...
                    elseif($request->plan_id!='' && $request->account_no!='')
                    {
                        ...
                        $db_member_data = Memberinvestments::where('account_number',$request->account_no)->first();
                    ...
                    $chk1=1; 
                        if($db_member_data)
                        {
                            ...
                            else
                            {
                                ...
                                if($request->plan_id==0)
                                {
                                    $invest_data = Memberinvestments::with('member','associateMember')->where('plan_id',7)->where('account_number',$request->account_no)->whereIn('member_id',$memberArray1)->first();
                                }
                                elseif($request->plan_id==2)
                                {
                                    $invest_data = Memberinvestments::with('member','associateMember','ssb')->where('plan_id',1)->where('account_number',$request->account_no)->whereIn('member_id',$memberArray1)->first();
                                }
                                else
                                {
                                    $invest_data = Memberinvestments::with('member','associateMember')->whereIn('plan_id',[2, 3, 5, 6, 10,11])->where('account_number',$request->account_no)->whereIn('member_id',$memberArray1)->first();
                                   ...
                                   // $ssb_detail='';
                                    ...
                                    if($invest_data->renewal_correction_request==1)
                                    {
                                        ...
                                        $messages = 'Account number temporarily not active contact to administrator.';
                                            ...
                                            $investmentLastDate = \App\Models\Investmentplantransactions::select('deposite_date','deposite_month')->where('investment_id',$invest_data->id)->orderBy('id', 'desc')->first();
                ...
                if($token == $request->token)
                {
                   ...
                   /* if($request->branch_id=='' && $request->branch_id>0)
                    ...
                    else
                    {
                        ...
                        if($invest_plan_id==2)
                        { //ssb renew
                            $received_cheque_id = $cheque_id= NULL; $cheque_deposit_bank_id = NULL; $cheque_deposit_bank_ac_id = NULL; $cheque_no=NULL;$cheque_date =$pdate=NULL;            
                            $online_deposit_bank_id = NULL; $online_deposit_bank_ac_id = NULL;
                             $online_transction_no = NULL;  $online_transction_date = NULL;
                            ...
                            { 
                                ...
                                if($db_member_data)
                                {
                                    ...
                                    if($invest_data)
                                    {
                                        $invest_branch_id=$invest_data->branch_id;$getState=\App\Models\Branch::where('id',$invest_branch_id)->first();
                                        $stateid=$getState->state_id;
                                        $globaldate = checkMonthAvailability(date('d'),date('m'),date('Y'),$stateid);
                                        Session::put('created_at',$globaldate);

                                                        $getState=\App\Models\Branch::where('id',$invest_branch_id)->first();
                                        $stateid=$getState->state_id;
                                        $globaldate = checkMonthAvailability(date('d'),date('m'),date('Y'),$stateid);
                                        Session::put('created_at',$globaldate);
...
//------------------HEAD IMPLEMENT ------- ---------  ------
                        ...
                        { // investment renew
                            ...
                            { 
                                ...
                                $db_member_data = Memberinvestments::where('account_number',$val_info->account)->first();
                                if($db_member_data)
                                {

                                    $invest_data = Memberinvestments::with('member','associateMember')->whereIn('plan_id',[2, 3, 5, 6, 10,11,7])->where('account_number',$val_info->account)->whereIn('member_id',$memberArray1)->first();
                                    if($invest_data)
                                    {
...
public  function investHeadCreate($amount,$globaldate,$investmentId,$planId,$cheque_no,$cheque_id,$cheque_date,$online_transction_no,$online_transction_date,$online_deposit_bank_id,$online_deposit_bank_ac_id,$branch_id,$associate_id,$memberId,$ssbId,$createDayBook,$payment_mode,$investmentAccountNoRd,$login_user,$ssb_tran_id_from)
  ...
  {  
    ...
    $ssb_tran_id_from=$ssb_tran_id_from;$ssb_tran_id_to=NULL;
    ...
    $currency_code='INR'; $headPaymentModeRD=0; $payment_type_rd='CR';
    ...
    $created_by=3; $created_by_id=$login_user;
                ...
                if($token == $request->token)
                {
                    ...
                    else
                    {
                        ...
                        if($plan_code==709)
                        {
                            ...
                            $investmentAmount = \App\Models\Investmentplanamounts::where('plan_fa_code',$fa_code)->where('year',$tenure)->select('amount')->first();
            ...
            if ($member) {
                ...
                if($token == $request->token){ 
                        ...
                        { 
                            ...
                            $pid=1;
                            $data_re =\App\Models\Daybook::with(['dbranch','member','associateMember','investment' => function($query){ $query->select('id', 'plan_id','account_number','tenure');}])
            ...
            if ($member) {
                ...
                if($token == $request->token){ 
                        ...
                        { 
                            ...
                            $pid=1;
                            $data_re =\App\Models\SavingAccountTranscation::with(['savingAc','dbranch','associateMember']) ->where('type',2)->where('app_login_user_id',$member->id)->where('is_app',1); 
                            $startDate='';
                            $endDate='';
                                if($request->start_date !='')
                                {                
...
public function registerSSBInvestment(Request $request)
        ...
        $member = Member::select('id','associate_app_status','first_name','last_name')->where('associate_no',$request->associate_no)->where('associate_status',1)->where('is_block',0)->first();
            ...
            if ($member) {
                ...
                if($token == $request->token)
                {
                    ...
                    $memberIdAuto='';$agent_id_associate='';
                    ...
                    else
                    {
                        ...
                        $getState=\App\Models\Branch::where('id',$branch_id)->first();
                        $stateid=$getState->state_id;
                        $globaldate = checkMonthAvailability(date('d'),date('m'),date('Y'),$stateid);
                        Session::put('created_at',$globaldate);
 ...
 //---- ssb acount amount debit  --------
                        ...
                        $cheque_no=NULL; $cheque_id=NULL;$cheque_date=NULL;$online_transction_no=NULL;$online_transction_date=NULL;$online_deposit_bank_id=NULL;$online_deposit_bank_ac_id=NULL;
                    ...
                    // -------------------- head implement ---------------
                        ...
                        $memberFNomi = \App\Models\Memberinvestmentsnominees::create($fNominee);
                        if($request['second_nominee_name']!='' || $request['second_nominee_relation']!='' || $request['second_nominee_gender']!='' || $request['second_nominee_dob']!='' || $request['second_nominee_age']!='' || $request['second_nominee_percentage']!='')
                        {
                            ...
                            $memberSNomi = \App\Models\Memberinvestmentsnominees::create($sNominee);
                        } 
            ...
            if ($member) {
                ...
                if($token == $request->token)
                {
                    ...
                    else
                    {
                    	...
                    	$fa_code = 709;
                            $tenure = 18-$request->age;
                            $investmentAmountK = \App\Models\Investmentplanamounts::where('plan_fa_code',$fa_code)->where('year',$tenure)->select('amount')->first();
 ...
 //---- ssb acount amount debit  --------
            ...
            /* ------------------ commission genarate-----------------*/
                        ...
                        $respayments = \App\Models\Memberinvestmentspayments::create($paymentData); 
