...
class ShareHolderController extends Controller
{
    ...
    // create shareholder and directors
    ...
    public function verifyMember(Request $request)
    {
        ...
        $m = Member::pluck('id','member_id');
        if(isset($m[$m_id])){
            $member = MemberCompany::with('member.memberIdProof','member.memberBankDetails')->where('customer_id',$m[$m_id])->where('company_id',$company_id)->where('is_deleted',0)->first();
           
            
            if ($member) { 
            $idProofs = $member->member->memberIdProof;
            $aadhar = '';
            $panCard = '';         
            if ($idProofs->first_id_type_id == 3) {
                $aadhar = $idProofs->first_id_no;
            } elseif ($idProofs->first_id_type_id == 5) {
                $panCard = $idProofs->first_id_no;
            }
            
            if ($idProofs->second_id_type_id == 3) {
                $aadhar = $idProofs->second_id_no;
            } elseif ($idProofs->second_id_type_id == 5) {
                $panCard = $idProofs->second_id_no;
            }
                ...
                $data = [
                    ...
                    'fullname' =>(($member->member->first_name??'') . ' ' . ($member->member->last_name??'')),
                    'fatherName' => ($member->member->father_husband??''),
                    'address' => ($member->member->address??''),
                    'mobile_no' => ($member->member->mobile_no??''),
                    'aadhar' => $aadhar ?? '',
                    'panCard' => $panCard ?? '',
                    ...
                    'resCount' => 1,
                    'memCreateDate' => $member->re_date,
                    'ssbAccount' => getMemberCompanySsbAccountDetail($member->member->id,$member->company_id),
                    'bankDetails' => $member->member->memberBankDetails,
    ...
    public function verifyssbAccount(Request $request)
    {
        ...
        {
            $member_id = Member::where('member_id', $request->member_id)->first();
            $member = MemberCompany::select('id','company_id')->where('customer_id',$member_id->id)->where('company_id',$request->company_id)->first();
            $data = getMemberCompanySsbAccountDetailNew($member->id,$member->company_id);         
    ...
    public function edit($type, $id)
    {
        ...
        $data['data'] = ShareHolder::with('company:id,name')->where('id', $id)->first();
        $data['member'] = customGetMemberData($data['data']->member_id);
    ...
    public function update(Request $request)
    {
        ...
        try
        {
            ...
            AccountHeads::where('head_id', $request->head_id)->update(['sub_head' => $head_data, 'company_id' => json_encode([(int)$request->company_id])]);
            $data['name'] = $request->name;
            $data['father_name'] = $request->father_name;
            $data['type'] = $request->type;
            $data['address'] = $request->address;
    ...
    public function transfer_share()
    {
        ...
        $data['shareholder'] = ShareHolder::select('id', 'name', 'company_id')
    ...
    // shareholder details on change
    public function get_share_holder_detail(Request $request)    
    {
        $id = $request->id;

        $shareholder = ShareHolder::with('company')
            ->select('id', 'name', 'father_name', 'address', 'aadhar_card', 'pan_card', 'created_at', 'amount', 'current_balance', 'company_id', 'member_id')
            ->find($id);
        ...
        $member = Member::select('id', 'member_id', 'first_name', 'last_name')->find($shareholder->member_id);

        

        $memcompany = $member ? MemberCompany::select('id', 'customer_id', 'company_id')->where('customer_id', $member->id)->where('company_id', $shareholder->company_id)->first() : null;

        $ssb = $memcompany ? \App\Models\SavingAccount::select('id', 'member_id', 'account_no', 'created_at')->where('member_id', $memcompany->id)->first() : null;
        ...
        $data = [
            ...
            'ssb' => $ssb,
            'rgister_date' => $shareholder->created_at->format('d/m/Y'),
            'ssb_created_date' => $ssb ? $ssb->created_at->format('d/m/Y') : ' ',
        ];
    ...
    public function share_holder_deposit_payment()
    {
        ...
        $data['title'] = "Shareholder | Deposit";
        $data['shareholder'] = ShareHolder::select('id', 'name', 'company_id')
        ->whereHas('company', function ($query) {
            $query->where('status', '1')->where('delete', '0');
        })
    ...
    public function director_deposit_payment()
    {
        ...
        $data['title'] = "Director | Deposit";
        $data['directors'] = ShareHolder::select('id', 'name', 'company_id')
        ->whereHas('company', function ($query) {
            $query->where('status', '1')->where('delete', '0');
        })
        ->where('type', 19)
    ...
    public function director_withdrawal_payment()
    {
        ...
        $data['title'] = "Director | Withdrawal";
        $data['directors'] = ShareHolder::select('id', 'name', 'company_id')
        ->whereHas('company', function ($query) {
            $query->where('status', '1')->where('delete', '0');
        })
        ...
        try
        {
            ...
            // bank --- cheque / online
                ...
                //online
                    ...
                    $des1 = $description = getAcountHeadNameHeadId($type_id) . ' withdrawal payment through online NEFT charge ' . $request->utr_number;
                    $description_dr1 = getAcountHeadNameHeadId($type_id) . ' A/c Dr ' . $request->neft_charge . '/-';
                    $description_cr1 = 'To Bank A/c Cr ' . $request->neft_charge . '/-';
                    ...
                    // --------- NEFT charge------------------
                    $allTranNeft = CommanController:: headTransactionCreate($daybook_ref_id, $branch_id, $bank_id, $bank_ac_id , 92, $type, $sub_type, $type_id, $associate_id= NULL, $member_id, $branch_id_to= NULL, $branch_id_from= NULL,  $request->neft_charge, $des, 'DR', $payment_mode, $currency_code,  $v_no,  $ssb_account_id_from, $cheque_no,  $transction_no,  $entry_date, $entry_time, $created_by, $created_by_id, $created_at, $updated_at, $tranId, $jv_unique_id, $ssb_account_id_to, $ssb_account_tran_id_to, $ssb_account_tran_id_from, $cheque_type, $cheque_id,$companyId);

                    $smbdc = CommanController:: branchDaybookCreateModified($daybook_ref_id, $branch_id, $type, $sub_type, $type_id, $associate_id= NULL, $member_id, $branch_id_to= NULL, $branch_id_from= NULL, $request->neft_charge,  $des1, $description_dr1, $description_cr1, 'DR', $payment_mode, $currency_code, $v_no,  $ssb_account_id_from, $cheque_no, $transction_no,  $entry_date, $entry_time, $created_by, $created_by_id,  $created_at, $updated_at, $type_transaction_id, $ssb_account_id_to, $companyId);
                ...
                $smbdc1 = CommanController::NewFieldAddSamraddhBankDaybookCreateModify($daybook_ref_id, $bank_id, $bank_ac_id, $type, $sub_type, $type_id, $associate_id= NULL, $member_id, $branch_id, $opening_balance= NULL, $amount1, $closing_balance = NULL, $des, $description_dr, $description_cr, 'DR', $payment_mode, $currency_code, $amount_to_id, $amount_to_name, $amount_from_id, $amount_from_name, $v_no, $v_date, $ssb_account_id_from, $cheque_no, $cheque_date, $cheque_bank_from, $cheque_bank_ac_from, $cheque_bank_ifsc_from, $cheque_bank_branch_from, $cheque_bank_to, $cheque_bank_ac_to, $transction_no, $transction_bank_from, $transction_bank_ac_from, $transction_bank_ifsc_from, $transction_bank_branch_from, $transction_bank_to, $transction_bank_ac_to, $transction_date, $entry_date, $entry_time, $created_by, $created_by_id, $created_at, $updated_at, $type_transaction_id, $ssb_account_id_to, $cheque_bank_from_id, $cheque_bank_ac_from_id, $cheque_bank_to_name, $cheque_bank_to_branch, $cheque_bank_to_ac_no, $cheque_bank_to_ifsc, $transction_bank_from_id, $transction_bank_from_ac_id, $transction_bank_to_name, $transction_bank_to_ac_no, $transction_bank_to_branch, $transction_bank_to_ifsc, $ssb_account_tran_id_to, $ssb_account_tran_id_from, $jv_unique_id, $cheque_type, $cheque_id, $companyId);
            ...
            // SSB Transaction 
                ...
                $description_cr = 'To cash A/c Cr ' . $amount . '/-';
                $description_dr = getAcountHeadNameHeadId($type_id) . ' A/c Dr ' . $amount . '/-';
                $des1 = $description = getAcountHeadNameHeadId($type_id) . ' deposite payment through Director Withdrwal ' . $ssbAccount;
                $des = $description = getAcountHeadNameHeadId($type_id) . ' Withdrwal payment through SSB A/c ' . $ssbAccount;
                $detail = 'Deposit - director fund';
                $ssbTranCalculation = CommanController::SSBDateDRNewUpdate($ssbId, $ssbAccount, $ssbBalance, $amount, $detail, 'INR', 'CR', 5, $branch_id, $associate_id = NULL, 14, $created_at,$request->company_id);
                $ssbBack = CommanController::SSBBackDateDRNewUpdate($ssbId, $created_at, $amount);
                ...
                $subtype1 = 161;
                // ssb head entry +
                $allTranSSB = CommanController::headTransactionCreate($refId, $branch_id, $bank_id = NULL, $bank_ac_id = NULL,$head4SSB ,15, $subtype1, $ssbId, $associate_id = NULL, $member_id, $branch_id_to= NULL, $branch_id_from= NULL,  $amount, $des1, 'CR', $payment_mode, $currency_code,  $v_no,  $ssb_account_id_from, $cheque_no,  $transction_no,  $entry_date, $entry_time, $created_by, $created_by_id, $created_at, $updated_at, $tranId, $jv_unique_id, $ssb_account_id_to, $ssb_account_tran_id_to, $ssb_account_tran_id_from, $cheque_type, $cheque_id,$companyId);
                ...
                // ssb entry in branch daybook
                $brDaybook1 = CommanController:: branchDaybookCreateModified($refId, $branch_id, $type, $subtype1, $head4SSB, $associate_id= NULL, $member_id= NULL, $branch_id_to= NULL, $branch_id_from= NULL, $amount,  $des1, $description_dr, $description_cr, 'CR', $payment_mode, $currency_code, $v_no,  $ssb_account_id_from, $cheque_no, $transction_no,  $entry_date, $entry_time, $created_by, $created_by_id,  $created_at, $updated_at, $type_transaction_id, $ssb_account_id_to, $companyId);
    ...
    public function save_transfer(Request $request)
    {
        ...
        try
        {
            ...
            $createId = ShareHolder::create($data);
             ...
             $tranId = $createId->id;
             $newShareholderName = $createId->name;
            ...
            $des3 =  'Amount  Transfer  to ' . $newShareholderName;
            ...
            $allHeadTrans = \App\Models\AllHeadTransaction::select('daybook_ref_id', 'branch_id', 'head_id', 'type', 'sub_type', 'amount', 'payment_type', 'company_id')
            ->where('head_id', $shareholder->head_id)
            ->where('payment_type', 'CR')
            ->where('type', 16)
            ->whereIn('sub_type', [161,162])
            ->get();

            $trans = [];
            $branchDaybook = [];
           
            foreach ($allHeadTrans as $aht) {
                $trans[] = [
                    'daybook_ref_id' => $refId,
                    'branch_id' => $aht->branch_id,
                    'bank_id' =>$bank_id,
                    ...
                    'ssb_account_id_from' => $ssb_account_id_from,
                    'ssb_account_id_to'=> $ssb_account_id_to,
                    'ssb_account_tran_id_to' =>$ssb_account_tran_id_to,
                    'ssb_account_tran_id_from' =>$ssb_account_tran_id_from,
                    'cheque_type'=> $cheque_type,
                    'cheque_id'=>$cheque_id,
                    'cheque_no'=>$cheque_no,
                    'transction_no'=>$transction_no,
                    'entry_date' => $entry_date,
                    'entry_time'=>$entry_time,
                  
                    'company_id' => $aht->company_id, 
                ];
            ...
            $shareholderData = \App\Models\AllheadTransaction::insert($trans);
            $branchDaybook = \App\Models\BranchDaybook::insert($branchDaybook);
    ...
    public function getBranches(Request $request){
         $data['branch']=\App\Models\CompanyBranch::where('company_id', $request->company_id)->with('branch:id,name,branch_code')->get();
        return json_encode($data);
    }

    // check aadhar card already exists in shareholder or not
    public function aadharExist(Request $aadhar){
        
        $shareholder = Shareholder::where('aadhar_card', $aadhar['aadhar'])->where('company_id',$aadhar['company_id'])->where('type',$aadhar['type'])->first();
    ...
    // check pan card already exists in shareholder or not
    public function panExist(Request $pan){
    
        $shareholder = Shareholder::where('pan_card', $pan['pan'])->where('company_id',$pan['company_id'])->where('type',$pan['type'])->first();
    ...
    // list director according to company in deposite and withdrawl
    public function directorCompany(Request $request)
    {
        
        $shareholder = shareholder::select('id','name','company_id')
        ->where('type',19)
        ->where('status',1)
        ->where('is_deleted',0)
    ...
    // list shareholder according to company in transfer
    public function shareholderCompany(Request $request)
    {
        $shareholder = shareholder::select('id','name','company_id')
        ->where('company_id',$request->company)
        ->where('type',15)
    ...
    public function shareholderDepositeCompany(Request $request)
    {
        $shareholder = shareholder::select('id','name','company_id')
        ->where('company_id',$request->company)
        ->where('type',15)
