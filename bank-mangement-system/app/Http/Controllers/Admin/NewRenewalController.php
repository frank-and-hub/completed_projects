...
class NewRenewalController extends Controller
{
    ...
    public function renew()
    {
        ...
        return view('templates.admin.investment_management.renewalNew.renew', $data);
    }
    /**
     * Display renew form.
     *
    ...
    //         $getBranchCode = getBranchDetail($branch_id);
    ...
    //         // if ($request['i'] == 1) {
    //         //     //insert in user_log table only once during ajax call
    ...
    //                     $encodeDate = json_encode($ssb);
    //                     // ---------------------------  Day book modify --------------------------
    //                     // $lastAmount = Daybook::where('investment_id',$request['investment_id'][$key])->where('account_no',$accountNumber)->whereIN('transaction_type',[1])->where('created_at','<=',date("Y-m-d ".$entryTime."", strtotime(convertDate($request['renewal_date']))))->orderby('id','desc')->first();
    ...
    //                     if ($lastAmount) {
    //                         $lastBalance = ($lastAmount->opening_balance??0) + $request['amount'][$key];
    //                     } else {
    ...
    //                     $dayBookTransactions['created_by_id'] = Auth::user()->id;
    ...
    //                     $dayBookTransactions['created_by'] = 1;
    ...
    //                     }
    //                     $transactionId = Daybook::insertGetId($dayBookTransactions);
    ...
    //                 // $allAccountNumbers = array_filter($allAccountNumbers, function($value) {
    //                 //     return $value !== null;
    //                 // });
    ...
    //     $currentBalances = http_build_query($currentBalances);


    //    if($ren_dates=='' || $amounts==''){
    //     $baseurl = URL::to('/');
    //     $redirect_url = $baseurl . '/admin/renew/recipt/new/' . $encodeRequests . '/' . $encodebranchCode . '/' . $encodebranchName . '/' . $ssb . '/' . $totalAmount;
    //     return redirect($redirect_url);
    //    }



    //     return redirect()->route('admin.renew.new.sendMessage', [
    ...
    public function storeAjax(RenewalRequest $request)
    {
        // $m = CommanController::renewlimit($request, $this->repository);;
        // if($m['msg']){
    ...
    public function renewalDetails($url, $branchCode, $branchName, $ssb = NULL, $totalAmount = NULL)
    {
        ...
        if (isset($arr_new) && $arr_new != NULL) {
            return view('templates.admin.investment_management.renewalNew.recipt', $arr_new);
        } else {
            return view('templates.admin.investment_management.renewalNew.recipt', $data);
        }
        //End of code
    ...
    public function transactionData($request, $investmentId, $amount)
    {
        ...
        $branchCode = $getBranchCode->branch_code;
        // $sAccount = $this->getSavingAccountDetails($request['member_id']);
        $sAccount = SavingAccount::where('customer_id', $request['member_id'])->where('company_id', $request['company'])->first();
    ...
    public function investHeadCreateSSB($amount, $globaldate, $investmentId, $planId, $cheque_no, $cheque_id, $cheque_date, $online_transction_no, $online_transction_date, $online_deposit_bank_id, $online_deposit_bank_ac_id, $branch_id, $associate_id, $memberId, $ssbId, $createDayBook, $payment_mode, $investmentAccountNoRd, $ssbFromId, $ssbAccountTranFromId, $ssbToId, $ssbAccountTranToId, $companyId)
    {
        ...
        if ($payment_mode == 1) {  // cheque moade
            ...
            //bank head entry
            $allTranRDonline = CommanController::createAllHeadTransaction(
                $refIdRD,
                $branch_id,
                $bank_id = NULL,
                $bank_ac_id = NULL,
            ...
            // branch cash  head entry +
            $allTranRDcash = CommanController::createAllHeadTransaction(
                $refIdRD,
                $branch_id,
                $bank_id = NULL,
                $bank_ac_id = NULL,
        ...
        //branch day book entry +
        $daybookInvest = CommanController::branchDayBookNew(
            $refIdRD,
            $branch_id,
            $type,
            $sub_type,
        ...
        // Investment head entry +
        $allTranInvest = CommanController::createAllHeadTransaction(
            $refIdRD,
            $branch_id,
            $bank_id = $chequeDetail->deposit_bank_id ?? NULL,
            $bank_ac_id = $chequeDetail->deposit_account_id ?? NULL,
    ...
    {
        if (check_my_permission(Auth::user()->id, "280") != "1") {
            return redirect()->route('admin.dashboard');
        }
    ...
    {
        $response = CommanController::renewlimit($request, $this->repository);
        return response()->json($response);
    }
    ...
    public function send_message($allContactNumbers, $allAccountNumbers, $rAmounts, $encodeRequests, $encodebranchCode, $encodebranchName, $ssb, $totalAmount, $amount, $ren_dates)
    {
        ...
        // ->whereIn('account_number', $allAccountNumbers)
        // ->get();
        // $currentBalancesArray = [];
        // foreach ($currentBalances as $balance) {
        //     $currentBalancesArray[$balance->account_number] = $balance->totalBalance;
        // }
        // $currentBalancesArray = http_build_query($currentBalancesArray);
        // parse_str($currentBalancesArray, $currentBalancesArray);



        if (!empty($allContactNumbers)) {
