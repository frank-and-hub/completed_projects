<?php
namespace App\Http\Controllers\CommonController\Member;
...
use App\Models\{MemberLog, Employee, Designation};
...
class MemberController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('templates.CommonViews.Member.Register.index', $data);
    }
    /**
     * Get members list
        ...
        if ($request->ajax() && check_my_permission(Auth::user()->id, "3") == "1") {
            ...
            if (isset($arrFormData['is_search']) && $arrFormData['is_search'] == 'yes') {
                $data = MemberCompany::select(
                    ...
                    ->has('company')
                    ->with(['branch:id,name,branch_code,sector,zone'])
                                ...
                                ->with([
                                    'memberIdProof' => function ($q) {
                                        $q->select('id', 'first_id_no', 'second_id_no', 'member_id', 'first_id_type_id', 'second_id_type_id')
                                            ->with(['idTypeFirst:id,name'])
                                            ->with(['idTypeSecond:id,name']);
                                    }
                                ])
                                ->with(['children:id,first_name,last_name'])
                                ->with([
                                    'memberNomineeDetails' => function ($q) {
                                        $q->select('id', 'name', 'relation', 'age', 'member_id', 'gender')
                                            ->with(['nomineeRelationDetails:id,name']);
                                    }
                                ]);
                        }
                    ])->with([
                            'savingAccountNew' => function ($q) {
                                $q->select('id', 'account_no', 'member_id', 'company_id');
                            }
                ...
                if ($arrFormData['start_date'] != '') {
                    ...
                    // $data=$data->whereBetween(\DB::raw('DATE(re_date)'), [$startDate, $endDate]);
                    $data = $data->whereBetween('re_date', [$startDate, $endDate]);
                }
                /******* fillter query End ****/
                $count = $data->orderby('id', 'DESC')->count('id');
                // $count=count($data1);
                $data = $data->orderby('id', 'DESC')->offset($_POST['start'])->limit($_POST['length'])
                    ->get(['id', 're_date', 'member_id', 'company_id', 'associate_id', 'is_block', 'customer_id', 'branch_id', 'created_at', 'status']);
                $totalCount = $count;
                $sno = $_POST['start'];
                $rowReturn = array();
                foreach ($data as $row) {
                    ...
                    if ($row->savingAccountNew) {
                        $accountNo = $row->savingAccountNew->account_no ?? 'N/A'; //getMemberSsbAccountDetail($row->id)->account_no;
                    }
                    ...
                    $val['associate_name'] = $memberchildfname . ' ' . $memberchildlname; //getSeniorData($row->associate_id,'first_name').' '.getSeniorData($row->associate_id,'last_name');
                    ...
                    $val['secondId'] = $idTypeSecond . ' - ' . $second_id_no; //getIdProofName($idProofDetail->second_id_type_id).' - '.$idProofDetail->second_id_no;
                    $val['address'] = $row['member']->address ?? 'N/A';
                    $val['state'] = $row['member']['states']->name ?? 'N/A'; //getStateName($row->state_id);
                    $val['district'] = $row['member']['district']->name ?? 'N/A'; //getDistrictName($row->district_id);
                    $val['city'] = $row['member']['city']->name ?? 'N/A'; //getCityName($row->city_id);
                    ...
                    $url = URL::to("admin/member-detail/" . $row->id . "?type=0");
                    ...
                    if (($row['member']) && ($row['member']['memberIdProof']['first_id_type_id'] == '5' || $row['member']['memberIdProof']['second_id_type_id'] == '5')) {
    ...
    public function register()
    {
        ...
        return view('templates.CommonViews.Member.Register.member_register', $data);
    }
    /**
    ...
    public function detail($id)
    {
        ...
        $recipt = '';
        if ($_GET['type'] == 0) {
            $data['membercompany'] = MemberCompany::has('company')
                ->with([
                    'memberAssociate' => function ($q) {
                        $q->select('id', 'first_name', 'last_name', 'associate_no', 'member_id', 'company_id');
                    }
                ])
                ->where('id', $id)->first();
            ...
            ])->where('id', $data['membercompany']->customer_id)->first();
        } else {
            $data['memberDetail'] = Member::with([
                'children' => function ($q) {
                    $q->select(['id', 'first_name', 'last_name', 'associate_no', 'member_id']);
        ...
        $data['bankDetail'] = \App\Models\MemberBankDetail::where('member_id', $data['memberDetail']->id)->first();
        $data['nomineeDetail'] = \App\Models\MemberNominee::where('member_id', $data['memberDetail']->id)->first();
        $data['idProofDetail'] = \App\Models\MemberIdProof::where('member_id', $data['memberDetail']->id)->first();
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            if ($request['id'] == 0) {
                ...
                if ($request['branch_id'] == '' && !isset($request['branch_id'])) {
            ...
            if ($request['id'] == 0) {
                ...
                $data['created_at'] = date('Y-m-d', strtotime(convertdate($request['created_at'] ?? $request['application_date'])));
                $member = Member::create($data);
                $memberId = $member->id;
                $customerid = $member->member_id;

                Employee::where('employee_code', $request['employee_code'])->update(['is_customer' => '1', 'customer_id' => $memberId, 'customer_code' => $customerid]);
            ...
            if ($request->hasFile('signature')) {
                ...
                $signature_filename = $memberId . '_s_' . time() . '.' . $signature_image->getClientOriginalExtension();
                $signature_location = '/profile/member_signature/' . $signature_filename;
            ...
            if ($request->hasFile('photo')) {
                ...
                $photo_filename = $memberId . '_m_' . time() . '.' . $photo_image->getClientOriginalExtension();
                $photo_location = '/profile/member_avatar/' . $photo_filename;
                ...
                if ($request->hidden_photo != null && $request->hidden_photo != 'noimage') {
                    ImageUpload::ImageCopy($request->hidden_photo, 'asset/employee/', 'asset/profile/member_avatar/');
                    $memberUpdate->photo = $request->profile_photo;
                }
            }
            if ($imgChk > 0) {
        ...
        if ($request['id'] == 0) {
            if ($memberId) {
                ...
                if ($admin == true) {
                    return redirect()->route('admin.member.index')->with('success', 'Member registered successfully! ' . $msg);
                } else {
            ...
            if (isset($request['action']) && $request['action'] == 'change-request') {
                ...
                return Redirect::to('admin/member/corrections')->with('success', 'Member updated successfully!');
            } else {
    ...
    public function getData($request, $type)
    {
        ...
        if ($type == 'create') {
            ...
            $data['dob'] = date("Y-m-d", strtotime(convertDate($request['dob'] ?? null)));
            $data['gender'] = $request['gender'];
        }
        $data['form_no'] = $request['form_no'] ?? null;
        $data['current_carder_id'] = $request['carder_id'] ?? '0';
        $data['photo'] = $request['profile_photo'] ?? null;
        $data['employee_id'] = $request['emp_id'] ?? null;
        $data['is_employee'] = $request['isEmployee'] ?? null;
        $data['employee_code'] = $request['employee_code'] ?? null;
        $data['associate_id'] = $request['associate_id'] ?? null;
        $data['associate_code'] = $request['associate_code'] ?? null;
        $data['last_name'] = $request['last_name'] ?? null;
        $data['age'] = $request['age'] ?? null;
        $data['occupation_id'] = $request['occupation'] ?? null;
        $data['annual_income'] = $request['annual_income'] ?? null;
        $data['mother_name'] = $request['mother_name'] ?? null;
        $data['father_husband'] = $request['f_h_name'] ?? null;
        $data['marital_status'] = $request['marital_status'] ?? null;
        $data['anniversary_date'] = date("Y-m-d", strtotime(convertDate($request['anniversary_date'] ?? null)));
        $data['religion_id'] = $request['religion'] ?? null;
        $data['special_category_id'] = $request['special_category'] ?? null;
        $data['state_id'] = $request['state_id'] ?? null;
        $data['district_id'] = $request['district_id'] ?? null;
        $data['city_id'] = $request['city_id'] ?? null;
        $data['village'] = $request['village_name'] ?? null;
        $data['pin_code'] = $request['pincode'] ?? null;
    ...
    public function getBankData($request, $type, $memberId)
    {
        ...
        $data['address'] = trimData($request['bank_branch_address']);
        return $data;
    }
    /**
    ...
    public function getIdProofData($request, $type, $memberId)
    {
        ...
        $data['member_id'] = $memberId;
        $data['first_id_type_id'] = $request['first_id_type'] ?? 0;
        $data['first_id_no'] = $request['first_id_proof_no'] ?? null;
        $data['first_address'] = trimData($request['first_address_proof'] ?? null);
        $data['second_id_type_id'] = $request['second_id_type'] ?? 0;
        $data['second_id_no'] = $request['second_id_proof_no'] ?? null;
        $data['second_address'] = trimData($request['second_address_proof'] ?? null);
    ...
    public function imageUpload(Request $request)
    {
        ...
        $memberUpdate->created_at = date('Y-m-d H:i:s', strtotime(convertDate($request['created_at'])));
        $memberUpdate->save();
    ...
    public function account($id)
    {
        ...
        if (!empty($data['accountDetail'])) {
            $data['accountTranscation'] = \App\Models\SavingAccountTranscation::where('saving_account_id', $data['accountDetail']->id)->orderby('created_at', 'DESC')->limit(10)->get();
    ...
    public function statement_filter(Request $request)
    {
        ...
        $data['accountTranscation'] = \App\Models\SavingAccountTranscation::where('saving_account_id', $request['id'])->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->orderby('created_at', 'DESC')->get();
    ...
    public function transactionsListing(Request $request)
    {
        if ($request->ajax()) {
            ...
            $data = MemberTransaction::with(['memberTransaction', 'memberTransactionBranch'])->where('customer_id', $id)->where('is_deleted', 0);
            if (isset($arrFormData['is_search']) && $arrFormData['is_search'] == 'yes') {
                if ($arrFormData['start_date'] != '') {
                    $startDate = date("Y-m-d", strtotime(convertDate($arrFormData['start_date'])));
                    if ($arrFormData['end_date'] != '') {
            ...
            return Datatables::of($data)
                ...
                ->addColumn('branch_name', function ($row) {
                    ...
                    if ($row->memberTransactionBranch) {
                        $branch_name = $row->memberTransactionBranch->name . '(' . $row->memberTransactionBranch->branch_code . ')';
                    }
                    return $branch_name;
                })
    ...
    {
        if ($request->first_id_type == 3 || $request->first_id_type == 5 || $request->second_id_type == 3 || $request->second_id_type == 5 || $request->first_id_type == 2 || $request->second_id_type == 2) {
            $isexist = MemberIdProof::join('members', 'member_id_proofs.member_id', '=', 'members.id')
                ...
                })->with('member:id,member_id')
                ->get(['member_id_proofs.member_id','second_id_no', 'first_id_no']);
            ...
            if ($counts > 0) {
                ...
                $e = $isexist[0]['member']['member_id'];
            }
            return response()->json(['first_id_type' => $f, 'second_id_type' => $s,'c_id' => $e, 'msg' => 'exists', 'status' => 200]);
        } else {
            return MemberIdProof::join('members', 'member_id_proofs.member_id', '=', 'members.id')
        ...
        if ($request->ajax() && check_my_permission(Auth::user()->id, "3") == "1") {
            ...
            if (isset($arrFormData['is_search']) && $arrFormData['is_search'] == 'yes') {
                ...
                foreach ($data as $row) {
                    ...
                    if (isset($row['branch']) && isset($row['branch']->name) && isset($row['branch']->branch_code)) {
                    ...
                    if ($row->id) {
                        $relation_id = $NomineeDetail->relation ?? 'N/A';
                        if ($relation_id) {
                            $val['relation'] = $NomineeDetail['nomineeRelationDetails']->name ?? 'N/A';
                        }
                    }
                    if ($NomineeDetail) {
                    ...
                    $val['is_upload'] = $is_upload;
                    if ($row['memberIdProof'] && $row['memberIdProof']['idTypeFirst']) {
                        $val['firstId'] = $row['memberIdProof']['idTypeFirst']->name . ' - ' . $row['memberIdProof']->first_id_no;
                    } else {
                        $val['firstId'] = 'N/A';
                    }
                    if ($row['memberIdProof'] && $row['memberIdProof']['idTypeSecond']) {
    ...
    public function membersSaving(Request $request)
    {
        if ($request->ajax()) {
            $memberId = $request['member_id'];
            $data = \App\Models\SavingAccount::has('company')->with([
                'getMemberinvestments:plan_id,id,form_number',
                'savingBranch:id,name,branch_code',
                'getMemberinvestments.plan:id,name',
                'customerSSB:member_id,id,first_name,last_name',
                'company:id,name'
            ])
    ...
    {
        $data['title'] = 'Member Logs';
        $data['log'] = MemberLog::where('customer_id', $id)->orderBy('created_at', 'DESC')->get();
        return view('templates.admin.member.member_logs', $data);
    }

    /*Fetch the Employee Data as per the emp_code
    ...
    public function empDetail(Request $request)
    {
        try {
            ...
            $member_details = Member::where('employee_code', $emp_code)->first();

            if ($member_details == '' && $member_details == null) {
                $emp_detail = Employee::with([
                    'designations:id,designation_name',
                    'branch:id,name'
                ])
                    ->where('employee_code', $emp_code)
                    ->select('id AS emp_id', 'branch_id', 'employee_name', 'designation_id', 'email', 'mobile_no', 'gender', 'mother_name', 'father_guardian_name', 'marital_status', 'dob', 'status', 'bank_name', 'bank_branch_name', 'bank_account_no', 'bank_ifsc_code', 'bank_address', 'photo', 'aadhar_card', 'pen_card', 'voter_id', 'permanent_address', 'salary', 'created_at')
                    ->where('is_terminate', '0')
                    ->where('is_resigned', '0')
                    ->whereStatus('1')
                    ->first();
                if (!empty($emp_detail)) {
                    $folderName = 'employee/' . $emp_detail->photo;
                    if ($emp_detail->photo != null) {
                    ...
                    $dob = date('d/m/Y', strtotime(convertDate($emp_detail['dob'])));
                    return response()->json(['data' => $emp_detail, 'photo' => $photo_url, 'dob' => $dob]);
                } else {
                    $message = (Employee::where('employee_code', $emp_code)->where('status', '0')->exists()
                        ? 'Please check Provided Employee is Inactive Or Deleted'
                        : (Employee::where('employee_code', $emp_code)->where('is_resigned', '1')->exists()
                            ? 'Please check Provided Employee is on Resigned'
                            : (Employee::where('employee_code', $emp_code)->where('is_terminate', '1')->exists()
                                ? 'Please check Provided Employee is Terminated'
                                : 'Data Not Found..!!'))
                    );
