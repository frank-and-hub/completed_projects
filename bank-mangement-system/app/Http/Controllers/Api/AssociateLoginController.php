...
class AssociateLoginController extends Controller
{
    ...
    public function sendOtp(Request $request)
    {
        ...
        try {
            if ($member > 0) {
                ...
                $sms_text = 'Your Associate APP Login OTP is ' . urlencode($otp) . ' From Samraddh Bestwin Micro finance Association X9VGltZrsl0';
                ...
                // curl_setopt($ch, CURLOPT_POSTFIELDS, "key=26059BB05DCA39&routeid=7&campaign=11207&type=text&contacts=" . $contacts . "&senderid=SBMFAP&msg=" . $sms_text . "&template_id=1207168578819998483");
                curl_setopt($ch, CURLOPT_POSTFIELDS, "key=26059BB05DCA39&routeid=7&campaign=11207&type=text&contacts=" . $contacts . "&senderid=SBMFAP&msg=" . $sms_text . "&template_id=1207170289144902118");
                ...
                return response()->json(compact('versionCode', 'status', 'code', 'messages', 'result', 'token', 'associate_status'), $code);
            } else {
    ...
    public function loginWithUpi(Request $request)
    {
        ...
        if (!isset($request->version)) {
            ...
            $code = 201;
            $token = '';
            $messages = 'To continue with the login. Please update your app.';
            $result = ['member' => $data];
            $associate_status = 9;
            return response()->json(compact('status', 'code', 'messages', 'result', 'token', 'associate_status'), $code);
        }
        ...
        try {
            ...
            $mDetails = Member::select('id', 'token', 'device_id', 'mobile_token', 'branch_id', 'associate_no', 'first_name', 'last_name', 'associate_senior_id', 'associate_app_status', 'photo')->with('branch')->where('associate_no', $request->associate_no)->where('associate_upi', $request->upi)->where('is_block', 0)->where('associate_status', 1)->first();
            ...
            if ($member > 0) {
                $token = md5($request->associate_no);
                $statusMain = CheckUserLoginExist::userCheck($request, $mDetails, 4);
                if ($token == $request->token) {
                    ...
                    $data['profile_imge'] = $mDetails->photo ? ImageUpload::fileExists('profile/member_avatar/' . $mDetails->photo) ? ImageUpload::generatePreSignedUrl('profile/member_avatar/' . $mDetails->photo) : env('APP_URL') . '/asset/images/no-image.png' : env('APP_URL') . '/asset/images/no-image.png';
            ...
            if ( $member > 0) {
                ...
                if($token == $request->token){
                    $data = array() ;
                            ...
                            $data['profile_imge']  = isset($memberData->photo) ? ImageUpload::fileExists('profile/member_avatar/' . $memberData->photo) ? ImageUpload::generatePreSignedUrl('profile/member_avatar/' . $memberData->photo) : 'http://uat.samraddhbestwin.com/asset/images/no-image.png' : 'http://uat.samraddhbestwin.com/asset/images/no-image.png';
                           
                            $data['signatureurl'] = isset($memberData->signature) ? ImageUpload::fileExists('profile/member_signature/' . $memberData->signature) ? ImageUpload::generatePreSignedUrl('profile/member_signature/' . $memberData->signature) : 'http://uat.samraddhbestwin.com/asset/images/no-image.png' : 'http://uat.samraddhbestwin.com/asset/images/no-image.png';
    ...
    public function send_payment_otp(Request $request)
    { 
     
         $member = Member::where('associate_no',$request->associate_no)->where('associate_status',1)->where('is_block',0)->count();
            $mDetails = Member::select('id','mobile_no','associate_app_status','member_id')->where('associate_no',$request->associate_no)->where('associate_status',1)->where('is_block',0)->first();
            $type= $request->type;    
        try {
            ...
            if ( $member > 0) {
                ...
                if($type == 1){
                    $member=Member::with('memberinvestments')->find($mDetails->id);
                    $member->associate_otp=$otp;
                    $member->associate_varifiy_time=time();
                    $member->associate_is_varified=1;
                    $member->save();

                    if(isset($member['memberinvestments'][0]->id)){
                        $status   = "Success";
                        $code     = 200;
                        $messages = 'Member investment type exist OTP send successfully!';
                        $result   = $otp;
                        $associate_status=$mDetails->associate_app_status;
                        return response()->json(compact('status', 'code', 'messages', 'result','token','associate_status'), $code);
                    }else{
                        $status   = "error";
                        $code     = 200;
                        $messages = 'Member investment type not exists!';
                        $result   = $otp;
                        $associate_status=$mDetails->associate_app_status;
                        return response()->json(compact('status', 'code', 'messages', 'result','token','associate_status'), $code);
                    }
                    ...
                    $member=Member::with('memberloans')->find($mDetails->id);
                    $member->associate_otp=$otp;
                    $member->associate_varifiy_time=time();
                    $member->associate_is_varified=1;
                    $member->save();
                    ...
                    if(isset($member['memberinvestments'][0]->id)){
                        $status   = "Success";
                        $code     = 200;
                        $messages = 'Member Loan type exist OTP send successfully!';
                        $result   = $otp;
                        $associate_status=$mDetails->associate_app_status;
                        return response()->json(compact('status', 'code', 'messages', 'result','token','associate_status'), $code);
                    }else{
                        $status   = "error";
                        $code     = 200;
                        $messages = 'Member Loan type not exists!';
                        $result   = $otp;
                        $associate_status=$mDetails->associate_app_status;
                        return response()->json(compact('status', 'code', 'messages', 'result','token','associate_status'), $code);
                    }
                }else if($type == 3){
                    $member=Member::with('grouploans')->find($mDetails->id);
                    $member->associate_otp=$otp;
                    $member->associate_varifiy_time=time();
                    $member->associate_is_varified=1;
                    $member->save();
                    ...
                    if(isset($member['memberinvestments'][0]->id)){
                        $status   = "Success";
                        $code     = 200;
                        $messages = 'Member Group Loan type exist OTP send successfully!';
                        $result   = $otp;
                        $associate_status=$mDetails->associate_app_status;
                        return response()->json(compact('status', 'code', 'messages', 'result','token','associate_status'), $code);
                    }else{
                        $status   = "error";
                        $code     = 200;
                        $messages = 'Member Group Loan type not exists!';
                        $result   = $otp;
                        $associate_status=$mDetails->associate_app_status;
                        return response()->json(compact('status', 'code', 'messages', 'result','token','associate_status'), $code);
                    }
                    ...
                    $status = "Error";
                    $code = 201;
                    $messages = "Associate type not Found";
                    $result = '';
                    $token = '';
                    $associate_status=9;
                    return response()->json(compact('status', 'code', 'messages', 'result','token','associate_status'), $code);
