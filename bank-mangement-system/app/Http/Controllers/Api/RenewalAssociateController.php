...
class RenewalAssociateController extends Controller
{
    public function __construct(Request $request, RepositoryInterface $repository)
    {
        ...
        $this->member = \App\Models\Member::with(['savingAccount_Customnew.savingAccountBalance', 'branch'])
            ->select('id', 'associate_app_status', 'member_id', 'branch_id', 'associate_code')
        ...
        try {
            if ($member) {
                ...
                if ($token == $request->token) {
                    ...
                    $investmentDetails = Memberinvestments::whereHas('company')->with('member')
                    ...
                    $data = array();
                    $newMember = \App\Models\Member::with([
                        'savingAccount_Custom3' => function ($q) use ($companyId) {
                        ...
                        ->where('is_block', 0)
                        ->first();
                    $getSSb = SavingAccount::whereHas('company')->where('customer_id', $newMember->id)->where('company_id', $companyId)->first();
                    foreach ($accountNumber as $key => $value) {
                        // dd($investmentDetails[$value]);
    ...
    public function submitRenewals(Request $request)
    {
        ...
        $globaldate = $this->globaldate; // current date
            ...
            if ($member) {
                ...
                if ($token == $request->token) {
                    ...
                    foreach ($accountNumberDetails as $key => $account) {
                        $investmentRecord = \App\Models\Memberinvestments::whereHas('company')
                        ->with([
                            'plan',
                            'assoCiateSSbAccount' => function ($q) use ($companyId) {
                        ...
                        //     ->where('is_block', 0)
                        //     ->get();
                        $recordi = \App\Models\Memberinvestments::find($investmentRecord->id);
                        if (($recordi) && strtotime($recordi->maturity_date) < strtotime($globaldate)) {
                            $status = "Error";
                            $code = 201;
                            $message = 'Your account maturity date has expired ! ' . $account['account_number'];
                            $result = '';
                        ...
                        $newMember = \App\Models\Member::with([
                            ...
                            ->where('is_block', 0)
                            ->first();
                        $getSSb = SavingAccount::whereHas('company')->with('savingAccountBalance')->where('customer_id', $newMember->id)->where('company_id', $companyId)->first();
                        ...
                        if (isset($investmentRecord->id)) {
                            ...
                            if ($request['payment_mode'] == 0) {
                                ...
                                $req = [
                                    ...
                                    'amount' => $account['deposite_amount'],
    ...
    public function transactionData($request, $investmentId, $amount)
    {
        ...
        $sAccount = SavingAccount::whereHas('company')->where('customer_id', $request['member_id'])->where('company_id', $request['company_id'])->first();
    ...
    {
        $getDetails = \App\Models\SavingAccount::whereHas('company')->where('member_investments_id', $investmentId)->select('id', 'balance', 'account_no', 'company_id')->first();
        return $getDetails;
    }
    public function getSavingAccountDetails($mId, $companyId)
    {
        $getDetails = \App\Models\SavingAccount::whereHas('company')->where('member_id', $mId)->whereCompanyId($companyId)->select('id', 'balance', 'account_no', 'company_id')->first();
        return $getDetails;
    }
    /**
    ...
    public function collectionReport(Request $request)
    {
        ...
        try {
            if ($token == $checkToken) {
                $transactionRecords = \App\Models\Daybook::whereHas('company')->select('id', 'created_at', 'deposit', 'account_no', 'member_id', 'is_app', 'transaction_type', 'company_id')->whereIn('transaction_type', [2, 4])->where('associate_id', $member->id)->where('is_deleted', 0);
