...
class LoginController extends Controller
{
    ...
    public function sendOtp(Request $request)
    {
        ...
        try {
            if ($member > 0) {
                ...
                // $sms_text = 'Your OTP is '. urlencode($otp) . ' From SBMFA F0BMC1IosIy';
                ...
                $result   = '';
                return response()->json(compact('status', 'code', 'messages', 'result', 'token', 'versionCode'), $code);
            } else {
                $status = "Error";
                $code = 201;
     ...
     */
     ...
     public function loginWithUpi(Request $request)
     {
         ...
         $mDetails = Member::select('id', 'token', 'device_id', 'mobile_token','e_passbook_device_id','e_passbook_mobile_token')->where('member_id', $request->member_id)->where('upi', $request->upi)->where('is_block', 0)->first();
         $versionCode = '1.6';
         try {
             ...
             if ($member > 0) {
                 ...
                 $statusMain = CheckUserLoginExist::userEpassBookCheck($request, $mDetails, 4);
                 ...
                 if ($token == $request->token) {
                     ...
                     $result   = '';
                     return response()->json(compact('status', 'code', 'messages', 'result', 'versionCode'), $code);
                 } else {
                     $status = "Error";
                     $code = 201;
                     $messages = 'API token mismatch!';
