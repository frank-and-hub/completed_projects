...
class AssociateLoanController extends Controller
{
    ...
    public function loanList(Request $request)
    {
        ...
        try {
            ...
            // $member = Member::with(['savingAccount_Custom.savingAccountBalance:id,member_id,associate_app_status'])
            ...
            if ($member) {
                ...
                if ($token == $request->token) {
                    if ($request->page > 0 && $request->length > 0) {
                        $data = array();
                        $loan_data = Memberloans::select('id', 'applicant_id', 'account_number', 'approve_date', 'emi_option', 'emi_period', 'deposite_amount', 'amount', 'status', 'loan_type', 'associate_member_id', 'branch_id', 'created_at', 'approved_date', 'emi_amount', 'transfer_amount','customer_id')->with(['loan:id,name', 'loanMember:associate_code,id,first_name,last_name,member_id','loanMemberCompany' => function ($q) {
                            $q->select('id','member_id')
                            ->with(['ssb_detail'=>function($q1){ $q1->select('id','account_no','member_id','customer_id')
                                ->with(['getSSBAccountBalance']);
                              }]); 
                        }])->with(['loanBranch' => function ($query) {
                            $query->select('id', 'name', 'branch_code', 'sector', 'regan', 'zone');
                        }])->where('loan_type', '!=', 3)->where('associate_member_id', $member->id);
                        ...
                        foreach ($loan_data as $key => $row) {
                            ...
                            $data[$key]['customer_id'] = $row['loanMember']->member_id;
                            ...
                            $data[$key]['last_recovery_date'] = $last_recovery_date;
                            if ($row->emi_option == 1) {
                            ...
                            if (loanOutsandingAmount($row->id, $row->account_number)) {
                                $total_payment =  loanOutsandingAmount($row->id, $row->account_number);
                            } else {
                                $total_payment = 'N/A';
                            }
                            $totalbalance = $row->emi_period * $row->emi_amount;
                            $Finaloutstanding_amount = $totalbalance - $row->received_emi_amount;
                            //dd($Finaloutstanding_amount);
                            $data[$key]['out_standing_amount'] = number_format((float)$Finaloutstanding_amount, 2, '.', '');
                            $data[$key]['associate_code'] = $row['loanMember']->associate_code ?? '';
                            ...
                            $data[$key]['totalAmount']  = $row['loanMemberCompany']['ssb_detail']['getSSBAccountBalance']['totalBalance']??0;
                            $data[$key]['AccountNumber'] = $row['loanMemberCompany']['ssb_detail']['getSSBAccountBalance']['account_no']??0;
                        }
                        $status   = "Success";
                        $code     = 200;
                        ...
                        // dd($tree_array1); 
                        $loan_data = Memberloans::with('loan', 'loanMember', 'loanMemberAssociate')->with(['loanBranch' => function ($query) {
                            $query->select('id', 'name', 'branch_code', 'sector', 'regan', 'zone');
                        }])->where('loan_type', '!=', 3)->whereIn('associate_member_id', $tree_array1);
                        $loan_data1 = $loan_data->orderby('created_at', 'DESC')->get();
                        $count = count($loan_data1);
                        $loan_data = $loan_data->orderby('created_at', 'DESC')->get();
                        foreach ($loan_data as $key => $row) {
                            $data[$key]['id'] = $row->id;
                            if ($row->group_activity == 'Group loan application') {
                                if ($row->groupleader_member_id) {
                        ...
                        $code     = 200;
                        $messages = 'Loan listing!';
                        $result   = ['member' => $data, 'total_count' => $count];
                        $associate_status = $member->associate_app_status;
                        return response()->json(compact('status', 'code', 'messages', 'result', 'associate_status'), $code);
                    } else {
                        $status = "Error";
            ...
            if ($member) {
                ...
                if ($token == $token) {
                    if ($request->page > 0 && $request->length > 0) {
                        ...
                        //     }                           
                        //  }
                        $loan_data = Grouploans::with('loanMembers', 'loanMemberAssociatec')->with(['gloanBranch'])
                        ->with(['loan' => function ($q) {
                            $q->select('id', 'name', 'loan_type');
                        }])
                        ->with(['loanMemberCompanyid'=>function($q){ 
                            $q->select('id','member_id')
                            ->with(['ssb_detail'=>function($q1){ $q1->select('id','account_no','member_id','customer_id')
                                ->with(['getSSBAccountBalance']);
                              }]); 
                        }])
                        ->where('associate_member_id', $member->id);
                        $loan_data1 = $loan_data->orderby('created_at', 'DESC')->get();
                        $count = count($loan_data1);
                        if ($request->page == 1) {
                            $start = 0;
                        ...
                        $loan_data = $loan_data->orderby('created_at', 'DESC')->offset($start)->limit($request->length)->get();
                        // echo '<pre>';
                        // print_r($loan_data->toArray());
                        // die;
                        if ($loan_data[0]['loan_type'] != 3) {
                            $status = "Error";
                            $code = 201;
                            $messages = 'There is no group loan on this id!';
                            $result = '';
                            $associate_status = 9;
                            return response()->json(compact('status', 'code', 'messages', 'result', 'associate_status'), $code);
                        }
                        foreach ($loan_data as $key => $row) {
                            ...
                            $data[$key]['member_loan_id'] = $row->member_loan_id;
                            // echo '<pre>';
                            // print_r($row->toArray());
                            // die;
                            $data[$key]['applicant_name'] = $row['loanMemberAssociatec']->first_name . ' ' . $row['loanMemberAssociatec']->last_name;
                            $data[$key]['account_number'] = $row->account_number;
                            ...
                            $data[$key]['customer_id'] = $row['loanMemberCompanyid']->member_id;
                            $data[$key]['member_id'] = $row['loanMembers']->member_id;
                            $data[$key]['member_name'] = $row['loanMembers']->first_name.' '.$row['loanMembers']->last_name;
                            if ($row->emi_option == 1) {
                                $tenure =  $row->emi_period . ' Months';
                            } elseif ($row->emi_option == 2) {
                                $tenure =  $row->emi_period . ' Weeks';
                            ...
                            $totalbalance = $row->emi_period * $row->emi_amount;
                            $Finaloutstanding_amount = $totalbalance - $row->received_emi_amount;
                            $data[$key]['outstanding_amount'] = $Finaloutstanding_amount;
                            $data[$key]['last_recovery_date'] = date("d/m/Y", strtotime($row->closing_date));
                            $data[$key]['associate_code'] = $row['loanMemberAssociate']->associate_no;
                            $m = Member::where('id', $row->associate_member_id)->first(['id', 'first_name', 'last_name']);
                            $associate_name = $m->first_name . ' ' . $m->last_name;
                            $data[$key]['associate_name'] = $associate_name;
                            ...
                            $data[$key]['total_payment'] =  $total_payment;
                            $data[$key]['acc_no'] = $row['loanMemberCompanyid']['ssb_detail']['getSSBAccountBalance']['account_no'];
                            $data[$key]['ssb_balance'] = $row['loanMemberCompanyid']['ssb_detail']['getSSBAccountBalance']['totalBalance'];
                        }
                        $status   = "Success";
                        $code     = 200;
                        ...
                        $loan_data = Grouploans::with('loanMembers', 'loanMemberAssociate')->with(['gloanBranch' => function ($query) {
                            $query->select('id', 'name', 'branch_code', 'sector', 'regan', 'zone');
                        }])->whereIn('associate_member_id', $tree_array1);
                        $loan_data1 = $loan_data->orderby('created_at', 'DESC')->get();
                        $count = count($loan_data1);
                        $loan_data = $loan_data->orderby('created_at', 'DESC')->get();
                        foreach ($loan_data as $key => $row) {
                            ...
                            $data[$key]['group_loan_id'] = $row->group_loan_common_id;
                            $data[$key]['member_loan_id'] = $row->member_loan_id;
                            $data[$key]['applicant_name'] = getMemberData($row->applicant_id)->first_name . ' ' . getMemberData($row->applicant_id)->last_name;
                            $data[$key]['account_number'] = $row->account_number;
                            $data[$key]['br_name'] = $row['gloanBranch']->name;
                            $data[$key]['br_code'] = $row['gloanBranch']->branch_code;
                            $data[$key]['so_name'] = $row['gloanBranch']->sector;
    ...
    public static function createLoanDayBook($loan_type, $loan_sub_type, $loan_id, $group_loan_id, $account_number, $applicant_id, $roi_amount, $principal_amount, $opening_balance, $deposit, $description, $branch_id, $branch_code, $payment_type, $currency_code, $payment_mode, $cheque_dd_no, $bank_name, $branch_name, $payment_date, $online_payment_id, $created_by, $status, $cheque_date, $bank_account_number, $online_payment_by, $amount_deposit_by_name, $associate_id, $amount_deposit_by_id, $app_login_user_id, $is_app)
    {
        ...
        $data['branch_code'] = getBranchCode($branch_id)->branch_code;
        ...
        try {
            ...
            if ($member) {
                ...
                if ($token == $request->token) {
                    if ($request->page > 0 && $request->length > 0) {
                        $data = array();
                        $loan_data =  LoanDayBooks::where('loan_type', '!=', 3)->where('app_login_user_id', $member->id)->where('is_deleted', 0)->where('is_app', 1);
                        $loan_data1 = $loan_data->orderby('created_at', 'DESC')->get();
                        $count = count($loan_data1);
                        if ($request->page == 1) {
                            $start = 0;
                ...
                if ($token == $request->token) {
                    if ($request->page > 0 && $request->length > 0) {
                        ...
                        foreach ($loan_data as $key => $row) {
                            ...
                            $data[$key]['associate_code'] = $associate_id;
                            $data[$key]['associate_name'] = $associate_name;
                            if (loanGroupOutsandingAmount($row->id)) {
                                $total_payment =  loanGroupOutsandingAmount($row->id);
                            } else {
                                $total_payment = 'N/A';
                            }
                ...
                if ($token == $request->token) {
                    if ($request->id > 0) {
                        ...
                        if ($loan_data) {
                            ...
                            $data['loan_details']['associate_code'] = getAssociateNo($loanDetails->associate_member_id);
                            ...
                            foreach ($loanDetails['LoanApplicants'] as $key => $LoanApplicant) {
                                ...
                                $data['applicant'][$key]['details']['temporary_address'] = $temporary_address;
                                ...
                                if (isset($LoanApplicant->designation)) {
                                    $data['applicant'][$key]['employment_details']['designation'] = $LoanApplicant->designation;
                                } else {
                                    $data['applicant'][$key]['employment_details']['designation'] = '';
                                }
                            ...
                            foreach ($loanDetails['LoanCoApplicants'] as $key => $LoanCoApplicant) {
                                ...
                                $data['co_applicant'][$key]['details']['temporary_address'] = $temporary_address;
                                ...
                                $data['co_applicant'][$key]['documents']['under_taking_doc'] = array();
                                if ($coapplicantUnderDocFiles) {
                                    foreach ($coapplicantUnderDocFiles as $k2 => $coapplicantUnderDocFile) {
                                        // dd($coapplicantUnderDocFile);
                                        $data['co_applicant'][$key]['documents']['under_taking_doc'][$k2]['path'] = URL('core/storage/images/loan/document/' . $loanDetails->id . '/coapplicant/undertakingdoc/' . $coapplicantUnderDocFile->file_name . '');
                                        $data['co_applicant'][$key]['documents']['under_taking_doc'][$k2]['name'] = $coapplicantUnderDocFile->file_name;
                                    }
                                }
                            ...
                            foreach ($loanDetails['LoanGuarantor'] as $key => $LoanGuarantor) {
                                ...
                                $data['guarantor_applicant'][$key]['details']['temporary_address'] = $temporary_address;
                                ...
                                if (isset($LoanGuarantor->designation)) {
                                    $data['guarantor_applicant'][$key]['employment_details']['designation'] = $LoanGuarantor->designation;
                                } else {
                                    $data['guarantor_applicant'][$key]['employment_details']['designation'] = '';
                                }
                ...
                if ($token == $request->token) {
                    if ($request->id > 0) {
                        ...
                        if ($loan_data) {
                            ...
                            $data['loan_details']['loan_purpose'] = $loanDetails->loan_purpose;
                            $data['loan_details']['associate_code'] = getAssociateNo($loanDetails->associate_member_id);
                            if (isset($loanDetails->bank_account)) {
                                $data['loan_details']['bank_acount'] = $loanDetails->bank_account;
                            } else {
                                $data['loan_details']['bank_acount'] = '';
                            }
                            if (isset($loanDetails->ifsc_code)) {
                                $data['loan_details']['ifsc'] = $loanDetails->ifsc_code;
                            } else {
                                $data['loan_details']['ifsc'] = '';
                            }
                            if (isset($loanDetails->bank_name)) {
                                $data['loan_details']['bank_name'] = $loanDetails->bank_name;
                            } else {
                                $data['loan_details']['bank_name'] = '';
                            }
                            ...
                            foreach ($loanDetails['LoanApplicants'] as $key => $LoanApplicant) {
                                ...
                                $data['applicant'][$key]['details']['father_name'] = $aDetails->father_husband;
                                $data['applicant'][$key]['details']['mobile_no'] = $aDetails->mobile_no;
                                if (isset($aDetails->email)) {
                                    $data['applicant'][$key]['details']['email_id'] = $aDetails->email;
                                } else {
                                    $data['applicant'][$key]['details']['email_id'] = '';
                                }
                                $permanent_address = '';
                                if ($LoanApplicant->address_permanent == 0)
                                    $permanent_address = 'Self';
                    ...
                    if ($request->id > 0) {
                        ...
                        if ($loan_data) {
                            ...
                            foreach ($loanDetails['loanInvestmentPlans'] as $key => $loanInvestmentPlan) {
                                ...
                                $data['applicant_deposite_detail'][$key]['deposit'] = $investmentDetails->current_balance;
                    ...
                    if ($request->id > 0) {
                        ...
                        if ($loan_data) {
                            ...
                            foreach ($loanDetails['LoanApplicants'] as $key => $LoanApplicant) {
                                ...
                                $data['applicant'][$key]['details']['email_id'] = $email;
                                ...
                                $des = '';
                                if (isset($LoanApplicant->designation)) {
                                    $des = $LoanApplicant->designation;
                                }
                                $data['applicant'][$key]['employment_details']['designation'] = $des;
                                $data['applicant'][$key]['employment_details']['monthly_income'] = $LoanApplicant->monthly_income;
                                $data['applicant'][$key]['employment_details']['year_from'] = $LoanApplicant->year_from;
                                $data['applicant'][$key]['bank_detail']['bank_name'] = $LoanApplicant->bank_name;
                                $data['applicant'][$key]['bank_detail']['bank_ac'] = $LoanApplicant->bank_account_number;
                            ...
                            foreach ($loanDetails['LoanCoApplicants'] as $key => $LoanCoApplicant) {
                                ...
                                $data['co_applicant'][$key]['details']['mobile_no'] = $caDetails->mobile_no;
                                $email = '';
                                if (isset($caDetails->email)) {
                                    $email = $caDetails->email;
                                }
                                $data['co_applicant'][$key]['details']['email_id'] = $email;
                                $permanent_address = '';
                                if ($LoanCoApplicant->address_permanent == 0)
                                    $permanent_address = 'Self';
                                elseif ($LoanCoApplicant->address_permanent == 1)
                                ...
                                $data['co_applicant'][$key]['employment_details']['organization'] = $LoanCoApplicant->organization;
                                $designation = '';
                                if (isset($LoanCoApplicant->designation)) {
                                    $designation = $LoanCoApplicant->designation;
                                }
                                $data['co_applicant'][$key]['employment_details']['designation'] = $designation;
                                $data['co_applicant'][$key]['employment_details']['monthly_income'] = $LoanCoApplicant->monthly_income;
                                $data['co_applicant'][$key]['employment_details']['year_from'] = $LoanCoApplicant->year_from;
                                $data['co_applicant'][$key]['bank_detail']['bank_name'] = $LoanCoApplicant->bank_name;
                                $data['co_applicant'][$key]['bank_detail']['bank_ac'] = $LoanCoApplicant->bank_account_number;
                                ...
                                if ($coapplicantUnderDocFiles) {
                                    foreach ($coapplicantUnderDocFiles as $k1 => $coapplicantUnderDocFile) {
                                        $data['co_applicant'][$key]['documents']['under_taking_doc'][$k1]['path'] = URL('core/storage/images/loan/document/' . $loanDetails->id . '/coapplicant/undertakingdoc/' . $coapplicantUnderDocFile->file_name . '');
                                        $data['co_applicant'][$key]['documents']['under_taking_doc'][$k1]['name'] = $coapplicantUnderDocFile->file_name;
                                    }
                                }
                                if ($LoanCoApplicant->income_type == 0)
                                    $income = 'Salary Slip ';
                            ...
                            foreach ($loanDetails['LoanGuarantor'] as $key => $LoanGuarantor) {
                                ...
                                $designation = '';
                                if (isset($LoanGuarantor->designation)) {
                                    $designation = $LoanGuarantor->designation;
                                }
                                $data['guarantor_applicant'][$key]['details']['applicant_id'] = $a;
                                $data['guarantor_applicant'][$key]['details']['name'] = $LoanGuarantor->name;
                                $data['guarantor_applicant'][$key]['details']['father_name'] = $LoanGuarantor->father_name;
                                ...
                                $data['guarantor_applicant'][$key]['employment_details']['occupation'] = getOccupationName($LoanGuarantor->occupation);
                                $data['guarantor_applicant'][$key]['employment_details']['organization'] = $LoanGuarantor->organization;
                                $data['guarantor_applicant'][$key]['employment_details']['designation'] = $designation;
                                $data['guarantor_applicant'][$key]['employment_details']['monthly_income'] = $LoanGuarantor->monthly_income;
                                $data['guarantor_applicant'][$key]['employment_details']['year_from'] = $LoanGuarantor->year_from;
                                $data['guarantor_applicant'][$key]['bank_detail']['bank_name'] = $LoanGuarantor->bank_name;
                                $data['guarantor_applicant'][$key]['bank_detail']['bank_ac'] = $LoanGuarantor->bank_account_number;
