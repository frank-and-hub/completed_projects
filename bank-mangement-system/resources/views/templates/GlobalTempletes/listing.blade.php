@if (Auth::user()->role_id == 3 && !isset($col))
    <div class="col-md-6">
        <div class="form-group row">
            <label class="col-form-label col-lg-3">{{ $filedTitle ?? '' }} <sup class="required">*</sup></label>
            <div class="col-lg-9 error-msg">
                <select
                    class="form-control @if (isset($multipal) == true) multiselect @endif @if (isset($classes)) {{ $classes }} @endif" name="{{ $name }}" id="{{ $name }}" @if (isset($multipal) == true) multiple @endif
