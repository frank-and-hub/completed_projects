<script type = "text/javascript" >
    ...
    $(document).ready(function() {
        ...
        // Change form action according to loan and group loan
        ...
        $('#account_number').on('change', function() {
            ...
            if ($(this).val() == "") {
                ...
                if (type == '') {
                    swal("Warning!", "Select Loan Type First", "warning");
                    $('#account_number').val('');
                    return false;
                }
                $.ajax({
                    type: 'POST',
                    url: '{{ route("admin.common.LoanAccountDetails") }}',
                    data: {
                    ...
                    success: function(res) {
                        ...
                        switch (res) {
                            ...
                            default:
                                ...
                                $(".recovered-amount-list").attr("href", "admin/loan/deposit/emi-transactions/" + res?.loanRecord?.id + "/" + res?.loanRecord?.loan_type);
                                ...
                                $('#loan_associate_code').val(res?.loan_associate_code).removeAttr('readonly');
								$('#loan_associate_name').val(res?.loan_associate_name);
                                ...
                                $('#application_date').val(endDate);
                                $('#loan_associate_code').change();
                                var companyId = res?.loanRecord?.company_id;
                                var branchId = res?.loanRecord?.loan_branch?.id;
                                // Get Branch on change account number
                                $.ajax({
                                    type: 'POST',
                                    ...
                                    success: function(res) {
                                        try {
                                            var branchData = JSON.parse(res);
                                            if (Array.isArray(branchData.branch)) {
                                                var currentBranch = branchData.branch.filter(function(branchDetail) {
                                                ...
                                                console.error('branchData.branch is not an array');
                                            }
                                        } catch (error) {
        ...
        $('#verify').on('click', function(e) {
            ...
            if ($("#otp_form").valid()) {
                $.ajax({
                    type: "POST",
                    url: "{!! route('admin.verify.ssb_otp') !!}",
                    dataType: 'JSON',
                    data: {
        ...
        $(document).on('click', '.send_otp , .cursor', function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('common.withdraw.accountdetails') !!}",
                dataType: 'JSON',
                data: {
...
function emptyOtp(){
    ...
    $.ajax({
                type: "POST",
                url: "{!! route('admin.update.ssb.otp') !!}",
                dataType: 'JSON',
                data: {
