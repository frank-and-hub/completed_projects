...
$(document).ready(function() {
  ...
  $('#prematurity_account_number, #prematurity_account_number_confirm, #maturity_account_number, #maturity_account_number_confirm').on('cut copy paste', function(e) {
            e.preventDefault();
        });
    ...
    $('#add_demand_advice,#edit_demand_advice').validate({ // initialize the plugin
        rules: {
            ...
            'maturity_bank_account_number_confirm' : {required: function(element) {
                ...
                },number:true,minlength: 8,maxlength: 20,equalTo: "#maturity_bank_account_number"},
  ...
  return this.optional(element) || (element.files[0].size <= param)
    ...
    $(document).on('change','#company_id,#branch',function(){
        $('.payment-type-sub-box').hide();
        // $('#maturity_prematurity_type').val('');
        // $('#maturity_prematurity_date').val('');
    ...
    $('#fresh_expense_record,#select_all_fresh_expense').on('change',function(){
            const userbranchId = $(this).attr('data-branch');
            const selectedBranchId = $('#branch_id').val();
            const paymentMode = $('#amount_mode').val();
            if((userbranchId != selectedBranchId) && paymentMode == 0 &&  paymentMode != '' )
            {
                swal("Warning","Please Select Selected Branch Record",'warning');
                $('#fresh_expense_record').prop('checked',false);
                $('#select_all_fresh_expense').prop('checked',false);
                return false;
            }
       })
...
//           })
//    })
// Changed on 9 jan 2024 from mahesh
var highestDate ='01/04/2021';
    @isset($highestDate)

    var highestDate = @json($highestDate);
    var highestDate = moment(highestDate).format('DD/MM/YYYY');
            // Use phpData in your jQuery script

        console.log(highestDate);
        @endisset
    $(".date-from,.date-to,#date,#death_help_date,#payment_date").hover(function(){
      var today=$('.create_application_date').val();
      $('.date-from,.date-to,#date,#death_help_date,#payment_date').datepicker({
        format: "dd/mm/yyyy",
        orientation: "bottom",
        autoclose: true,
        endDate: today,
                ...
                if(response.status == 200 ){
                    ...
                    if(response.investmentDetails.member){
                        ...
                        if((response.investmentDetails.member.photo)){
                            // $('.'+classVal+'_photo').html(' <img src="{{url('/')}}/asset/profile/member_avatar/'+response.investmentDetails.member.photo+'" alt="photo" width="180" height="100">');
    ...
    $(document).on('change','#amount_mode,#mode',function(){
        ...
        if(modeValue == 0 && modeValue != ''){
            ...
            if(type == 3)
            {
                const ssbAccount = $('#nominee_ac_number').val();
                if(ssbAccount == '')
                {
                    $(this).val('');
                    swal("Warning!", "Member Nominee Account Number Does Not Exist!", "warning");
                    return false;
                }
            }
            $('.bank-mode').hide();
    ...
    $('#select_all_fresh_expense').trigger('click');
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    data.forEach(element => {
                        html +=
                            `<option value="${element.account_no}" data-account="${element.id}"" >${element.account_no}</option>`;
                    });
    ...
    $('#bank_account_number').change(function () {
            ...
            let from_bankk = $('#bank').val();
            let from_Bank_account_no = $('#bank_account_number').val();
            if (date == "") {
    ...
    $(document).on('change','#branch_id,#cash_type', function () {
        ...
        const userbranchId = $('#fresh_expense_record').attr('data-branch');

        const companyId = $('#company_id option:selected').val() ??$('#company_id').val() ;
        var date = $('#payment_date').val();
    ...
    $('.export').on('click', function(e) {
        ...
        $('.export_new').on('click', function(e) {
            e.preventDefault();
            var extension = $(this).attr('data-extension');
            $('#demand_advice_report_export').val(extension);
            if (extension == 0) {
                ...
                $(".spiners").css("display", "block");
                $(".loaders").text("0%");
                doChunkedExportTransactionsNew(0, chunkAndLimit, formData, chunkAndLimit,'');
                $("#cover").fadeIn(100);
            } else {
                $('#demand_advice_report_export').val(extension);
            	$('form#filter_report').attr('action',"{!! route('admin.demandadvice.export_new') !!}");
			$('form#filter_report').submit();
            }
        });
    ...
    function doChunkedExportTransactions(start, limit, formData, chunkSize,fileName) {
            formData['start'] = start;
            formData['limit'] = limit;
            ...
            jQuery.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.result == 'next') {
                        ...
                        doChunkedExportTransactions(start, limit, formData, chunkSize,fileName);
                        $(".loaders").text(response.percentage + "%");
                    } else {
                        var csv = response.fileName;
                        console.log('DOWNLOAD');
        ...
        function doChunkedExportTransactionsNew(start, limit, formData, chunkSize,fileName) {
            formData['start'] = start;
            formData['limit'] = limit;
            formData['fileName'] = fileName;
            jQuery.ajax({
                type: "post",
                dataType: "json",
                url: "{!! route('admin.demandadvice.export_new') !!}",
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(response) {
                    ...
                    if (response.result == 'next') {
                        start = start + chunkSize;
                        fileName = response.tokenName;
                        doChunkedExportTransactionsNew(start, limit, formData, chunkSize,fileName);
                        $(".loaders").text(response.percentage + "%");
                    } else {
                        var csv = response.fileName;
                        console.log('DOWNLOAD');
...
function resetApplicationForm()
{
    ...
    $(".odd").remove();
    $(".even").remove();
    $("#demand-advice-application-table_info").remove();
    $("#demand-advice-application-table_paginate").remove();
    $('#is_search').val("yes");
    demandAdviceApplicationTable.draw();
}
