...
<div class="row">
    <div class="col-lg-12 ">
        <h3 class="mb-0 text-dark bg-md-dark px-3 py-2 text-dark my-3">Guarantor Details</h3>
        <div class="row">
            <label class=" col-lg-4">Guarantor Id</label>
            <div class="col-lg-1">:</div>
            <div class="col-lg-7  ">
                {{ $LoanGuarantor->member_id??'N/A'}}
            </div>
        </div>
        <div class="row">
...
<div class="row">
    <div class="col-lg-12 ">
        ...
        <div class="row">
            @php $applicantFiles = getFileData($LoanGuarantor->id_proof_file_id); @endphp
            <label class=" col-lg-4">Security cheque/Stamp</label>
            <div class="col-lg-1">:</div>
            @if($applicantFiles)
            @foreach($applicantFiles as $applicantFile)
            @php
            $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/id_proof/'.$applicantFile->file_name.'';
            $url = ImageUpload::generatePreSignedUrl($foldeName);
            @endphp
            <div class="col-lg-7  "> 
                <a href="{{ $url }}" target="blank">{{ $applicantFile->file_name }}</a> 
            </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-1">:</div>
            @php $applicantAddressFiles = getFileData($LoanGuarantor->address_proof_file_id); @endphp
            @if($applicantAddressFiles)
            @foreach($applicantAddressFiles as $applicantAddressFile)
            @php
            $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/address_proof/'.$applicantAddressFile->file_name.'';
            $url = ImageUpload::generatePreSignedUrl($foldeName);
            @endphp
            <div class="col-lg-7  "> 
                <a href="{{ $url }}" target="blank">{{ $applicantAddressFile->file_name }}</a>
            </div>
        ...
        <div class="row">
            ...
            <div class="col-lg-1">:</div>
            @php $gUnderDocFiles = getFileData($LoanGuarantor->under_taking_doc); @endphp
            @if($gUnderDocFiles)
            @foreach($gUnderDocFiles as $gUnderDocFile)
            @php
            $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/undertakingdoc/'.$gUnderDocFile->file_name.'';
            $url = ImageUpload::generatePreSignedUrl($foldeName);
            @endphp
            <div class="col-lg-7  "> 
                <a href="{{ $url }}" target="blank">{{ $gUnderDocFile->file_name }}</a> 
            </div>
        ...
        <div class="row">
            ...
            @php
            $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/income_proof/'.$applicantIncomeFile->file_name.'';
            $url = ImageUpload::generatePreSignedUrl($foldeName);
            @endphp
            <div class="col-lg-7  "> 
                <a href="{{ $url }}" target="blank">{{ $applicantIncomeFile->file_name }}</a>
            </div>
    ...
    <div class="col-lg-12 ">
        ...
        <div class="row">
            <label class=" col-lg-4">Upload File</label>
            <div class="col-lg-1">:</div>
            @php $files = getFileData($loanotherdocs['file_id']); @endphp
            @if($files)
            @foreach($files as $file)
            @php
            $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/moredocument/'.$file->file_name.'';
            $url = ImageUpload::generatePreSignedUrl($foldeName);
            @endphp
            <div class="col-lg-7  "> 
                <a href="{{ $url }}" target="blank">{{ $file->file_name }}</a> 
            </div>
