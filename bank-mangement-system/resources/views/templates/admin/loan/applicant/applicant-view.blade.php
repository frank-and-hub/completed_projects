...
<div class="row">
    <div class="col-lg-12 ">
        <h3 class="mb-0 text-dark bg-md-dark px-3 py-2 text-dark my-3">Applicant Details</h3>

        @php
            $aDetails = getMemberCompanyDataNew($loanDetails->customer_id);
            $url = URL::to('admin/member-detail/' . $aDetails->customer_id . '?type=0');
        @endphp
        ...
        <div class="row">
            ...
            @if ($loanDetails['loan']->loan_type != 'G')
                    <div class="col-lg-7  "> <a href="{{ $url }}" target="_blank">
                            {{ $aDetails->member->member_id }}</a></div>
                @else
                    <div class="col-lg-7  "> {{ $aDetails->member->member_id }}</div>
                @endif
        </div>
