<div class="card pt-3 pb-3">
    <div class="card-header">
        ...
        @if($branchName)<h6 class="card-title font-weight-semibold pt-2"><strong>Branch :</strong> {{$branchName}}</h6>@endif
    </div>
...
<div class="card">
    ...
    <div class="card-body">
        <form id="myform" name="myform" class="" method="POST">
            ...
            <table id="head_listing" class="table">
                ...
                <tbody>
                    ...
                    @foreach ($data as $head)
                        ...
                        $colorss = '';
                        if ($type == 1) {
                            $closingDetail1 = getheadClosingValue($start_y, $end_y, $head->head_id, $company_id);
                            $amount = 0.0;
                            if ($closingDetail1) {
                        ...
                        ?>
                        <tr>
                            <td style="color:#089a08" >{{ strtoupper($head->sub_head) }}</td>
                            @if ($type == 1)
                        ...
                        @if (count($data) > 0)
                            @foreach ($data as $taxonomy)
                                ...
                                $colora = '';
                                if ($type == 1) {
                                    $closingDetail = getheadClosingValue($start_y, $end_y, $taxonomy->head_id, $company_id);
                                    $amount1 = 0.0;
                                    if ($closingDetail) {
                                ...
                                ?>
                                <tr>
                                    <td style="padding-left: {{ $p }}px"> {{ strtoupper($taxonomy->sub_head) }}</td>
                                    @if ($type == 1)
                                        <td style="color:{{ $colora }} "> {{ number_format((float) $amount1, 2, '.', '') }} </td>
                                    @else
                    ...
                    @if ($type != 1)
                        <tr>
                            <td colspan="2" style="text-align: right;"><input type="submit" name="save" value="Save" class="btn btn-primary" id="myformsubmit">
                                {{-- <button type="button" class="btn btn-gray" id="my_form_reset" onClick="myFormreset()" >Reset </button></td> --}}
                        </tr>
                    @endif
                </tbody>
            </table>
        ...
        @if ($type == 1)
            {{-- <button style="float:right;" type="button" class="btn btn-gray" id="reset_finance_head" onClick="resetFinanceHead()" >Reset </button> --}}
        @endif
    </div>
</div>