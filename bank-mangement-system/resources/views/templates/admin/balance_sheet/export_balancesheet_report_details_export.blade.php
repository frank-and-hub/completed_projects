...
$subHeadsIDS = App\Models\AccountHeads::where('head_id',$headDetail->head_id)->where('status',0)->pluck('head_id')->toArray();
 ...
 ?>
                    ...
                    @endforeach
                    @else
                    @foreach($subchildHead as $subHead)
                        <tbody>
                            <tr  @if($subHead->status==1) class="child_inactive" @endif >
                          <td>{{$subHead->sub_head}}</td>
                           <td> &#X20B9;{{ number_format((float)headTotalNew($subHead->head_id,$date_filter,$end_date_filter,$branch_filter), 2, '.', '')}}</td> 
