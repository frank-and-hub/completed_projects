...
$(document).ready(function() {
    $("#update_15_g").on('submit', function(e) {
        ...
        if($('#update_15_g').valid()){
            ...
            var year = $('#year').val();
            var file = $('#file').val();
            $.post("{{route('admin.form15g.datacheck')}}", {
                'year': year,
                ...
                if (e.data > 0) {
                    swal({
                        title: 'Warning!',
                        text: '15G Form Already Uploaded in selected Finencial Year, if you want to Upload New Click on "Yes" Button',
                        icon: 'info',
                        ...
                        if (isConfirmed) {
                            ...
                            if (year && file) {
                                $.ajax({
                                    type: 'POST',
                                    url: "{!! route('admin.update_15g.save') !!}",
                                    data: formData,
                                    dataType: 'json',
                                    contentType: false,
                                    cache: false,
    ...
    $.ajax({
        type: 'POST',
        url: "{!! route('admin.form_g.getData') !!}",
        data: {
            'memberId': m_id,
            'companyId': companyId
        },
	...
	$(document).ajaxComplete(function(){
        ...
        var html = "";
        console.log(month,"month",year,"year",nextyear,"nextyear",extrayear,"extrayear");
        if (month > 3) {
    ...
    $('.export').on('click', function() {
        ...
        $('form#filter').attr('action', "{!! route('admin.update_15g.report.export') !!}");
        $('form#filter').submit();
    });
});
    ...
    swal({
        ...
        function(isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: "POST",
                    url: "{!! route('admin.update_15g.record.delete') !!}",
                    dataType: 'JSON',
                    data: {'id': value,},
                    success: function(e) {
                        if (e) {
