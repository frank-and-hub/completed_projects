...
$(document).ready(function () {
 ...
 $( document ).ajaxStart(function() {
  ...
  $('.export').on('click',function(){
        ...
        $('form#filter').attr('action',"{!! route('admin.daybook.report.export') !!}");
        $('form#filter').submit();
    });
...
{  
    if($('#filter').valid())
    {
        ...
        $.ajax({
              type: "POST",  
              url: "{!! route('admin.report.day_booklisting') !!}",
              dataType: 'JSON',
...
function load_more(page){
  ...
  $.ajax(
        {
            type: "POST",  
            url: "{!! route('admin.daybook.transaction_listing') !!}",
            dataType: 'JSON',
            data: {'is_search':is_search,'start_date':start_date,'branch':branch,'end_date':end_date,'page':page,'limit':limit,'index':index},
            headers: {
                  'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            beforeSend: function()
