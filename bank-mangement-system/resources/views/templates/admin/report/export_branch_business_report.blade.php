...
?>
        ...
        <tbody>
          ...
          <tr>
                ...
                <tr>    
					...
					<td>{{ $totalCaseInHandAmount = (number_format((float)$total_case_current_daily_new_deno_ac, 2, '.', '')) + (number_format((float)$total_case_current_monthly_deno_sum, 2, '.', '')) + (number_format((float)$total_case_current_fd_deno_sum, 2, '.', '')) + (number_format((float)$total_loan_recovery_amt_cashmode, 2, '.', '')) + (number_format((float)$file_chrg_amount_total_cash_mode, 2, '.', '')) + (number_format((float)$Other_ncc_sum, 2, '.', '')) }}</td>
					<td></td>
                    <td>TOTAL</td>
                    <td>{{$fw_total}}</td>
      ...
      <table border = "1" class="table-flush" width = "100%" style="border-collapse: collapse;font-size:12px;margin-top:1rem;">
        ...
        <tr>
           <td colspan="3" style="font-weight: bold;">SAMRADDH</td>
            <td colspan="3" style="font-weight: bold;">LOAN</td>
            <td ></td>
        </tr>
        <tr>    
      ...
      <?php if( (count($micros) > 0) || (count($loans) > 0) ) { 
		...
		?>
			...
			<tr>
				...
				<?php if($i == 0) {?>
					<td><?php echo "&#x20B9;".number_format((float)$totalAmounts, 2, '.', ''); ?></td>
				<?php } else { ?>
					<td></td>
				<?php }	?>
			</tr>
    ...
    <table border = "1" class="table-flush" width = "100%" style="border-collapse: collapse;font-size:12px;margin-top:1rem;">
        <thead>
            <tr>
                ...
                <td>&#x20B9;{{ number_format((float)$total_final_payments, 2, '.', '') }}</td>
            </tr>
            <tr>
                <th>ACTUAL CASH IN HAND</th>
                <td>&#x20B9;{{ number_format((float)$totalNEWBusinessAmount, 2, '.', '') +  number_format((float)$totalNewRenewalAmount, 2, '.', '') + number_format((float)$totalCaseInHandAmount, 2, '.', '') - number_format((float)$total_final_payments, 2, '.', '')  }}</td>
            </tr>
        </thead>   
