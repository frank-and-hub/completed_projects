<script type="text/javascript">
    $(document).ready(function () {
        ...
        maturityReport = $('#upcoming_listing').DataTable({
            processing: true,
            serverSide: true,
            pageLength: 20,
            ...
            ajax: {
                "url": "{!! route('admin.report.maturityUpcominglist') !!}",
                "type": "POST",
            ...
            columns: [
                ...
                { data: 'remaining_period', name: 'remaining_period' },
                { data: 'expected_deposit', name: 'expected_deposit' },
        ...
        $('.export-upcoming').on('click', function (e) {
            e.preventDefault();
        ...
        function doChunkedExport(start, limit, formData, chunkSize, page) {
            ...
            jQuery.ajax({
                type: "post",
                dataType: "json",
                url: "{!! route('admin.maturityUpcomingExport.report.export') !!}",
                data: formData,
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        $.each(response.plans, function (index, value) {
                            $("#plan").append("<option value='" + value.id + "' data-plancategory='" + value.plan_category_code + "' data-plansubcategory='" + value.plan_sub_category_code + "'>" + value.name + "</option>");
                        });
                    }
                });
            }
    ...
    function searchForm() {
        if ($('#filter').valid()) {
            $('#is_search').val("yes");
            const plancategory = $("#plan").find(':selected').data('plancategory');
            const plansubcategory = $("#plan").find(':selected').data('plansubcategory');
            $(".plancategory").val(plancategory);
            $(".plansubcategory").val(plansubcategory);
