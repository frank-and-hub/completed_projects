...
//         var ledgerDate = '01/{{$re_month1}}/{{$re_year1}}';
...
//    })
  ...
  $('.employee_salary_payable_export').on('click',function(e){	
        e.preventDefault();		
    ...
    function doChunkedExport(start,limit,formData,chunkSize){
		...
		$.ajax({
			type : "post",
			dataType : "json",
			url :  "{!!route('admin.hr.salary_export')!!}",
			data : formData,
...
$(document).on('keyup','.deduction',function(){ 
    ...
    if(parseFloat(comp) < parseFloat(data)){
      swal("Warning!", "You cannont do deduction more then total salary", "warning");
      $(this).val(0);
    }
    calculatioAmountfinalPay(res);
...
$(document).on('keyup','.incentive_bonus',function(){ 
  ...
  $(document).on('keyup','.tds_amount',function(){ 
    ...
    if(parseFloat(comp) < parseFloat(tocompare)){
      swal("Warning!", "Tds cannot be equal h or more then salary", "warning");
      $(this).val(0);
    }
    calculatioAmountfinalPay(res);
  });
