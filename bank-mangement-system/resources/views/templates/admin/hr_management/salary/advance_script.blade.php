...
$(document).ready(function() {
    ...
    $('#payment_branch').on('change', function() {
        ...
        if (branch_id > 0 ) {
            if (entrydate == '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "{!! route('admin.hr.salary.getbranchbankbalanceamount') !!}",
                    dataType: 'JSON',
                    data: {
                        'branch_id': branch_id,
                        'entrydate': entrydate,
    ...
    $(document).on('keyup', '#advance_settel', function() {
        ...
        total_transfer_amount = parseFloat($('#due_amount').val()).toFixed(2);
        ...
        if (advance_payment > 0) {
            if (advance_settel > 0) {
                ...
                a = total_transfer_amount - advance_settel - neft_charge
        ...
        $('#transfer_amount').val(total_salary);
    ...
    $.validator.addMethod("chkadvancr", function(value, element, p) {
        ...
        if (parseFloat(advance_payment) < parseFloat(settle_amount)) {
            $.validator.messages.chkadvancr =
                "Advance settle amount must be lower than or equal to advance amount";
            result = false;
        } else {
            $.validator.messages.chkadvancr = "";
            result = true;
...
$.validator.addMethod("chkpayable", function(value, element,p) {     
  paybale_amount=$('#due_amount').val();
    advance_payment  = $('#advance_payment').val(); 
    if(parseFloat(advance_payment) > parseFloat(paybale_amount))
    {
      $.validator.messages.chkpayable = "Advance settle amount must be lower than or equal to due amount";
        result = false; 
    }
    else
    {
