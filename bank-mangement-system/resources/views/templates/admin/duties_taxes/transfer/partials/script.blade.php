<script type="text/javascript">
    ...
    $(document).ready(function() {        
        ...
        tdstransferListing = $('#tds_transfer_list').DataTable({
            ...
            ajax: {
                ...
                "url": "{!! route('admin.duties_taxes.transfer_listing') !!}",
                "type": "POST",
                "data": function(d) {
        ...
        $('.export_tds_transafer').on('click',function(e){
            ...
            if (extension == 0) {
                ...
                $('#tds_transfer_export').val(extension);
                $('form#tds_filter').attr('action', "{!! route('admin.duties_taxes.export_transafer_list') !!}");
        ...
        $(document).on('keyup', '#payable_late_penalty,#payable_tds_amount', function() {
            ...
            if(tds_amount > payable_paid_amount){
                swal('Warning!',"Amount Equal or less then " + payable_paid_amount.toFixed(2),'warning');
    ...
    function loadAmount() {
        ...
        $.ajax({
            type: "POST",
            url: "{!! route('admin.duties_taxes.payable') !!}", // admin.tdspayableamount
            dataType: 'JSON',
            data: {
                'startDate': startDate,
                'endDate': endDate,
    ...
    function doChunkedExport2(start, limit, formData, chunkSize) {
        ...
        jQuery.ajax({
            type: "post",
            dataType: "json",
            url: "{!! route('admin.duties_taxes.export_transafer_list') !!}",
            data: formData,
            success: function(e) {
                if (e.result == 'next') {
                    start = start + chunkSize;
                ...
                success: function(res) {
                    ...
                    for (var key in res.tdsHeads) {
                        if (res.tdsHeads.hasOwnProperty(key)) {
                            ...
                            $('#payable_head_id').append('<option value="' + key + '">' + optionText +
                                '</option>');
                        }
                    }
                },
