<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('click', '#fn_edit_as_minor', function() {
            $("#fn_edit_as_minor").prop("checked") == true ?
        ...
        $(document).on('click', '#sn_edit_as_minor', function() {
            ...
            $("#sn_edit_as_minor").prop("checked") == true ?
                $('#nominee_edit_parent_second_detail').show() :
                ($('#nominee_edit_parent_second_detail').hide(),
                    $('#sn_parent_nominee_name').val(''),
                    $('#sn_parent_nominee_mobile_no').val(''));
        ...
        $(document).on('submit', '#register-plan', function() {
            $('.loader').show();
        });
        ...
        $(document).on('change', '#branch,#memberid', function() {
        ...
        $('#memberid').on('change', function() {
            ...
            const url = "{!! route('admin.investment.member') !!}";
            $.post(url, {
                "memberid": memberid,
                "companyId": companyId,
                "branchId": branchId,
                ...
                $('#investmentplan').find('option').remove();
                if (response.plans.length > 0 && response.plans) {
                    var html = '<option value="">Please select</option>';

                    $.each(response?.plans, function(index, value) {
                        html += '<option data-val="' + value?.slug + '"  value="' + value?.id + '" data-category="' + value?.plan_category_code + '" data-min = "' + value?.min_deposit + '" data-max = "' + value?.max_deposit + '" data-multiple = "' + value?.multiple_deposit + '" data-ssb-required="' + value?.is_ssb_required + '" data-sub-category="' + value?.plan_sub_category_code + '" >' + value?.name + '</option>';
                    });
                    $('#investmentplan').append(html);

                }
                if (response.resCount > 0 && response.member) {
                    $('#newUser').val(response.newUser);

                    (response.newUser == false && (!response.checkMemberInOtherCompany)) && $('.mi-charge').show();
                    ...
                    response?.member?.special_category_id ? $('#specialcategory').val(response?.member?.special_category_id) : $('#specialcategory').val("General Category");
        ...
        $(document).on('click', '.ssb-account-availability', function() {
            ...
            if (aVal == 0) {
                ...
                var optionExists = $('#payment-mode').find('option[value="' + 3 + '"]').length > 0;
                if (optionExists) {
                    swal('Alert!', 'SSB Account Already Exist', 'error');
                    $('.ssb-account-availability[value="' + 0 + '"]').prop('checked', true);
                    return false;
                }
                $('.ssb-show').show();
                if (ssbValue == '') {
        ...
        $(document).on('change', '#cheque_id', function() {
            ...
            $.ajax({
                ...
                success: function(response) {
                    //alert(response.id);
                    if (parseFloat(amount).toFixed(2) == parseFloat(response.amount).toFixed(2)) {
                        ...
                        $("#cheque_id").val('');
                        swal("Warning!", "Cheque amould Should be same as Deposit Amount!", "warning");
                        return false;
                    }
        ...
        $(document).on('keyup change', '.dd-tenure,.dd-amount', function() {
            ...
            var special = $(".dd-tenure option:selected").data('spe-roi');
            var principal = $('.dd-amount').val();
            var time = tenure;
            var specialCategory = $('#specialcategory').val();
            // var  rate =  (specialCategory == 'Special Category N/A') ? special : rate;
            var rate = (specialCategory != 0) ? special : rate;
            var ci = 12;
            var freq = 12;
            var irate = rate / ci;
            var year = time / 12;
        ...
        $(document).on('keyup change', '.fd-tenure,.fd-amount', function() {
            ...
            //   var  rate =  (specialCategory == 'Special Category N/A') ? sprcialRate : roi;
            var rate = (specialCategory != 0) ? sprcialRate : roi;
        ...
        $(document).on('keyup change', '.rd-tenure,.rd-amount', function() {
            ...
            var specialCategory = $('#specialcategory').val();
            var time = tenure;
            var special = $(".rd-tenure option:selected").data('spe-roi');
