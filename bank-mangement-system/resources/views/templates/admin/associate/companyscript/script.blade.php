...
$(document).ready(function() {
    ...
    /*
    $.validator.addMethod("customerId", function(value, element, p) {
            if (this.optional(element) || /^[0-9]{4}CI[0-9]{6}$/.test(value)) {
                $.validator.messages.customerId = "";
                result = true;
            } else {
                $.validator.messages.customerId = "Please enter a valid Customer Id.";
    ...
    */
    ...
    $.validator.addMethod("check_ssb_account", function(value, element, p) {
        ...
        $.ajax({
            type: "POST",
            url: "{!! route('admin.associatessbaccountcheck') !!}",
            dataType: 'JSON',
            data: {
                'account_no': value,
                'member_id': member_id
        ...
        if($('#customerNameForm').valid()){
            if (form == 1) {
                if ($('#customAssociateRegister').valid()) {
                    ...
                    $.ajax({
                        type: "POST",
                        url: "{!! route('admin.associate.store.customer') !!}",
                        dataType: 'JSON',
                        data: formData,
                        contentType: false,
                        processData: false,
            ...
            if(form == 2){
                if ($('#customAssociateRegister').valid()) {
                    ...
                    $.ajax({
                    type: "POST",
                        url: "{!! route('admin.associate.dependents.customer') !!}",
                        dataType: 'JSON',
                        data: formData2,
                        cache: false,
                        contentType: false,
                        ...
                        success: function(e) {
                            ...
                            //     // swal("Success!",'Recipt Not Generated Yet !',"success");
                            ...
                            //             window.location.href = " {!! route('admin.associate') !!}";
                            //         }else{
                            //             window.location.href = " {!! route('admin.associate_listing') !!}";
                            //         }
                            //     });
                            // } 
                            if (e.msg_type == "success") {
    ...
    $(document).on('keyup', '#customer_id', function() {
        ...
        if($('#customerNameForm').valid()){
            if (code != '') {
                $.post("{!! route('admin.customerDataGet') !!}", {
                    'code': code
                }, function(e) {
                console.log(e);
    ...
    $(document).on('change', '#payment_mode', function() {
        ...
        if (val == 1) {
            ...
            $.post("{!! route('admin.approve_recived_cheque_lists') !!}", {
                'data': ''
            }, function(e) {
        ...
        if (val == 3) {
            ...
            $.post("{!! route('admin.associateSsbAccountGet.customer') !!}", {
    ...
    $(document).on('keyup', '#senior_code', function() {
        ...
        if (code != 0)
            $.ajax({
                type: "POST",
                url: "{!! route('admin.seniorDetail.customer') !!}",
                dataType: 'JSON',
                data: {
                    'code': code
                },
                    ...
                    if (e.resCount > 0) {
                        if (e.msg == 'block') {
                            ...
                            if (e.msg == 'InactiveAssociate') {
                                ...
                                $.post("{!! route('admin.getCarderAssociate.customer') !!}", {
                                        'id': e.carder_id
                                    },
                                    function(e) {
...
function rd_check(){
    var member_id=$('#id').val();
        $.post("{!! route('admin.associateRdAccount') !!}",{'member_id':member_id}, function(e) {
                if ( $.isEmptyObject(e) ) {
...
function check_ssb() {
  ...
  $.post("{!! route('admin.associateSsbAccountGet.customer') !!}",{ 'customerId': customerId },function (e) {
      if (e.resCount == 1) {
