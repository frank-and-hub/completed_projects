...
@foreach($data as $index => $row)  
  <tr>
    ...
    <td>@if(isset($row['investment']->account_number )){{ $row['investment']->account_number }}@else @endif</td>
    <td>{{ transactionPlanName($row->type_id) }}</td>
    <td>{{ $row->total_amount }} </td>
    <td>{{ $row->commission_amount }} </td>
    <td>{{ $row->percentage }} </td>
    <td>@if($row->type==5) Collection Charge @else {{ getCarderName($row->carder_id) }} @endif</td>
