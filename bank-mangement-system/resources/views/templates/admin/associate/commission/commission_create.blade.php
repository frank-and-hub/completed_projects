    ...
    <div class="row">  
        ...
        <div class="col-md-12" id='hide_div'>
                <div class="card">
                                ...
                                <table id="member_listing" class="table table-flush">
                                    ...
                                    ?>
                                         ...
                                         //$memData=memberDetailGetByID($val->member_id);
                                          $getPan =get_member_id_proofNew($val->member_id,5);
                                          ?> 
                                         <tr>
                                             <td>{{ $index+1 }}</td>
                                             <td>{{ $val->associate_no }}
                            ...
                            <?php
                                if($getPan)
                                {
                                    ...
                                    if($tds_with)
                                    {
                                        ...
                                        $p=$tds_with->tds_per;
                                       // echo associateTdsDeductGet($val->member_id,$start_date_time);die;
                                            if(associateTdsDeductGet($val->member_id,$start_date_time)>0)
                                             {
                                                $perAmount=round(($amount*$p)/100,2);
                                             }
                                             else
                                             {
                                                $totalCommisionGet=calculateAssocaiteTotalCommissionDate($val->member_id,$start_date_time);
                                                $totalCommisionGet1=$totalCommisionGet+$amount;
                                               // echo 'c=='.$totalCommisionGet1;
                                                if($totalCommisionGet1>=$amount1)
                                                {
                                                    $perAmount=round(($totalCommisionGet1*$p)/100,2);
                                                }
                                             }
                                            ...
                                            if(associateTdsDeductGet($val->member_id,$start_date_time)>0)
                                             ...
                                             else
                                             {
                                                // calculate  tds 
                                                 ...
                                                 $totalCommisionGetP=calculateAssocaiteTotalCommissionDate($val->member_id,$start_date_time);
                                                 $totalCommisionGetP1=$totalCommisionGetP+$amount;

                                                if($totalCommisionGetP1>=$amount1)
                                                {
                                                    $perAmount=round(($totalCommisionGetP1*$p)/100,2);
                                                }
                                             }
                                ...
                                {{ $perAmount }}
                                             ...
                                             <td>
                                                <?php 
                                                $finalAmount=round(($amount-$perAmount),2);
                                                if($finalAmount>0)
                                                {
                    ...
                    @endif 
                    <?php
                        if(getAssociateOneYear($start_date,$val->member_id)==1)
                        {
                           $fule = $fule;
                        }
                        else
