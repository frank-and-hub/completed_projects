<script type="text/javascript">
    $(document).ready(function () {
        function chequerenewlimit(cValue,mId,thisvalue){
            ...
            $.post("{!! route('admin.investment.renewlimit') !!}",
                {
                    'amount': cValue,
                    'account_number': mId,
                    'renewPlan': renewPlan,
        ...
        $(document).on('change', '.account-number', function () {
            ...
            $.ajax({
                ...
                dataType: 'JSON',
                data: { 'account_number': accountNumber, 'renewPlan': renewPlan, 'renewPlanId': renewPlanId, 'renewalDate': renewalDate , 'company_id': company_id , 'branch_id': branch_id ,'chequee' :chequee},
                headers: {
                ...
                success: function (response) {
                    ...
                    if (response.resCount > 0 && response.minutes > 5) {
                        ...
                        $.each(response.investment, function (key, value) {
                            // console.log(response.investment[0].deposite_amount,"due amount");
                            if (value.member) {
                            ...
                            // this column is added by sachin sir on 22-03-24 with discussed with mukesh sir
                            var d = new Date(value.maturity_date);
                            var day = ('0' + d.getDate()).slice(-2);
                            var month = ('0' + (d.getMonth() + 1)).slice(-2);
                            var year = d.getFullYear();
                            var formattedDateTime = day + '/' + month + '/' + year;
                            $('.maturity-date-' + attVal + '').val(formattedDateTime);
                    ...
                    if (renewPlan == 0) {
                        ...
                        `<tr class="tr_class" id="row_${x}">
                            ...
                            <td>
                                ...
                                <input class="maturity-date-${x} form-control" type="text" readonly/>
                            </td>
                            <td>
                                <div class="col-lg-12">
                        ...
                        `<tr class="tr_class" id="row_${x}">
                            <td>
                                <input type="text" data-val="${x}" name="account_number[${x}]" class="form-control account-number account-number-${x}" autocomplete="off" required>
                                <input type="hidden" name="investment_plan[${x}]" class="col-lg-0 investment-plan-${x}">
                                <input type="hidden" name="investment_id[${x}]" class="col-lg-0 investment-id-${x}">
                                <input type="hidden" name="investment_member_id[${x}]" class="col-lg-0 investment-member-id-${x}">
                                <input type="hidden" name="investment_tenure[${x}]" class="col-lg-0 investment-tenure-${x}">
                            ...
                            <td>
                                <div class="col-lg-12">
                                    <div class="rupee-img"></div>
                                    <input type="text" data-val="${x}" name="amount[${x}]" class="form-control rupee-txt rdfrd-renew-amount rdfrd-renew-amount-${x}" required>
                                </div>
                                <input type="hidden" data-val="${x}" name="deo_amount[${x}]" class="col-lg-0 form-control rupee-txt deo-amount-${x}" readonly="" tabIndex="-1">
                                <input type="hidden" name="hidden_due_amount[${x}]" class="col-lg-0 hidden-due-amount-${x}">
                                <input type="hidden" name="hidden_deposite_amount[${x}]" class="col-lg-0 hidden-deposite-amount-${x}">
                            ...
                            <td>
                                <input type="text" data-val="${x}" name="acoount_associate_name[${x}]" class="form-control associate-name-${x}" readonly="true" tabIndex="-1" />
                                <a href="javascript:void(0);" class="remove-rdfrd-renew-button" tabIndex="-1">Remove</a>
                            </td>
                        </tr>`; //New input field html
                        $('.' + appendClass + '').append(list_fieldHTML); //Add field html
                    }
        ...
        $(document).on('change', '#payment_mode', function () {
            ...
            var companyId = $('option:selected', '#company_id').val();
