                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="margin: 20px">
                  ...
                  <tr>
                    ...
                    <td style="padding: 7px;width:25%">
                      ...
                      @elseif($row->type==8)
                      {{(getMemberCustom($row->associate_id)) ? getMemberCustom($row->associate_id)->first_name ." ".getMemberCustom($row->associate_id)->last_name . " (".getMemberCustom($row->associate_id)->associate_no.")":"N/A"}}
                      @endif
                    </td>
                  ...
                  <tr>
                    <td style="padding: 5px;width:25%"> Receive Amount :</td>
                    <td style="padding: 7px;width:25%">{{number_format((float)$row->amount, 2, '.', '')}} &#x20B9;</td>
                    <td style="padding: 7px;width:25%"> Receive Mode :</td>
                    <td style="padding: 7px;width:25%">@if($row->received_mode==1) Cheque @elseif($row->received_mode==2) Online @else Cash @endif</td>
                  </tr>
                  @if($row->received_mode==1)
                  <tr>
                    <td style="padding: 7px;width:25%"> Cheque No :</td>
                    <td style="padding: 7px;width:25%"> {{$row['rvCheque']->cheque_no}}</td>
                    <td style="padding: 7px;width:25%"> Cheque Date :</td>
                    <td style="padding: 7px;width:25%">{{ date("d/m/Y", strtotime($row->cheque_date)) }} </td>
                  </tr>
                  <tr>
                    <td style="padding: 7px;width:25%"> Party Name :</td>
                    <td style="padding: 7px;width:25%"> {{$row['rvCheque']->account_holder_name}}</td>
                    <td style="padding: 7px;width:25%"> Party Bank :</td>
                    <td style="padding: 7px;width:25%">{{$row['rvCheque']->bank_name}}</td>
                  </tr>
                  <tr>
                    <td style="padding: 7px;width:25%"> Party Bank A/c :</td>
                    <td style="padding: 7px;width:25%">{{$row['rvCheque']->cheque_account_no}}</td>
                    <td style="padding: 7px;width:25%"> Receive Bank :</td>
                    <td style="padding: 7px;width:25%"> {{getSamraddhBank($row['rvCheque']->deposit_bank_id)->bank_name}}</td>
                  </tr>
                  <tr>
                    <td style="padding: 7px;width:25%"> Receive Bank A/c :</td>
                    <td style="padding: 7px;width:25%"> {{getSamraddhBankAccountId($row['rvCheque']->deposit_account_id)->account_no}}</td>
                    <td style="padding: 7px;width:25%"> Account Head :</td>
                    <td style="padding: 7px;width:25%"> {{getAcountHeadNameHeadId($row->account_head_id)}} </td>
                  </tr>
                  ...
                  <tr>
                    ...
                    <td style="padding: 7px;width:25%">{{ date("d/m/Y", strtotime($row->online_tran_date)) }} </td>
                  </tr>
                  <tr>
                    ...
                    <td style="padding: 7px;width:25%">{{$row->online_tran_bank_name}}</td>
                  </tr>
                  <tr>
                    <td style="padding: 7px;width:25%"> Transaction Bank A/c :</td>
                    <td style="padding: 7px;width:25%">{{$row->online_tran_bank_ac_no}}</td>
                    <td style="padding: 7px;width:25%"> Receive Bank :</td>
                    <td style="padding: 7px;width:25%"> {{getSamraddhBank($row->receive_bank_id)->bank_name}}</td>
                  </tr>
                  <tr>
                    <td style="padding: 7px;width:25%"> Receive Bank A/c :</td>
                    <td style="padding: 7px;width:25%"> {{getSamraddhBankAccountId($row->receive_bank_ac_id)->account_no}}</td>
                    <td style="padding: 7px;width:25%"> Account Head :</td>
                    <td style="padding: 7px;width:25%"> {{getAcountHeadNameHeadId($row->account_head_id)}} </td>
                  </tr>
                  @else
                  ...
                  @if (isset(getGstTransation($row->id)->amount_of_tax_igst) && $row->type == 6)
                  <tr>
                  ...
                  @endif
                  @if (isset(getGstTransation($row->id)->amount_of_tax_cgst) && $row->type == 6)
                  <tr>
