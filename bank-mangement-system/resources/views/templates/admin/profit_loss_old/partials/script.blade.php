    ...
    if($('#profit-loss-filter').valid())
    {
      ...
      $('#filter_data').html(''); 
       $.ajax({
              ...
              dataType: 'JSON',
              data: {'is_search':is_search,'start_date':start_date,'to_date':to_date,'branch_id':branch,'financial_year':financial_year},
              headers: {
...
function resetForm()
{
   ...
   $('#is_search').val("no");  
    ...
    $.ajax({
          ...
          url: "{!! route('admin.profit_loss_fillter') !!}",
          dataType: 'JSON',
            data: {'is_search':is_search,'start_date':start_date,'to_date':to_date},
          headers: {
              'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
          },
          success: function(response) { 
