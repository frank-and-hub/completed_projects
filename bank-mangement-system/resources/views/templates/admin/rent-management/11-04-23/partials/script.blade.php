     ...
     if(to>=from)
      {
        ...
        $.validator.messages.chk_created = "Register date  must be greater than or equal to employee date.";
        result = false;  
      }
...
if($('#owner_ssb_account').val()!='' )
{
     if(to>=from)
      {
        ...
        $.validator.messages.chk_created_ssb = "Register date  must be greater than or equal to SSB account date.";
        result = false;  
      }
}     
    ...
    $('#add-rent-liability,#edit-rent-liability').validate({ // initialize the plugin
        rules: {
            ...
            'select_date' : {required: true,dateDdMm: true,},
            'branch' : {required: true},
            'rentType' : {required: true},
            'agreement_from' : {required: true,dateDdMm: true,agrDateVal:true},
            'agreement_to' : {required: true,dateDdMm: true,agrDateVal:true},
            //'date' : {required: true},
            'place' : {required: true},
            'owner_name' : {required: true},
            'owner_mobile_number' : {required: true,number: true,minlength: 10, maxlength: 12 },
            'owner_pen_card' : {required: true,checkPenCard: true},
            'owner_aadhar_card' : {required: true,checkAadhar: true},
          //  'owner_ssb_account' : {required: true,number: true},
          //  'owner_ssb_id' : {required: true,},
            'bank_name' : {required: true},
            'bank_account_number' : {required: true,number: true, minlength: 8,maxlength: 20},
            'ifsc_code' : {required: true,checkIfsc:true,},
            'security_amount' : {required: true,decimal: true,zero: true},
            'rent' : {required: true,decimal: true,zero: true},
            'yearly_increment' : {required: true,decimal: true,check_per:true,zero: true},
            'office_area' : {required: true,decimal: true,zero: true},
            'employee_id' : {required: true},
            'employee_code' : {required: true},
            'employee_name' : {required: true},
            'employee_designation' : {required: true},
            'mobile_number' : {required: true,number: true},
            //'rent_agreement' : {required: true},
            'employee_date':{required: true,chk_created: true},
    ...
    $('#transferr_rent_amount').validate({ // initialize the plugin
        rules: {
            'owner_ssb_account' : {required: true},
            'bank' : {required: true},
            'bank_name' : {required: true},
            'bank_account_number' : {required: true, minlength: 8,maxlength: 20},
            'mode' : {required: true},
    ...
    rentLiability = $('#rent-liabilities-table').DataTable({
        ...
        columns: [
            ...
            {data: 'rent_agreement', name: 'rent_agreement'},
    ...
    rentPayable = $('#rent-payable-table').DataTable({
        ...
        ajax: {
            ...
            "type": "POST",
            "data":function(d) {d.searchform=$('form#rent_payable_filter').serializeArray()}, 
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
        },
    ...
    rentReport = $('#rent-report-table').DataTable({
        ...
        ajax: {
            ...
            "type": "POST",
            "data":function(d) {d.searchform=$('form#rent_report_filter').serializeArray()}, 
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
        },
    ...
    $('#select_all').on('click', function(){
      var rows = rentPayable.rows({ 'search': 'applied' }).nodes();
...
*/
    $('.filter-report').on('click', function(){
        ...
        $.ajax({
            ...
            dataType: 'JSON',
            data: {'isSearch':isSearch,'rentBranch':rentBranch,'rentMonth':rentMonth,'rentYear':rentYear,'rentType':rentType},
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function(response) {
          ...
          { 
            ...
            if(response.resCount==1)
            {
              if(ssb_account == response.account_no.account_no && $.trim(name) ==$.trim(response.name.toLowerCase()))
              {
                ...
                swal("Error!", "Owner name or ssb account holder name("+response.name.toLowerCase()+") not match!", "error");
