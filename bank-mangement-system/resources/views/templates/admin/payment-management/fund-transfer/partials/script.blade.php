<script type="text/javascript">
    ...
    $(document).ready(function () {
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(response.account, function(index, value) {
                        $("#bank_ac").append(`<option value='${value.id}'>${value.account_no}</option>`);
                    });
                }
            })
        })
        ...
        branchToHoTable = $('#branch_to_ho_listing').DataTable({
            ...
            ajax: {
                "url": "{!! route('admin.fundtransfer.branchtoholisting') !!}",
                "type": "POST",
                "data": function (d) {
        ...
        $(document).on('change', '#branch,#date', function () {
            ...
            if(!branchId){
                branchId = $("#edit_branch option:selected").val();
            }
            var editId = 0;
            var date = $('#date').val();
            ...
            if (date == '') {
                ...
                var branch_code = $('option:selected', "#branch").attr('data-code');
                if(!branch_code){
                    branch_code = $("#edit_branch option:selected").attr('data-value');
                }
    ...
    $(document).on('click', '.delete-transfer', function (e) {
        e.preventDefault();
        var id = $(this).data('id');
        var created_at = $('.gdatetime').html();
        var csrfToken = $('meta[name="csrf-token"]').attr('content');
        var t = $(this).data('title');
        swal({
            title: `Are you sure to delete ${t} ?`,
            text: "Reason for deletion", // Text preceding the textarea
            content: {
                element: "textarea", // Indicating that the content is a textarea
                attributes: {
                    id: "delete-reason",
            ...
            if (isConfirmed) {                               
                ...
                if(reason != ''){
                    $.post("{!! route('admin.fund_transfer.delete') !!}", { 'id': id, '_token': csrfToken ,'created_at':created_at,'remark':reason},function(e){
                        if(e.type == 'success'){
                            swal('Success!',e.msg,'success');
                            memberTable.draw();
                            branchToHoTable.draw();
                            bankToBankTable.draw();
                    ...
                    swal('error',"Reason for deletion is required",'error').then((isConfirmed) => {
                        $(this).click();
                    })
                }
            }
    ...
    $('.export').on('click', function (e) {
        ...
        if (extension == 0) {
            ...
            $('#report_export').val(extension);
            $('form#filter').attr('action', "{!! route('admin.fundTransfer.export') !!}");
            $('form#filter').submit();
        }
    });
    ...
    function doChunkedExport(start, limit, formData, chunkSize) {
        ...
        jQuery.ajax({
            type: "post",
            dataType: "json",
            url: "{!! route('admin.fundTransfer.export') !!}",
            data: formData,
            success: function (response) {
                console.log(response);
    ...
    function statusUpdate(id, status, branchId, companyId) {
        ...
        .then((willUpdate) => {
            if (willUpdate) {
                let url = "{{ route('admin.fund.transfer.updateStatus', ['id' => ':id', 'status' => ':status', 'branchid' => ':branchId', 'companyId' => ':companyId']) }}";
                url = url.replace(':id', id).replace(':status', status).replace(':branchId', branchId).replace(':companyId', companyId);
                $.ajax({
                    type: "GET",
                    url: url,
                    dataType: 'json',
                    data: { 'id': id, 'status': status, 'branchid': branchId, 'companyId': companyId },
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success: function (response) {
