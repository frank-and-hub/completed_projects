      ...
      <input type="file" name="co-applicant_id_file" id="co-applicant_id_file" class="form-control">
        ...
        @if(count($coapplicantFiles) > 0)
          @foreach($coapplicantFiles as $coapplicantFile)
            <span><a href="{{ URL('core/storage/images/loan/document/'.$loanDetails->id.'/coapplicant/id_proof/'.$coapplicantFile->file_name.'') }}" target="blank">{{ $coapplicantFile->file_name }}</a></span>
