...
$('#addrequest').validate({ // initialize the plugin
    ...
    submitHandler: function(form) {
        $('#addrequest').submit(function(event) {
            ...
            $.ajax({
                type: 'POST',
                url: "{!! route('branch.advancePayment.advancerequest') !!}",
                data: formData,
                contentType: false,
                processData: false,
                success: function(response) {
                    ...
                    swal({
                        ...
                        window.location.href =
                            "{{route('branch.advancePayment.requestList')}}";
                    });
...
$('#company_id').on('change', function() {
    ...
    $.ajax({
        type: "POST",
        url: "{!! route('branch.advancePayment.getOwnerNames') !!}",
        dataType: 'JSON',
        data: {
            'branch': branchId,
        ...
        success: function(response) {
            var optionHtml = '';
            $('#date').val("{{ headerMonthAvailability(date('d'),date('m'),date('Y'),$stateid) }}");
    ...
    $.ajax({
        type: "POST",
        url: "{!! route('branch.advancePayment.getemployeee') !!}",
        dataType: 'JSON',
        data: {
            'branch': branchId,
            'company_id': company_id,
            ...
            success: function(response) {
                ...
                if (paymenttype == 2 || paymenttype == 1) {
                    ...
                    if (response[0].branch_id == branchId) {
                        ...
                        // var formattedDate = $('#current_date').val();
                        // if (date1 < date2) {
