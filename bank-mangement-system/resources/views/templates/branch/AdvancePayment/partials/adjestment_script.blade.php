	...
	$(document).ready(function() {
		...
		$('#form').validate({ // initialize the plugin
			...
			rules: {
				...
				'account_head_more[0]': {
					required: function() {
						if ($("#account_head_0").data('value') == 1) {
							return true;
						} else {
							return false;
						}
				...
				'account_head_more[1]': {
					required: function() {
						if ($("#account_head_1").data('value') == 2) {
							return true;
						} else {
							return false;
						}
			...
			submitHandler: function(form) {
				...
				$('#form').submit(function(e) {
					...
					$.ajax({
						type: 'POST',
						url: "{!! route('branch.advancePaymentAdjestment.save') !!}",
						data: formData,
						success: function(response) {
							...
							if (response) {
								...
								// window.location.href = "{{ route('branch.advancePayment.requestList')}}";
							}
		...
		$("#add_row").click(function() {
			...
			$.ajax({
				type: "POST",
				url: "{!! route('branch.advancePayment.getHeads') !!}",
				dataType: 'JSON',
				headers: {
					'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
				},
				...
				success: function(response) {
					...
					expendHtml += '<td > <div class="col-lg-12 error-msg">  <select name="account_head_more[' + a + ']" id="account_head_' + a + '" data-append="no" class="account_head_more form-control" data-value=' + b + ' > <option value="">Select Account Head</option><option value="1" data-name="expence">Expense</option><option value="2" data-name="fixedasset">Fixed Assets</option></select></div>';
		...
		$(document).on('change', '.account_head_more', function() {
			...
			$.ajax({
				type: "POST",
				url: "{!! route('branch.advancePayment.get_indirect_expense_sub_head') !!}",
				dataType: 'JSON',
				data: {
					'head_id': id,
