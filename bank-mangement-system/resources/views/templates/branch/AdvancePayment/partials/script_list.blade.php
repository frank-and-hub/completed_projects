<script type="text/javascript">
  ...
  $(document).ready(function() {
    ...
    memberTable = $('#Advance_request').DataTable({
      ...
      ajax: {
        "url": "{!! route('branch.advancePayment.AdvancedRequestListing') !!}",
        "type": "POST",
        "data": function(d) {
  ...
  {{--
    ...
    memberInvestmentPaymentTable = $('#member_investment_payment_listing').DataTable({
      ...
      ajax: {
        "url": "{!! route('branch.member.investmentchequepaymentlisting') !!}",
        "type": "POST",
        "data": function(d) {
          // d.searchform = $('form#filter').serializeArray(),
          d.start_date = $('#start_date').val(),
    ...
    $('.member_export').on('click', function(e) {
      ...
      if (extension == 0) {
        ...
        $('form#filter').attr('action', "{!! route('branch.exportAdvanceRequestList') !!}");
    ...
    function doChunkedExport(start, limit, formData, chunkSize) {
      ...
      jQuery.ajax({
        type: "post",
        dataType: "json",
        url: "{!! route('branch.exportAdvanceRequestList') !!}",
        data: formData,
      ...
      swal({
        ...
        function(isConfirm) {
          if (isConfirm) {
            $.ajax({
              type: "POST",
              url: "{!! route('branch.action_blacklist_member_for_loan') !!}",
              dataType: 'JSON',
              data: {
                'memberID': memberID,
                'is_block': is_block
              ...
              success: function(response) {
                if (response.msg_type == 'success') {
                  ...
                  .then((value) => {
                    window.location.href = urls+'/branch/blacklist-members-on-loan';
                  });
