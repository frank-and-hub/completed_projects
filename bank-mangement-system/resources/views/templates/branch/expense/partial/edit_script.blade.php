<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#expenses').validate({
			...
			// 	if (particular == '') {
			// 		swal("Warning!", "Please Enter Particular!", "warning");
			// 		return false;
		...
		$('.export').on('click', function() {
			...
			$('form#filter').attr('action', "{!! route('branch.bill.export') !!}");
			$('form#filter').submit();
			return true;
		});
			...
			swal({
				...
				function(isConfirm) {
					if (isConfirm) {
						$.ajax({
							type: "POST",
							url: "{!! route('branch.expense.delete-expense') !!}",
							dataType: 'JSON',
							data: {
								'table_id': expense_id
							},
