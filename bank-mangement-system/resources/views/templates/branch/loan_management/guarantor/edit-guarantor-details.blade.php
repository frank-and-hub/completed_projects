...
<div class="form-group row">
  ...
  <div class="col-lg-4">
    <input type="text" name="guarantor_dob" id="guarantor_dob" class="form-control date_of_birth" value="{{ date("d/m/Y", strtotime(convertDate($loanDetail->dob))) }}">
  </div>
</div>
...
<div class="form-group row">
  ...
  <div class="col-lg-12">
    <input type="text" name="guarantor_dependents_number" id="guarantor_dependents_number" class="form-control" value="{{ $loanDetail->number_of_dependents }}" autocomplete="off">
  </div>
</div>
...
<div class="form-group row cheque-mode p-mode">
  ...
  <div class="col-lg-4">
    <input type="text" name="guarantor_designation" id="guarantor_designation" class="form-control" value="{{ $loanDetail->designation }}" readonly="">
  </div>
  ...
  <div class="col-lg-4">
    ...
    @if(count($guarantorFiles) > 0)
      @foreach($guarantorFiles as $guarantorFile)
        ...
        @php
          $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/id_proof/'.$guarantorFile->file_name.'';
          $url = ImageUpload::generatePreSignedUrl($foldeName);
        @endphp
        <span><a href="{{ $url }}" target="blank">{{ $guarantorFile->file_name }}</a></span>
...
<div class="form-group row">
  ...
  <div class="col-lg-4">
    ...
    @if(count($guarantorAddressFiles) > 0)
      ...
      @php
          $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/address_proof/'.$guarantorAddressFile->file_name.'';
          $url = ImageUpload::generatePreSignedUrl($foldeName);
        @endphp
        <span><a href="{{ $url }}" target="blank">{{ $guarantorAddressFile->file_name }}</a></span>
...
<div class="form-group row">
  ...
  <div class="col-lg-4">
    ...
    @if(count($guarantorIncomeFiles) > 0)
      ...
      @php
          $foldeName = 'loan/document/'.$loanDetails->id.'/guarantor/income_proof/'.$guarantorIncomeFile->file_name.'';
          $url = ImageUpload::generatePreSignedUrl($foldeName);
        @endphp
        <span><a href="{{ $url }}" target="blank">{{ $guarantorIncomeFile->file_name }}</a></span>
        ...
        <div class="col-lg-4">
          ...
          @if($files)
              ...
              @php
                ...
                $url = ImageUpload::generatePreSignedUrl($foldeName);
              @endphp
                  <span><a href="{{ $url }}" target="blank">{{ $file->file_name }}</a></span>
