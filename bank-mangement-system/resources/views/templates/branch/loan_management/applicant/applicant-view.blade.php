...
<div class="row">
    <div class="col-lg-12 ">
        ...
        @php
            $aDetails = getMemberCompanyDataNew($loanDetails->customer_id,$company->id);
        @endphp
        ...
        <div class="row">
            ...
            @php
                $url = URL::to('branch/member/detail/' . $loanDetails->customer_id . '?type=0');
            @endphp
            <div class="col-lg-7">
            @if ($loanDetails['loan']->loan_type != 'G')
                <a href="{{ $url }}" target="_blank">
                    {{ $aDetails->member->member_id }}
                </a>
            @else
                {{ $aDetails->member->member_id }}
            @endif
</div>
            <!-- <div class="col-lg-7"> {{ $aDetails->member->member_id }}</div> -->
        </div>
        
        <div class="row">
            <label class=" col-lg-4">Applicant Id</label> 
            <div class="col-lg-1">:</div>
            <div class="col-lg-7"> {{ $aDetails->member->memberCompany->member_id }}</div>
        </div>

        <div class="row">
            <label class=" col-lg-4">Name</label> 
            <div class="col-lg-1">:</div>
            <div class="col-lg-7  "> {{ $aDetails->member->first_name }} {{ $aDetails->member->last_name }}</div>
        </div>

        <div class="row">
            <label class=" col-lg-4">Father's Name</label> 
            <div class="col-lg-1">:</div>
            <div class="col-lg-7  "> {{ $aDetails->member->father_husband }} </div>
        </div>

        <div class="row">
            <label class=" col-lg-4">Email Id</label> 
            <div class="col-lg-1">:</div>
            <div class="col-lg-7  "> {{ $aDetails->member->email }} </div>
        </div>

        <div class="row">
            <label class=" col-lg-4">Mobile Number</label> 
            <div class="col-lg-1">:</div>
            <div class="col-lg-7  "> {{ $aDetails->member->mobile_no }} </div>
        </div>
    ...
    <div class="col-lg-12 ">
        ...
        <div class="row">                                   
            ...
            @elseif($LoanApplicant->id_proof_type==4)
                Identity Card 
            @elseif($LoanApplicant->id_proof_type==5)
                Aadhar Card
            @elseif($LoanApplicant->id_proof_type==6)
                Bank Passbook
            @elseif($LoanApplicant->id_proof_type==7)
                Electricity Bill
            @else
                Telephone Bill
            @endif--}}
            {{$address_proof_type[$LoanApplicant->id_proof_type]?ucwords($address_proof_type[$LoanApplicant->id_proof_type]):'N/A'}}
            </div>
        </div>
        <div class="row">
        ...
        <div class="row">
            ...
            <div class="col-lg-1">:</div>
            <div class="col-lg-7  "> 
                {{$address_proof_type[$LoanApplicant->address_proof_type]?ucwords($address_proof_type[$LoanApplicant->address_proof_type]):'N/A'}}
                {{--	0=>pen card,1=>DL,2=>Voter ID,3=>,4=>,5=>,6=>,7=>,8=>
