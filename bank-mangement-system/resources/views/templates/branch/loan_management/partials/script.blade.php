...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '.loan_type', function() {
            var selectVal = $('.loan_type').val();

            if (selectVal == 'G') {
                $('.group_leader_div').removeClass('d-none');
            } else {
                $('.group_leader_div').addClass('d-none');
            }
        });

        //Plans fetch as per compnay id for loan search form
        $(document).on('change', '#company_id', function() {
            var companyId = $('#company_id').val();
            $('#plan').find('option').remove().end().append(
                ' <option value="">----Select Loan Plan----</option>').val('');

            $.ajax({
                type: "POST",
                url: '{{ route("branch.loan.fetch") }}',
                dataType: 'JSON',
                data: {
        ...
        $('#loan_type').on('change', function() {
            var company_id = $('#transaction_loan_company_id').val();
            ...
            $.ajax({
                type: "POST",
                url: "{{ route('loan.getLoanPlanByType') }}",
                // url: "{{ route('branch.loan.getplanlist') }}",
                dataType: 'JSON',
                data: {
                    'loan_type': loanType,
        ...
        //Plans fetch as per compnay id for loan transaction search form
        // $(document).on('change', '#transaction_loan_company_id', function() {
        //     var company_id = $('#transaction_loan_company_id').val();
        //     var company_id = $('#transaction_loan_company_id').val();
        //     $('#plan').find('option').remove().end().append(
        //         ' <option value="">----Select Loan Plan----</option>').val('');
        ...
        //     $.ajax({
        //         type: "POST",
        //         url: '{{ route("transaction.loan.fetch") }}',
        //         dataType: 'JSON',
        //         data: {
        //             'company_id': company_id
        //         },
        ...
        // loan transactionentRequest = null;
        var guarantor = [];
        var groupMemberId = [];
        var today = new Date();
        const getFileChargeajax = (loantype, value, aDate, tenureId, purpose, node, emiOption, ROI, tenure,
            dInput) => {
            $.ajax({
                type: 'POST',
                ...
                success: function(response) {
                    let insuranceDetail = '0';
                    if (response.loans) {
                        if (parseFloat(value) >= parseFloat(response.loans.min_amount) &&
                            parseFloat(value) <= parseFloat(response.loans.max_amount) &&
                            response.loans.charge_type == 1 && response.loans.type == 1) {
                            var fileCharge = response.loans.charge;
                        } else if (parseFloat(value) >= parseFloat(response.loans.min_amount) &&
                            parseFloat(value) <= parseFloat(response.loans.max_amount) &&
                            response.loans.charge_type == 0 && response.loans.type == 1) {
                            var fileCharge = response.loans.charge * value / 100;
                        } else {
                            var fileCharge = '';
                        }
                        ...
                        calculateEmi(emiOption, value, tenure, ROI, insuranceDetail, fileCharge,
                            dInput);
                    } else {
                        if (node != 'emi_mode_option') {
                            ...
                            swal("Warning!", "File charge is not created on selected date",
                                "warning");
                            return false;
                        }
                    }
        ...
        //loan search filter form validation
        $('#filter').validate({
            rules: {
                'company_idd': 'required',
                'loan_type': 'required'
            },
            messages: {
                'company_idd': 'Select the company.',
                'loan_type': 'Select the loan type.'
            }
        });

        //Emi Transaction search filter form validation
        $('#loan_emi_transaction_filter').validate({
            rules: {
                'emi_transaction_company_id': 'required'
            },
            messages: {
                'emi_transaction_company_id': 'Select the company.'
            }
        });

        //Group loan search filter form validation


        //Loan Transaction search form validation
        $('#transaction-loan-filter').validate({
            rules: {
                'transaction_loan_company_id': 'required',
                'loan_type': 'required'
            },
            messages: {
                'transaction_loan_company_id': 'Select the Company.',
                'loan_type': 'Select the loan type.'
            }
        });
        ...
        // jQuery.validator.addMethod("notEqualIFSC", function(value, element, param) {
        //  return this.optional(element) || value != $(param).val();
        // }, "IFSC Number should be different");
        // jQuery.validator.addMethod("notEqualIFSC1", function(value, element, param) {
        //  return this.optional(element) || value != $(param).val();
        // }, "IFSC Number should be different");
        // jQuery.validator.addMethod("notEqualIFSC2", function(value, element, param) {
        //  return this.optional(element) || value != $(param).val();
        // }, "IFSC Number should be different");
        // jQuery.validator.addMethod("notEqualIFSC3", function(value, element, param) {
        //  return this.optional(element) || value != $(param).val();
        // }, "IFSC Number should be different");
        // jQuery.validator.addMethod("notEqualIFSC4", function(value, element, param) {
        //  return this.optional(element) || value != $(param).val();
        // }, "IFSC Number should be different");
        // jQuery.validator.addMethod("notEqualIFSC", function(value, element, param) {
        //     const IFSC = $(param+'_ifsc_code').val();
        //     const ac = $(param+'_bank_account_number').val();
        //     console.log(ac);
        //  return this.optional(element) || (value != ac && value != IFSC);
        // }, "IFSC Number And Account Number should be different");
        // jQuery.validator.addMethod("notEqualIFSC2", function(value, element, param) {
        //     const IFSC = $(param+'_ifsc_code').val();
        //     const ac = $(param+'_bank_account_number').val();
        //  return this.optional(element) || (value != ac && value != IFSC);
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.resCount > 0) {
                        ...
                        var firstName = response.member[0].first_name ? response.member[0]
                            .first_name : '';
                        var lastName = response.member[0].last_name ? response.member[0]
                            .last_name : '';
                        var ass_name = firstName + ' ' + lastName;
                        ass_name ? $('#acc_name').val(ass_name) : $('#acc_name').val(
                            "Name N/A");
                        response.bAccount ? $('.' + attVal + '-bank-account').val(response
                            .bAccount) : $('.' + attVal + '-bank-account').val("");
                        response.bIfsc ? $('.' + attVal + '-ifsc-code').val(response
                            .bIfsc) : $('.' + attVal + '-ifsc-code').val("");
                        response.bName ? $('.' + attVal + '-bank-name').val(response
                            .bName) : $('.' + attVal + '-bank-name').val("");
                        response.member[0].carders_name ? $('#acc_carder').val(response
                            .member[0].carders_name) : $('#acc_carder').val(
                            "Carder N/A");
        ...
        //                 }
        //                 if (loantype == 4 && response.member.saving_account.length != 0) {
        ...
        //                             if (value.plan_id != 1 && value.plan_id != 13) {
        //                                 $.ajax({
        //                                     type: "POST",
        ...
        // Get registered member by id
        $(document).on('change', '#co-applicant_auto_member_id,#guarantor_auto_member_id,#group_auto_member_id',
            function() {
                $.cookie('planTbaleCounter', '');
                var memberid = $(this).val();
                var attVal = $(this).attr('data-val');
                ...
                var currentRequest = $.ajax({
                    ...
                    success: function(response) {
                        if (response.msg_type == "success") {
                            ...
                            $("#" + attVal + "_id_number").val('');
                            $("#" + attVal + "_address_id_number").val('');
                            if (attVal == 'group-loan' && response.member.saving_account
                                .length == 0) {
                                $('#group_auto_member_id').val('');
                                swal("Warning!",
                                ...
                                if (response.member.member_id_proofs) {
                                    $("#applicant_id_proof option[value=" + response.member
                                        .member_id_proofs.first_id_type_id + "]").attr(
                                        "data-proof-val", '' + response.member
                                        .member_id_proofs.first_id_no + '');
                                    $("#applicant_address_id_proof option[value=" + response
                                            .member.member_id_proofs.second_id_type_id + "]")
                                        .attr("data-proof-val", '' + response.member
                                            .member_id_proofs.second_id_no + '');
                                }
                            }
                            /*else if(attVal=='group-loan' && jQuery.inArray(memberid, guarantor) == -1){
                                                    $('#group_auto_member_id').val('');
                            ...
                            if (attVal == 'guarantor') {
                                //$('.guarantor-name-section').hide();
                                $('#guarantor_occupation_id').html('');
                                $('#guarantor_occupation_id').append(
                                    '<option value="">Select Type</option><option value="1">Government Employee</option><option value="2">Private Employee</option><option value="3">Self Employees</option><option value="4">Other</option>'
                                );
                                $('#guarantor_occupation_id').append(
                                    '<option selected value="' + response.occupation_id +
                                    '">' + response.occupation + '</option>');
                                $('#guarantor_occupation_id').prop('disabled', true);
                                //$('.guarantor-member-detail-box').hide();
                                if (jQuery.inArray(memberid, guarantor) !== -1) {
                                    $('#guarantor_auto_member_id').val('');
                                    swal("Warning!", "Guarantor should not be a group member!",
                                        "warning");
                                    $('.' + attVal + '-member-detail').html('');
                                    $('#' + attVal + '_member_id').val('');
                                    $('#' + attVal + '_occupation_name').val('');
                                ...
                                //$('.guarantor-name-section').hide();
                                var gfirstName = response.member.first_name ? response.member
                                    .first_name : '';
                                var glastName = response.member.last_name ? response.member
                                    .last_name : '';
                                $('#' + attVal + '_name').val(gfirstName + ' ' + glastName);
                                $('#' + attVal + '_father_name').val(response.member
                                    .father_husband);
                                $('#' + attVal + '_dob').val(moment(response.member.dob).format(
                                    'DD/MM/YYYY'));
                                $("#guarantor_marital_status option[value=" + response.member
                                    .marital_status + "]").attr('selected', 'selected');
                                $('#local_address').val(response.member.address);
                                $('#' + attVal + '_mobile_number').val(response.member
                                    .mobile_no);
                            }
                            $('.' + attVal + '-member-detail').html(response.view);
                            $('#' + attVal + '_member_id').val(response.id);
                            ...
                            $('#' + attVal + '_designation').val(response.carderName);
                            if (response.member.member_id_proofs) {
                                $("#" + attVal + "_id_proof option[value=" + response.member
                                    .member_id_proofs.first_id_type_id + "]").attr(
                                    "data-proof-val", '' + response.member.member_id_proofs
                                    .first_id_no + '');
                                $("#" + attVal + "_address_id_proof option[value=" + response
                                    .member.member_id_proofs.second_id_type_id + "]").attr(
                                    "data-proof-val", '' + response.member.member_id_proofs
                                    .second_id_no + '');
                            }
                            if (loantype == 4) {
                                ...
                                if (response.member['associate_investment'].length != 0) {
                                    ...
                                    var isRecordExist = false;
                                    var i = 0;
                                    var invesmentLength = response.member[
                                        'associate_investment'].length;
                                    $.each(response.member['associate_investment'], function(
                                        key, value) {
                                        console.log('ttt', key + ": " + value.id);
                                        var months = value.tenure * 12;
                                        let cdate = new Date(value.created_at)
                                        let newDate = cdate.getDate() + "/" + (cdate
                                                .getMonth() + 1) + "/" + cdate
                                            .getFullYear()
                                        var dt = new Date(value.created_at);
                                        dt.setMonth(months);
                                        let current_datetime = new Date(dt)
                                        let duenewDate = current_datetime.getDate() +
                                                ...
                                                success: function(response) {
                                                    ...
                                                    if (sixMonthsFromNow
                                                        ...
                                                        $.cookie(
                                                            ...
                                                            key);
                                                        var list_fieldHTML =
                                                            '<tr><td class="plan-name">' +
                                                            response
                                                            .planName +
                                                            '<input type="hidden" name="investmentplanloanid[' +
                                                            key +
                                                            ']" value="' +
                                                            value.id +
                                                            '" class="form-control"></td><td class="account-id">' +
                                                            value
                                                            .account_number +
                                                            '</td><td class="open-date">' +
                                                            newDate +
                                                            '</td><td class="due-date">' +
                                                            duenewDate +
                                                            '</td><td class="deposite-amount">' +
                                                            value
                                                            .current_balance +
                                                            '<input type="hidden" name="hidden_deposite_amount"  class="hidden_deposite_amount-' +
                                                            key +
                                                            ' form-control" value="' +
                                                            value
                                                            .current_balance +
                                                            '"></td><td class="plan-months">' +
                                                            months +
                                                            '</td><td class="loan-amount-input"><input data-input="' +
                                                            key +
                                                            '" type="text" name="ipl_amount[' +
                                                            key +
                                                            ']" class="ipl_amount ipl_amount-' +
                                                            key +
                                                            ' form-control" style="width: 104px"></td></tr>';
                                                        $('.investment-plan-input-number')
                                                            .append(
                                                                list_fieldHTML
                                                            );
                                                    }
                                        ...
                                        i++;
                                        if (i == invesmentLength && isRecordExist ==
                                            false) {
                                            swal("Warning!",
                                                "You have not any investment plan in 6 months!",
                                                "warning");
                                            $('.applicant-member-detail').html('');
                                            $('.' + attVal + '-member-detail').html('');
                                            $('#' + attVal + '_member_id').val('');
                ...
                },
                {
                    ...
                    "render": function(data, type, row) {
                        if (row.status == 0) {
                            ...
                            return '<a href="javascript:void(0);"  data-toggle="modal" data-target="#rejection-view" data-rejection="' +
                                row.rejection_description +
                                '" class="view-rejection"><i class="icon-eye-blocked2  mr-2"></i>Rejected</a>';
                        } else if (row.status == 3) {
                            return 'Clear';
                        } else if (row.status == 4) {
                            return 'Due';
                        } else if (row.status == 54) {
                            return 'Rejected';
        ...
        const companyId = $('#emi_transaction_company_id option:selected').val();

        loanEmiTable = $('#listtansaction').DataTable({
            processing: true,
            serverSide: true,
            pageLength: 500,
            "fnRowCallback": function(nRow, aData, iDisplayIndex) {
            ...
            ajax: {
                ...
                "type": "POST",
                "data": function(d) {
                    d.searchform = $('form#loan_emi_transaction_filter').serializeArray(),
                        d.loanId = loanId,
                        d.loanType = loanType
                },
                headers: {
...
*/
        ...
        $(document).on('change', '#sloan_type', function() {
            ...
            switch (!aDate) {
                ...
                case false:
                    $.ajax({
                        type: 'POST',
                        url: "{!! route('branch.getActiveLoans') !!}",
                        data: {
                            'aDate': aDate,
                            'loanType': loanType
                        },
                        ...
                        success: function(res) {
                            if (res.loans) {
                                ...
                                $.each(res.loans, function(key1, value1) {
                                    switch (key1) {
                                        case '1':
                                            labelnme = 'Personal Loan';
                                            break;
                                        case '2':
                                            labelnme = 'Staff Loan';
                                            break;
                                        ...
                                        case '4':
                                            labelnme =
                                                'Loan Against Investment Loan';
                                            break;
                                    }
                                    $('#loan').append('<optgroup label="' +
                                        labelnme + '"></optgroup>');
                                    $.each(value1, function(key, value) {
                                        let ROIData = '';
                                        let tenureData = '';
                                        if (value.loan_tenure) {
                                            ROIData = JSON.stringify(value
                                                .loan_tenure.ROI);
                                            tenureData = JSON.stringify(
                                                value.loan_tenure.tenure
                                            );
                                        }
                                        if (value.emi_option == 1)
                                            emiType = 'Month';
                                        else if (value.emi_option == 2)
                                            emiType = 'Week';
                                        else
                                            emiType = 'Days';
                                        $('#loan').append(
                                            ...
                                            tenureData +
                                            '"  data-gstStatus = "' +
                                            res.gstData.IntraState +
                                            '" data-gstPer = "' + res
                                            .gstData.gst_percentage +
                                            '" data-gstFileStatus = "' +
                                            res.gstFileChargeData
                                            .IntraState +
                                            '" data-gstFilePer = "' +
                                            res.gstFileChargeData
                                            .gst_percentage + '" >' +
                                ...
                                $('.error-msg-loan').html(
                                    'Plan is Not created on selected Date');
                            }
                        }
                    })
                    break;
        ...
        tenureAppend = (aDate, loanType, purpose) => {
            $.ajax({
                type: 'POST',
                url: "{!! route('branch.getActiveLoans') !!}",
                data: {
                ...
                success: function(res) {
                    $('#emi_mode_option').empty().append(
                        '<option value="">---Please Select Emi---</option>');
                    $('#emi_mode_option_grp').empty().append(
                        '<option value="">---Please Select Emi---</option>');
                    $.each(res.loans, function(key, value) {
                        ...
                        if (value.emi_option == 1) {
                            ...
                            emiOption = 'Weeks';
                        } else if (value.emi_option == 3) {
                            emiOption = 'Days';
                        }
                        (loanType != 3) ? $('#emi_mode_option').append(
                            '<option value="' + value.id + '"  data-emiOption=' +
                            value.emi_option + ' data-ROInterest=' + value.ROI +
                            ' data-tenure=' + value.tenure + ' >' + value.name +
                            ' </option>'): $('#emi_mode_option_grp').append(
                            '<option value="' + value.id + '"  data-emiOption=' +
                            value.emi_option + ' data-ROInterest=' + value.ROI +
                            ' data-tenure=' + value.tenure + ' >' + value.name +
                            ' </option>');
        ...
        $(document).on('change', '#loan', function() {
            ...
            var loanType = $('option:selected', this).val();
            const category = $("option:selected", this).attr('data-category');
            const roiMode = $("option:selected", this).attr('data-roiPeriod');
            const aDate = $('.application_date').val();
            tenureAppend(aDate, loanType, 'tenure');
            $('.applicant-member-detail').html('');
            //$('#emi_mode_option').val(roiMode);
        ...
        $(document).on('change',
            '#applicant_id_proof,#applicant_address_id_proof,#co-applicant_id_proof,#co-applicant_address_id_proof,#guarantor_id_proof,#guarantor_address_id_proof',
            function() {
                var sectionval = $('option:selected', this).attr('data-val');
                var proofValue = $('option:selected', this).attr('data-proof-val');
                var loanType = $("#loan option:selected").attr('data-category');;
                if ($(this).val() == 1) {
                    $('#' + sectionval + '_id_tooltip').attr('data-original-title',
                        'Enter proper voter id number. For eg:- ABE1234566');
                } else if ($(this).val() == 2) {
                    $('#' + sectionval + '_id_tooltip').attr('data-original-title',
        ...
        calculateEmi = (emiOption, value, tenure, ROI, insuranceDetail, fileCharge, dInput) => {
            const gstStatus = $("#loan option:selected").attr('data-gststatus');
            const gstPercentage = $("#loan option:selected").attr('data-gstper');
            const gstFileStatus = $("#loan option:selected").attr('data-gstFilestatus');
            const gstFilePercentage = $("#loan option:selected").attr('data-gstFileper');
            ...
            calculateFileGst = () => {
                ...
                console.log(gstFileStatus, gstFilePercentage);
                $('#gstFileAmount').val(Math.ceil(genearateFilegstAmount));
                $('#gstFileStatus').val((gstFileStatus));
                $('#ml_gst_status').val(1);
                $('.g-loan-hidden-gst-file-charge-' + dInput + '').val(Math.ceil(
                    genearateFilegstAmount));
            ...
            var InsuranceAmount = () => {
                switch (value >= insuranceDetail.min_amount && value <= insuranceDetail.max_amount) {
                    case true:
                        const insAmount = (chargeType == 1 ? insuranceDetail.charge : ((value *
                            insuranceDetail.charge) / 100));
                        ...
                        calculateGst = () => {
                            ...
                            $('.g-loan-hidden-gst-charge-' + dInput + '').val(Math.ceil(
                                genearategstAmount));
        ...
        $(document).on('change', '#amount,#salary,#emi_mode_option', function() {
            ...
            const tenureId = $("#emi_mode_option option:selected").val();
            const emiOption = $("#emi_mode_option option:selected").attr('data-emioption');
            const ROI = $("#emi_mode_option option:selected").attr('data-ROInterest');
            const aDate = $('.application_date').val();
            ...
            // const insuranceMaxAmount = $( "#emi_mode_option option:selected" ).attr('data-maxAmount');
            // const insuranceAmount = $( "#emi_mode_option option:selected" ).attr('data-insuranceAmount');
            ...
            if (category == 1 || category == 2) {
                if ((value < minAmount || value > maxAmount) && value != '') {
                    ...
                    $('#loan-amount-error').html('Please enter amount between ' + minAmount + ' to ' +
                        maxAmount);
                    $('#loan_emi').val('');
                    //$('.loan-emi-amount').html('Loan EMI: ');
                    $('.loan-emi-amount').html('');
                    ...
                    if (aDate == '') {
                        ...
                        getFileChargeajax(loantype, value, aDate, tenureId, 'tenure', node, emiOption,
                            ROI, tenure);
                    }
                }
            } else {
        ...
        $(document).on('change', '#number_of_member', function() {
            ...
            for (var x = 1; x <= mNumber; x++) {
                var list_fieldHTML = '<tr><td><input data-input="' + x + '" type="text" name="m_id[' +
                    ...
                    ' form-control" style="width: 104px"></td><td><input data-input="' + x +
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.msg_type == "success") {
                        ...
                        $('.g-loan-member-fname-' + dInput + '').val(response.member
                            .father_husband);
                        $('.g-loan-member-s-' + dInput + '').html(response.member
                            .signature);
                        ...
                        if (response.member.member_bank_details.length > 0 && response
                            .member.saving_account.length != 0) {
                            $('.g-loan-member-bn-' + dInput + '').val(response.member
                                .member_bank_details[0].bank_name);
                            $('.g-loan-member-bac-' + dInput + '').val(response.member
                                .saving_account[0]['account_no']);
                            $('.g-loan-member-bi-' + dInput + '').val(response.member
                                .member_bank_details[0].ifsc_code);
                        } else {
                            $('.g-loan-member-bn-' + dInput + '').val('');
                            $('.g-loan-member-bac-' + dInput + '').val('');
                            $('.g-loan-member-bi-' + dInput + '').val('');
                        }
                        $('.g-loan-member-total-deposit-amount-' + dInput + '').val(response
                            .totalDeposit)
        ...
        $('.m-input-number').on('change', '.g-loan-member-amount', function() {
            ...
            const tenureId = $("#emi_mode_option_grp option:selected").val();
            const aDate = $('.application_date').val();
            const node = $(this).attr('id');
            var loantype = $("#loan option:selected").val();
            const emiOption = $("#emi_mode_option_grp option:selected").attr('data-emioption');
            const ROI = $("#emi_mode_option_grp option:selected").attr('data-ROInterest');
            const insuranceDetail = $("#emi_mode_option_grp option:selected").attr('data-gstInsurance');
            const tenure = $("#emi_mode_option_grp option:selected").attr('data-tenure');
            if (mId != '') {
                if ($(this).val() > 9999) {
                    var sum = 0;
                    $(".g-loan-member-amount").each(function() {
                ...
                getFileChargeajax(loantype, value, aDate, tenureId, "tenure", node, emiOption, ROI,
                    tenure, dInput)
            }
        });
        ...
        $('.investment-plan-input-number').on('focus', '.ipl_amount', function() {
            var dataval = $(this).attr('data-input');
            $(".ipl_amount").each(function() {
                $(this).on('keyup', function() {
                    const investment = $(this).attr('data-investment');
                    const laiAmnount = $(this).val();
                    const applicantDetail = $('#applicant_member_id').val();
                    $.ajax({
                        type: "POST",
                        url: "{!! route('branch.getInvestmentLoan.exist') !!}",
                        dataType: 'JSON',
                        data: {
                            'applicantDetail': applicantDetail,
                            'invetsment_id': investment
                        },
                        headers: {
                        ...
                        success: function(res) {
                            if (res.msg_type == 'warning') {
                                $('.ipl_amount-' + dataval).val('');
        ...
        $(document).on('click', '.submit-loan-form', function() {
            var loantype = $("#loan option:selected").val();
            var aDate = $("#application_date").val();
            if (loantype == '' || aDate == '') {
        ...
        $('#loan_emi_payment_mode').on('change', function() {
            ...
            if (paymentMode == 0 && paymentMode != '') {
                ...
                if (ssb_AccountNumber > 0) {
                    $.ajax({
                        type: "POST",
                        url: "{!! route('investment.planform_saving_account') !!}",
                        data: {
        ...
        $(document).on('click', '.pay-emi', function(e) {
            ...
            console.log("ssbAmount",ssbAmount);
            $('#loan_id').val(loanId);
        ...
        $('#deposite_amount,#penalty_amount').on('change', function() {
           var tAmount = 0
            ...
            $('#total_online_amount').val(parseInt(tAmount) + parseInt(penaltyAmount));
        });
        // Show loading image
        $(document).ajaxStart(function() {
            $(".loader").show();
    ...
    function showpay(amount, month, rate, divideBy) {
        ...
        if ((amount == null || amount.length == 0) || (month == null || month.length == 0) || (rate == null || rate
                .length == 0)) {
            var emi = '';
            return emi;
        } else {
    ...
    function loanTransactionResetForm() {
        ...
        $('#is_search').val("no");
        $('#date').val('');
        $('#transaction_loan_company_id').val('');
        $('#loan_type').val('');
        $('.from_date').val('');
        ...
        $('#payment_mode').val('');
        $('#plan').empty();
        $('#plan').append('<option value="">----Select Loan Plan----</option>');
        loantransactiontable.draw();
    }

    //EMI Transaction Part
    function emiTransactionSearchForm() {
        if ($('#loan_emi_transaction_filter').valid()) {
            $('#is_search').val("yes");
    ...
    function emiTransactionResetForm() {
        var form = $("#loan_emi_transaction_filter"),
            validator = form.validate();
        validator.resetForm();
        form.find(".error").removeClass("error");
        $('#is_search').val("no");
                ...
                success: function(response) {
                    ...
                    if (response.msg_type == "success") {
                        ...
                        if (attVal == 'group-loan' && response.member.saving_account
                            ...
                            $('.' + attVal + '-occupation').val('');
                            return false;
                        } else if (attVal == 'group-loan' && jQuery.inArray(memberid,
                                guarantor) == -1) {
                            $('#group_auto_member_id').val('');
                            swal("Warning!", "Applicant should be group member!",
                                "warning");
                            $('.' + attVal + '-member-detail').html('');
                            $('#' + attVal + '_member_id').val('');
                            $('#' + attVal + '_occupation_name').val('');
                        ...
                        $("#" + attVal + "_id_proof option[value=" + response.member
                            ...
                            "data-proof-val", '' + response.member.member_id_proofs
                            .first_id_no + '');
                        $("#" + attVal + "_address_id_proof option[value=" + response.member
                            .member_id_proofs.second_id_type_id + "]").attr(
                            "data-proof-val", '' + response.member.member_id_proofs
                            .second_id_no + '');
                        if (loantype == 4) {
                            $('.loan-against-investment-plan').show();
        ...
        $(document).on('change', '#number_of_member', function() {
            ...
            for (var x = 1; x <= mNumber; x++) {
                var list_fieldHTML = '<tr><td><input data-input="' + x + '" type="text" name="m_id[' +
                    ...
                    ']" class="g-loan-hidden-emi g-loan-hidden-emi-' + x + '"><input data-input="' + x +
                    '" type="hidden" name="ml_file_charge[' + x +
                    ']" class="g-loan-hidden-file-charge g-loan-hidden-file-charge-' + x +
                    '"><input data-input="' + x + '" type="hidden" name="ml_interest_rate[' + x +
                    ']" class="g-loan-hidden-interest-rate g-loan-hidden-interest-rate-' + x +
                    '"></td><td><input data-input="' + x + '" type="text" name="m_name[' + x +
                    ']" class="g-loan-member-name g-loan-member-name-' + x +
                    ' form-control" style="width: 104px" readonly=""></td><td><input data-input="' + x +
                    '" type="text" name="f_name[' + x +
                    ']" class="g-loan-member-fname g-loan-member-fname-' + x +
                    ...
                    '" type="text" name="m_amount[' + x +
                    ']" class="g-loan-member-amount g-loan-member-amount-' + x +
                    ' form-control" style="width: 104px"></td><td><a target="blank" data-input="' + x +
                    '" class="g-loan-member-s g-loan-member-s-' + x +
                    '" href="javascript:void(0);" style="width: 104px"></a></td><td><input data-input="' +
                    x + '" type="text" name="m_bn[' + x +
                    ']" class="g-loan-member-bn g-loan-member-bn-' + x +
                    ' form-control" style="width: 104px" readonly=""></td><td><input data-input="' + x +
                ...
                success: function(response) {
                    ...
                    if (response.msg_type == "success") {
                        ...
                        $('.g-loan-member-s-' + dInput + '').html(response.member
                            .signature);
                        $('.g-loan-member-s-' + dInput + '').attr('href',
                            'asset/profile/member_signature/' + response.member
                            .signature);
                        if (response.member.member_bank_details.length > 0) {
                            $('.g-loan-member-bn-' + dInput + '').val(response.member
                                .member_bank_details[0].bank_name);
                            $('.g-loan-member-bac-' + dInput + '').val(response.member
                                .member_bank_details[0].account_no);
                            $('.g-loan-member-bi-' + dInput + '').val(response.member
                                .member_bank_details[0].ifsc_code);
                        } else {
                            $('.g-loan-member-bn-' + dInput + '').val('');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.msg_type == 'success') {
                        ...
                        $('#associate_member_id').val(response.collectorDetails.id);
                        $('#loan_associate_name').val(firstName + ' ' + lastName);
                        $('#ssb_account_number').val(response.collectorDetails
                            .saving_account[0].account_no);
                        $('#ssb_account').val(response.ssbAmount);
                        $('#ssb_id').val(response.collectorDetails.saving_account[0].id);
                    } else if (response.msg_type == 'error') {
