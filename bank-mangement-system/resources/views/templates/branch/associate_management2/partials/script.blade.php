<script type="text/javascript">
  $(document).ready(function () {
    ...
    $(document).on('keyup', '#customer_id', function () {
      ...
      var code = $(this).val();
      if (code != '') {
        $.post("{!! route('branch.customerDataGet') !!}",{ 'code': code },function (response) {
            if (response.msg_type == "error2") {
              ...
              if (response.msg_type == "success") {
                ...
                $('#haveSsbAccount').val(response.haveSsbAccount);
                $('#Rd_Account_Investment').val(response.Rd_Account_Investment);
              } else if (response.msg_type == "error1") {
                $('#show_mwmber_detail').html('<div class="alert alert-danger alert-block">  <strong>Customer already associate!</strong> </div>');
              } else if(response.msg_type == "error3"){
                $('#customer_id').val('');
                $("#associate_register")[0].reset();
                $('#haveSsbAccount').val('');
                $('#Rd_Account_Investment').val('');  
                swal('Warning','For Associate Creation, it is compulsory to have an SSB Plan in every Company. Please contact the Administrator !','warning');
              }
              else{
              ...
              if (response.msg == 'block') {
                ...
                if (response.msg == 'InactiveAssociate') {
                  ...
                  $.each(response.data, function (index, value) {
                    $('#senior_name,#first_g_first_name').val(value.first_name + ' ' + value.last_name??'');
                    $('#senior_id').val(value.id);
                    $('#senior_mobile_no,#first_g_Mobile_no').val(value.mobile_no);
                    $('#seniorcarder_id').val(response.carder_id);
                    $('#first_g_address').val(value.address??'N/A');
                  });
                  $.post("{!! route('branch.getCarderAssociate.customer') !!}",{'id':response.carder_id},
                      function(response) { 
  ...
  */
    ...
    $('#associate_register').submit(function () {
      if ($('#associate_register').valid()) {
        ...
        if($('#haveSsbAccount').val() == '1'){
            $("#ssb_account_yes").trigger("click");
            // $('#ssb_account_detail').show();
        }else{
          $("#ssb_account_no").trigger("click");
          $('#ssb_account_detail').hide();
          // $('#ssb_account_form').show();
        }
        if($('#Rd_Account_Investment').val() == '1'){
          // $('#rd_account_detail').show();
          rdmaturity();
          $('#rd_account_form').hide();
          $('#rd_account_yes').prop("checked", false);
          $('#rd_account_no').prop("checked", true);
          $('#RdAccountInvestment').hide();
        }else{ 
          $('#rd_account_yes').prop("checked", false);
          $('#rd_account_no').prop("checked", true);
    ...
    $('#associate_register').submit(function () {
      if ($('#associate_register').valid()) {
        ...
        if($('#haveSsbAccount').val() == 'No'){
          $('#ssb_account_no').click();
        }else{
          $('#associate_register_next').submit();
        }
   ...
   */
   /*
    $('#associate_register').submit(function () {
      if ($('#associate_register').valid()) {
        ...
        if($('#Rd_Account_Investment').val() == 'No'){
          $('#RdAccountInvestment').show();
        }else{
    ...
    */
    ...
    $('#associate_register_next').submit(function () {
      if ($('#associate_register_next').valid()) {
        ...
        if ($('#ssb_accountexists').val() == 0) {
          $('#associate_register_btn').prop('disabled', true);
          $.ajax({
            type: "POST",
            url: "{!! route('branch.associate_save.customer') !!}",
            dataType: 'JSON',
            data: formData,
            cache: false,
            contentType: false,
            ...
            success: function (response) {
              ...
              if (response.msg_type == "success") {
                $('#associate_register_btn').prop('disabled', false);
    ...
    $('#associate_register').validate({
      ...
      messages: {
        ...
        application_date: {
          ...
          dateDdMm: "Please enter a valid date.",
        },
    ...
    $(document).on('click', '#ssb_account_yes', function () {
      ...
      $.ajax({
        type: "POST",
        url: "{!! route('branch.associateSsbAccountGet.customer') !!}",
        dataType: 'JSON',
    ...
    $(document).on('change', '#payment_mode', function () {
      ...
      if (val == 1) {
        ...
        $.post("{!! route('branch.approve_recived_cheque_list') !!}",{'data':''},function (response) {
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.resCount == 1) {
            ...
            // swal("Info!", "Member already exists SSB account!", "info");
