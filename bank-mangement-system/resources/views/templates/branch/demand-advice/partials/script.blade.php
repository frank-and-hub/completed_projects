<script type="text/javascript">
    "use strict";
    let demandAdviceTable,demandAdviceReportTable,demandAdviceMaturityTable,taAdvancedReportTable,demandAdviceApplicationTable,demandAdviceFreshExpenseTable ;
      ...
      let branchid = $( "#hbranchid option:selected" ).val();
        $('#add_demand_advice,#edit_demand_advice').validate({ // initialize the plugin
            rules: {
                ...
                'maturity_bank_account_number_confirm' : {required: function(element) {
                    return $('#payment_mode').val() !== 'SSB';
                },minlength: 8,maxlength: 20,equalTo: "#maturity_bank_account_number"},
                'prematurity_bank_account_number_confirm' : {required: function(element) {
                    return $('#payment_mode').val() !== 'SSB';
                },minlength: 8,maxlength: 20,equalTo: "#prematurity_bank_account_number"},
                'maturity_ifsc_code' : {checkIfsc:true},
                // 'maturity_letter_photo': {
                //         required: {
                //             depends: function(element) {
                //                 return $("#maturity_letter_photo_org").is(":blank");
                //             }
                //         },filesize: 160000
                //     },
                ...
                'prematurity_bank_account_number' : {number: true,minlength: 8,maxlength: 20},
                'prematurity_ifsc_code' : {checkIfsc:true},
            ...
            prematurity_bank_account_number_confirm: {
                equalTo: "Bank A/C and confirm bank A/C must be same.",
              },
            maturity_bank_account_number_confirm: {
                equalTo: "Bank A/C and confirm bank A/C must be same.",
              },
            },
        ...
        $('#filter_ta_advanced_report').validate({ // initialize the plugin
            rules: {
                employee_code : {required: true},
                employee_name : {required: true},
                date_from:{dateDdMm : true},
                date_to:{dateDdMm : true},
            },
        });
        $('#application_filter_report').validate({ // initialize the plugin
            rules: {
        ...
        /*$(document).on('click','.add-fresh-expense',function(){
            ...
            var expenseSubCategoryTitle = $('option:selected', '#expense_subcategory').attr('data-val');
            var expenseSubCategory = $('option:selected', '#expense_subcategory').val();
            ...
            if(expenseCategory == '' || expenseSubCategory == '' || party_name == '' || particular == '' || mobile_number == '' || amount == '' || billNumber == ''){
                ...
                $('.fresh-expense-total-amount').before('<tr><td>'+expenseCategoryTitle+'</td><input type="hidden" name="fresh_expense['+countRecord+'][id]" value=""><input type="hidden" name="fresh_expense['+countRecord+'][expenseCategory]" value="'+expenseCategory+'"><td>'+expenseSubCategoryTitle+'</td><input type="hidden" name="fresh_expense['+countRecord+'][expenseSubCategory]" value="'+expenseSubCategory+'"><td>'+party_name+'</td><input type="hidden" name="fresh_expense['+countRecord+'][party_name]" value="'+party_name+'"><td>'+particular+'</td><input type="hidden" name="fresh_expense['+countRecord+'][particular]" value="'+particular+'"><td>'+mobile_number+'</td><input type="hidden" name="fresh_expense['+countRecord+'][mobile_number]" value="'+mobile_number+'"><td>'+amount+' &#8377</td><input type="hidden" name="fresh_expense['+countRecord+'][amount]" class="fe-amount" value="'+amount+'"><td>'+billNumber+'</td><input type="hidden" name="fresh_expense['+countRecord+'][billNumber]" value="'+billNumber+'"><td><input type="file" name="fresh_expense['+countRecord+'][bill_photo]"></td><td> &nbsp; <a href="javascript:void(0);" class="remCF">Remove</a></td></tr>');
        ...
        /*$(document).on('click','.edit-fresh-expense',function(){
            ...
            $("#expense_category option[value=" + expenseCategory +"]").prop("selected",true) ;
            $("#expense_subcategory option[value=" + expenseSubCategory +"]").prop("selected",true) ;
        ...
        demandAdviceTable = $('#demand-advice-table').DataTable({
            ...
            ajax: {
                "url": "{!! route('branch.demandadvice.list') !!}",
                "type": "POST",
        ...
        demandAdviceReportTable = $('#demand-advice-report-table').DataTable({
            ...
            ajax: {
                "url": "{!! route('branch.demandadvice.reportlist') !!}",
                "type": "POST",
        ...
        demandAdviceMaturityTable = $('#demand-advice-maturity-table').DataTable({
            ...
            ajax: {
                "url": "{!! route('branch.demandadvices.demand_advice_maturity_list') !!}",
                "type": "POST",
        ...
        taAdvancedReportTable = $('#ta-advanced-table').DataTable({
            ...
            ajax: {
                "url": "{!! route('branch.taadvanced.list') !!}",
                "type": "POST",
       ...
       var Startdate = new Date();
        ...
        $('.date-from,#date,#payment_date').datepicker({
            format: "dd/mm/yyyy",
            todayHighlight: true,
            // startDate:Startdate,
            endDate: date,
            ...
            $('.date-to').datepicker('setStartDate', e.date, 'format',"dd/mm/yyyy");
        });
        $('.date-to,#date,#payment_date').datepicker({
            format: "dd/mm/yyyy",
            orientation: "bottom",
            autoclose: true,
            endDate:new Date(),
        ...
        $(document).on('change','#ta_employee_code,#advanced_salary_employee_code',function(){
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('branch.demand.getemployee') !!}",
                dataType: 'JSON',
        ...
        $(document).on('change','#maturity_account_number,#prematurity_account_number,#death_help_account_number',function(){
            ...
            const branch = $('option:selected','#branch').val();
            if(type == 2){
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('branch.demand.getinvestment') !!}",
                dataType: 'JSON',
                data: {'val':val,'type':type,'subtype':subtype,'date':date},
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(response) {
                    ...
                    if(response.status == 200 ){
                        if(response.investmentDetails.branch_id != branch)
                        {
                        ...
                        if(response.investmentDetails.member){
                            ...
                            if((response.investmentDetails.member.signature)){
                                // $('.'+classVal+'_signature').html(' <img src="{{url('/')}}/asset/profile/member_signature/'+response.investmentDetails.member.signature+'" alt="signature" width="180" height="100">');
                                ...
                                // $('.'+classVal+'_photo').html(' <img src="{{url('/')}}/asset/profile/member_avatar/'+response.investmentDetails.member.photo+'" alt="signature" width="180" height="100">');
                                $('.'+classVal+'_photo').html(' <img src="'+photo+'" alt="photo" width="180" height="100">');
                                }else{
                                $('.'+classVal+'_photo').html(' <img src="{{url('/')}}/asset/images/no-image.png" alt="signature" width="180" height="100">');
                                }
                        }
                        ...
                        if(response.investmentDetails.member_bank_detail){
                            $('#'+classVal+'_bank_account_number').val(response.investmentDetails.member_bank_detail.bank_name);
                        ...
                        $('.'+classVal+'_signature').html('');
                        $('.'+classVal+'_photo').html('');
        ...
        $(document).on('click','.calculate-maturity',function(){
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('branch.demand.getinvestmentdata') !!}",
                data: {'investmentId':investmentId,'paymentType':paymentType,'subPaymentType':subPaymentType},
                dataType: 'JSON',
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(response) {
                    ...
                    $('.'+investmentId+'_maturity_amount_payable').val(tAmount);
        ...
        $(document).on('click','.submit-maturity',function(){
            ...
            /*$('form#investment-maturity-form').attr('action',"{!! route('branch.demand.saveInvestmentMaturityAmount') !!}");
        ...
        demandAdviceApplicationTable = $('#branch-demand-advice-application-table').DataTable({
            processing: true,
            serverSide: true,
            pageLength: 20,
            lengthMenu: [10, 20, 40, 50, 100],
            ...
            ajax: {
                "url": "{!! route('branch.demandadvice.applicationlist') !!}",
                "type": "POST",
        ...
        $(document).on('change','#is_assests,#mode',function(){
            var type = $(this).val();
            if(type == 0 && type != ''){
                $('.is-assets').show();
            }else if(type == 1 && type != ''){
        ...
        $(document).on('change','#bank_account_number', function () {
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('branch.demadadvice.getbankdaybookamount') !!}",
                dataType: 'JSON',
                data: {'fromBankId':account},
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(response) {
                    $('#available_balance').val(response.bankDayBookAmount);
                }
            });
        });
        $(document).on('change','#branch_id,#cash_type', function () {
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('branch.demadadvice.getbranchdaybookamount') !!}",
                dataType: 'JSON',
                ...
                success: function(response) {
                    if(cashType == 0){
                        $('#cash_in_hand_balance').val(response.microDayBookAmount);
                    }else if(cashType == 1){
                        $('#cash_in_hand_balance').val(response.loanDayBookAmount);
                    }
                }
            });
        });
        $('#death_help_nominee_member_id').on('change',function(){
            var val = $('#death_help_nominee_member_id').val();
            const companyId = $('#company_id').val();
            if(companyId != '')
            {
                $.ajax({
                type: "POST",
                url: "{!! route('branch.demand.getmemberdata') !!}",
                dataType: 'JSON',
                data:  {'val':val,'company_id':companyId},
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function(response) {
                    if(response.count > 0 && response.mDetails.saving_account__customnew.length != 0 ){
                        console.log(response.mDetails.saving_account__customnew);
                        ...
                        if(response.mDetails.savingAccount_Customnew != ''){
                            $('#nominee_ssb_account').val(response.mDetails.saving_account__customnew[0]['account_no']);
                        }else{
                            $('#nominee_ssb_account').val('');
                        }
                    }else{
                        swal("Warning!", "Customer Id Not Found!", "warning");
                        $('#nominee_name').val('');
       ...
       /*
        $('.export').on('click',function(){
            ...
            $('form#filter_report').attr('action',"{!! route('branch.demandadvice.export') !!}");
        ...
        */
        ...
        function doChunkedExport(start,limit,formData,chunkSize,fileName){
            ...
            jQuery.ajax({
                type : "post",
                dataType : "json",
                url :  "{!! route('branch.demandadvice.export') !!}",
                data : formData,
                success: function(response) {
                    console.log(response);
                    if(response.result=='next'){
                        start = start + chunkSize;
                        fileName = response.tokenName;
                        doChunkedExport(start,limit,formData,chunkSize,fileName);
        ...
        $(document).on('change','.expense_category ',function(){
            ...
            if(head_id > 0){
                $.ajax({
                    type: "POST",
                    url: "{!! route('branch.getHeadLedgerData') !!}",
                    dataType: 'JSON',
    ...
    $(document).on('change', '#payment_mode', function(e){
        ...
        if(payment_mode == "SSB"){
            ...
            $('.required__ssb_account').show();
                ...
                $.ajax({
                    type: "POST",
                    url: "{!! route('branch.getSSBAccountNumber') !!}",
                    dataType: 'JSON',
    ...
    function datediff() {
            ...
            var f1 = moment($('.date-from').val() + ' 00:00', moment.defaultFormat).toDate();
            var f2 = moment($('.date-to').val() + ' 00:00', moment.defaultFormat).toDate();
