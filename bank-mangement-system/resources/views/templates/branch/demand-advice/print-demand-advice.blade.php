         ...
         @php
                                ...
                                elseif($row->payment_type == 1 || $row->payment_type == 2 || $row->payment_type == 3 || $row->payment_type == 4){
                                    ...
                                    if($demandAdvice['investment']->member_id){
                                        // $memberId = getMemberData($demandAdvice['investment']->member_id) ? getMemberData($demandAdvice['investment']->member_id)->member_id : 'N/A';
                                        $memberId = getMemberCustomData($demandAdvice['investment']->member_id)->member_id;
                                   ...
                                   if($row->payment_type == 4)
                                    {
                                        $demandAdvice = App\Models\DemandAdvice::with('investment')->where('id',$row->id)->first();  
                                        if($demandAdvice['investment']->member_id){
                                            $name = getMemberData($demandAdvice['investment']->member_id) ? getMemberData($demandAdvice['investment']->member_id)->first_name.' '.getMemberData($demandAdvice['investment']->member_id)->last_name ?? '' : 'N/A';
                                        }
                                    }
...
@section('script')
@include('templates.branch.demand-advice.partials.print_script')
@stop