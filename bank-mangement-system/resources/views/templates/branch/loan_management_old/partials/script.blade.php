...
$(document).ready(function() 
{
    $(".required").remove();
     ...
     var groupMemberId = [];
     var today = new Date();
     const getFileChargeajax = (loantype,value,aDate,tenureId,purpose,node,emiOption,ROI,tenure,dInput) => {
        $.ajax
        ({
            type:'POST',
            url:"{!! route('branch.getFileCharge') !!}",
            dataType:'JSON',
            data: {'loanType':loantype,'amount':value,'applicationDate':aDate,'tenure':tenureId,'purpose':purpose},
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success:function(response){
    ...
    $('#register-plan').validate({ // initialize the plugin
        rules: {
            ...
            'co-applicant_cheque_number_2' : {required: true, number: true, notEqual: "#co-applicant_cheque_number-1", notEqual1: "#applicant_cheque_number_1", notEqual2: "#applicant_cheque_number_2", notEqual3: "#guarantor_cheque_number_1", notEqual4: "#guarantor_cheque_number_2"},*/
            'guarantor_member_id' : {required: true, number: true},
            'guarantor_auto_member_id': {required: true, number: true},
            'guarantor_name' : 'required',
            'guarantor_father_name' : 'required',
            'guarantor_dob' : 'required',
            'guarantor_marital_status' : 'required',
    ...
    // Datatables
     loantable = $('#loan-listing').DataTable({
        ...
        columns: [
            ...
            {data: 'plan_name', name: 'plan_name'},
            {data: 'cgst_insurance_charge', name: 'cgst_insurance_charge'},
            {data: 'sgst_insurance_charge', name: 'sgst_insurance_charge'},
            {data: 'igst_insurance_charge', name: 'igst_insurance_charge'},
            {data: 'cgst_file_charge', name: 'cgst_file_charge'},
            {data: 'sgst_file_charge', name: 'sgst_file_charge'},
            {data: 'igst_file_charge', name: 'igst_file_charge'},
            {data: 'tenure', name: 'tenure'},
            {data: 'emi_amount', name: 'emi_amount'},
            {data: 'transfer_amount', name: 'transfer_amount'},
            {data: 'loan_amount', name: 'loan_amount'},
...
$('.export-group-loan').on('click',function(e){
        ...
        switch(!aDate)
        {
            ...
            case false:
                $.ajax({
                    ...
                    data:{'aDate':aDate,'loanType':loanType},
                    dataType:'JSON',
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success:function(res)
                    {
                        if(res.loans )
                        {
                           let labelnme = '';
                            ...
                            var select = $('#loan');
                            select.empty();
                            select.append('<option value="">--- Select Loan ---</option>');
                            $.each(res.loans,function(key1,value1){
                               switch(key1){
                                ...
                                break;
                                case '4':
                                    labelnme='Loan Against Investment Loan';
                                break;
                               }
                                $('#loan').append('<optgroup label="'+labelnme+'"></optgroup>');
                                $.each(value1,function(key,value){
                                let ROIData  = '';
                                let tenureData = '';
                               if(value.loan_tenure)
                               {    
                                    ROIData = JSON.stringify(value.loan_tenure.ROI); 
                                    tenureData = JSON.stringify(value.loan_tenure.tenure);
                               }
                                if(value.emi_option == 1)
                                    ...
                                    emiType  = 'Days';
                                $('#loan').append(' <option value="'+value.id+'"  data-val="'+value.slug+'" data-min="'+value.min_amount+'" data-max="'+value.max_amount+'" data-category="'+value.loan_category+'" data-emiOption="'+emiType+'" data-ROInterest="'+ROIData+'"  data-roiPeriod="'+tenureData+''+emiType +'" data-emiPeriod="'+tenureData+'"  data-gstStatus = "'+res.gstData.IntraState+'" data-gstPer = "'+res.gstData.gst_percentage+'" data-gstFileStatus = "'+res.gstFileChargeData.IntraState+'" data-gstFilePer = "'+res.gstFileChargeData.gst_percentage+'" >' +value.name+'</option> </optgroup>');
                            })    
                            })
                        }
                        else{
                            $('.error-msg-loan').html('Plan is Not created on selected Date');
                        }
                    }  
                })
                break;
    ...
    tenureAppend = (aDate,loanType,purpose) => {
        $.ajax({
            type:'POST',
            url:"{!! route('branch.getActiveLoans') !!}",
            data:{'aDate':aDate,'loanType':loanType,'purpose':purpose},
            dataType:'JSON',
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success:function(res)
            {
                $('#emi_mode_option').empty().append('<option value="">---Please Select Emi---</option>');
                $('#emi_mode_option_grp').empty().append('<option value="">---Please Select Emi---</option>');
                $.each(res.loans,function(key,value){
                    ...
                    (loanType != 3) ?  $('#emi_mode_option').append('<option value="'+value.id+'"  data-emiOption='+value.emi_option+' data-ROInterest='+value.ROI+' data-tenure='+value.tenure+' >' +value.name+' </option>') :  $('#emi_mode_option_grp').append('<option value="'+value.id+'"  data-emiOption='+value.emi_option+' data-ROInterest='+value.ROI+' data-tenure='+value.tenure+' >' +value.name+' </option>');
                });
            }  
        })
    }
...
$('#bank_transfer_mode').on('change',function(){
    ...
    calculateEmi = (emiOption,value,tenure,ROI,insuranceDetail,fileCharge,dInput) => {
        ...
        var  InsuranceAmount = () => {
           switch(value >= insuranceDetail.min_amount && value <=insuranceDetail.max_amount){
            case true:
               const insAmount =  (chargeType == 1 ?  insuranceDetail.charge:  ((value*insuranceDetail.charge)/100));
            ...
            if((value < minAmount || value > maxAmount) && value!='')  {
                ...
                return false;
                    ...
                    if(aDate == '')
                    {
                        ...
                        getFileChargeajax(loantype,value,aDate,tenureId,'tenure',node,emiOption,ROI,tenure);
                    }    
            }
        }    
        else{
    ...
    $(document).on('change','#number_of_member',function(){
        ...
        var x = 1;
        for (var x = 1; x <= mNumber; x++) {
            var list_fieldHTML = '<tr><td><input data-input="'+x+'" type="text" name="m_id['+x+']" class="g-loan-member-id g-loan-member-id-'+x+' form-control" style="width: 104px"><input data-input="'+x+'" type="hidden" name="m_id['+x+']" class="g-loan-member-id g-loan-hidden-m-id-'+x+'"><input data-input="'+x+'" type="text" name="ml_emi['+x+']" class="g-loan-hidden-emi g-loan-hidden-emi-'+x+'"  placeholder="Emi Charge"><input data-input="'+x+'" type="text" name="ml_file_charge['+x+']" class="g-loan-hidden-file-charge g-loan-hidden-file-charge-'+x+'" placeholder="File Charge"><input data-input="'+x+'" type="text" name="ml_insurance_charge['+x+']" class="g-loan-hidden-insurance-charge g-loan-hidden-insurance-charge-'+x+'" placeholder="insuranceCharge"><input data-input="'+x+'" type="text" name="ml_gst_charge['+x+']" class="g-loan-hidden-gst-charge g-loan-hidden-gst-charge-'+x+'" placeholder="gstCharge"><input data-input="'+x+'" type="hidden" name="ml_gst_status['+x+']" class="g-loan-hidden-gst-status g-loan-hidden-gst-status-'+x+'" placeholder="gstStatus"><input data-input="'+x+'" type="hidden" name="ml_gst_file_charge['+x+']" class="g-loan-hidden-gst-file-charge  g-loan-hidden-gst-file-charge-'+x+'" placeholder="gstFileCharge"><input data-input="'+x+'" type="hidden" name="ml_gst_file_status['+x+']" class="g-loan-hidden-gst-file-status g-loan-hidden-gst-file-status-'+x+'" placeholder="gstFileStatus"><input data-input="'+x+'" type="hidden" name="ml_interest_rate['+x+']" class="g-loan-hidden-interest-rate g-loan-hidden-interest-rate-'+x+'"  placeholder="Rate"></td><td><input data-input="'+x+'" type="text" name="m_name['+x+']" class="g-loan-member-name g-loan-member-name-'+x+' form-control" style="width: 104px" readonly=""></td><td><input data-input="'+x+'" type="text" name="f_name['+x+']" class="g-loan-member-fname g-loan-member-fname-'+x+' form-control" style="width: 104px" readonly=""></td><td><input data-input="'+x+'" type="text" name="m_amount['+x+']" class="g-loan-member-amount g-loan-member-amount-'+x+' form-control" style="width: 104px"></td><td><input data-input="'+x+'" type="text" readonly name="m_total_deposit_amount['+x+']" class="g-loan-member-total-deposit-amount g-loan-member-total-deposit-amount-'+x+' form-control" style="width: 104px"></td><td><a target="blank" data-input="'+x+'" class="g-loan-member-s g-loan-member-s-'+x+'" href="javascript:void(0);" style="width: 104px"></a></td><td><input data-input="'+x+'" type="text" name="m_bn['+x+']" class="g-loan-member-bn g-loan-member-bn-'+x+' form-control" style="width: 104px" readonly=""></td><td><input data-input="'+x+'" type="text" name="m_bac['+x+']" class="g-loan-member-bac g-loan-member-bac-'+x+' form-control" style="width: 104px" readonly=""></td><td><input data-input="'+x+'" type="text" name="m_bi['+x+']" class="g-loan-member-bi g-loan-member-bi-'+x+' form-control" style="width: 104px" readonly=""></td><td><div class="custom-control custom-radio mb-3 "><input type="radio" id="group-leader-'+x+'" class="custom-control-input group-leader-m-id" name="g_l_m_id" data-group="" value="0"><label class="custom-control-label" for="group-leader-'+x+'">Yes</label></div></td></tr>';
            $('.m-input-number').append(list_fieldHTML); //Add field html
        }
        $('.group-loan-amount').val('');
        $('#loan_amount').val('');
    ...
    $('.m-input-number').on('change','.g-loan-member-id',function(){
        ...
        $.ajax({
            ...
            url: "{!! route('loan.groupmember') !!}",
            dataType: 'JSON',
            data: {'memberid':mId,datesys:created_date,'loantype':loantype},
            headers: {
                'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
            },
            success: function(response) {
    ...
    $('.m-input-number').on('change','.g-loan-member-amount',function(){
        ...
        if(mId != ''){
            ...
            getFileChargeajax(loantype,value,aDate,tenureId,"tenure",node,emiOption,ROI,tenure,dInput)
        }
    });
    $(document).on('change','.group-information',function(){
        var moPaymentType = $( ".group-information option:selected" ).attr('data-val');
    ...
    $(".ipl_amount").each(function(){
        $(this).on('keyup',function(){
                ...
                $.ajax({
                    ...
                    dataType:'JSON',
                    data:{'applicantDetail':applicantDetail,'invetsment_id':investment},
                    headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success:function(res)
        ...
        if(paymentMode == 0 && paymentMode != ''){
			...
			if(ssb_AccountNumber>0){
				$.ajax({
					...
					data: {'account_no':ssb_AccountNumber},
					headers: {
						'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
					},
					success: function(response) {
						if(response.data == 0){
							swal("Warning","You can not pay with inactive SSB account, Please select other payment mode for payment!","warning");
							$("#loan_emi_payment_mode option:selected").prop("selected", false);
							return false;
						}
						if(response.data == 2){ 
							swal("Error","Member dose not have SSB Account !","error");
