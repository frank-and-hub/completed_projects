...
$file_chrg_total = App\Models\Daybook::whereIn('transaction_type', ['6,10'])->where('branch_id', $branch_id)->where('company_id', $company_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->count();
$file_chrg_amount_total = App\Models\Daybook::whereIn('transaction_type', ['6,10'])->where('branch_id', $branch_id)->where('company_id', $company_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('amount');;
...
$fund_transfer_total = \App\Models\FundTransfer::where('transfer_type', 0)->where('branch_id', $branch_id)->where('company_id', $company_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->count();
$fund_transfer_amount_total = \App\Models\FundTransfer::where('transfer_type', 0)->where('branch_id', $branch_id)->where('company_id', $company_id)->whereBetween(\DB::raw('DATE(created_at)'), [$startDate, $endDate])->sum('amount');
