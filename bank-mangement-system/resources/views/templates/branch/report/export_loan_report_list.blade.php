...
<table border = "1" width = "100%" style="border-collapse: collapse;font-size:12px;">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>@if(isset($row->approve_date)){{ date("d/m/Y", strtotime(convertDate($row->approve_date))) }}@endif</td>
            ...
            if($mode){
                if($mode->payment_mode == 0){
                    $pMode = 'Cash';
                }elseif($mode->payment_mode == 1){
                    $pMode = 'Cheque';
            ...
            @endphp
            <td>{{ $feDate }}</td>
            <td>@if(isset($row->closing_date)){{ date("d/m/Y", strtotime(convertDate($row->closing_date))) }}@endif</td>
            @php
            $amount = App\Models\LoanDayBooks::where('loan_type',$row->loan_type)->where('loan_id',$row->id)->sum('deposit');
            @endphp
            <td>{{ $amount }}</td>
