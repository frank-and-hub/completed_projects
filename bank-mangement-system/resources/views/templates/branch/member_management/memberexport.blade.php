...
?> 
  <tr>
    ...
    <td>@if($member->gender == 1) Male @else Female @endif</td>
    ...
    <td>@if($member['memberNomineeDetails']->gender == 1) Male @else Female @endif </td>
    <td>@if($member->is_block) Block @else @if($member->status==1) Active @else Inactive @endif @endif</td>
    <td><?php $is_upload='Yes'; if($member->signature=='') {   $is_upload = 'No';  } if($member->photo=='') {   $is_upload = 'No';  } ?>{{ $is_upload }}</td>
    <td>{{ $address }}</td>
    <td>{{ $state }}</td>
    <td>{{ $district }}</td>
    <td>{{ $city }}</td>
