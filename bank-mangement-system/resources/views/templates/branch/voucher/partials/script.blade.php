    ...
    $(document).ready(function() {
        ...
        $('#head').on('change', function() {
            ...
            if (headId == 19) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{!!route('branch.voucher.checkGstData')!!}",
                    data: {
        ...
        $(document).on('change', '#branch_id,#date,#head,#company_id', function() {
            ...
            if (head == 122) {
                // $.ajax({
                //     type: 'POST',
                //     url: "{!! route('branch.gst.gst_charge_member') !!}",
                //     dataType: 'JSON',
                //     data: {
                //         branchId: BranchId,
                //         memberid: memberid,
        ...
        $('#member_id').on('change', function() {
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('branch.voucher.memberdetails') !!}",
                data: {
                    'member_id': member_id,
                    'branch_id': branch_id,
                },
                ...
                success: function(response, textStatus, jqXHR) {
                    ...
                    if (jqXHR.status == 200 && textStatus == 'success') {
                        if (company_id == response.collectorDetails.company_id) {
                            ...
                            $('#member_id').val('');
                            $('#member_name').val('');
                            swal("Warning!", "Member is not exist in selected company.", "warning");
                            return false;
                        }
                    } else {
                        $('#member_id').val('');
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.emp != null) {
                        if (company_id != response.emp.company_id) {
                            swal("Error!", "EMP code does not exist in selected company.", "error");
                            $('#emp_code').val('');
                            $('#emp_id').val('');
                            $('#emp_name').val('');
                        }
                       else if (branch_id != response.emp.branch_id) {
                            swal("Error!", "EMP code does not exist in selected branch.", "error");
                            $('#emp_code').val('');
                            $('#emp_id').val('');
                            $('#emp_name').val('');
