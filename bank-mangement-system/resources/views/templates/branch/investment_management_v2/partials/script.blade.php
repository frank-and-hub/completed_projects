  ...
  $("#branch").select2();
                ...
                $.post(url,{
					...
					if(response.plans.length > 0 && response.plans){
						...
						$.each(response?.plans,function(index,value){
							html += '<option data-val="'+value?.slug +'"  value="'+value?.id +'" data-category="'+value?.plan_category_code +'" data-min = "'+value?.min_deposit+'" data-max = "'+value?.max_deposit+'" data-multiple = "'+value?.multiple_deposit+'" data-ssb-required="'+value?.is_ssb_required +'" >'+value?.name+'</option>';
						});
        ...
        $(document).on('change', '#payment-mode', function() {
            ...
            if (paymentMode == 'cheque-mode') {
                ...
                $.ajax({
                    ...
                    dataType: 'JSON',
                    data:{'companyId':companyId,'branch_id':branchId},
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    },
                    success: function(response) {
        ...
        $(document).on('keyup change', '.fd-tenure,.fd-amount', function() {
          ...
          var  rate =  (specialCategory != 'General Category') ? sprcialRate : roi;
            ...
            console.log("specialCategorydddd", specialCategory);
