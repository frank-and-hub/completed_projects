...
var cheque_id= $('input[name="cheque_id"]').val();
...
function success(i) {
 ...
 no_of_acc  =   rdfrd_no_of_accounts;
  ...
  if (i > no_of_acc) return;
    ...
    //for(i=1;i<=daily_no_of_accounts;i++){
    ...
    //do something special
            ...
            var hidden_due_amount= $('input[name="hidden_due_amount['+newObj[i]+']"]').val();
    ...
    $.ajax({
        url: "{{ route('branch.renew.storeajax') }}",
...
$("#cover").fadeOut(100);
                ...
                if(data.redirect_url!=''){
                    window.location.href = data.redirect_url; //return true;
                }
             }else{
             var pNumber = data.percentage;
...
$.validator.addMethod("chequeAmount", function(value, element,p) {
    ...
    $('#renewal-form').validate({ // initialize the plugin
        rules: {
            ...
            'amount[0]': {required: true, number: true, min:1},
        ...
        $.ajax({
            ...
            success: function(response) {
                if(response.msg_type == 'success')
                {
                    ...
                    if(response.collectorDetails.saving_account){
                        $('#collector_account_blance').val(response.collectorDetails.saving_account[0].balance);
                        $('#available_balance').val(response.collectorDetails.saving_account[0].balance);
                    }
                }
                else
                {
