<script type="text/javascript">
    $(document).ready(function () {
        function chequerenewlimit(cValue,mId,thisvalue){
            var renewPlan = $("#renewplan option:selected").val();
            var renewPlanId = $("#renew_investment_plan_id").val();
            var renewalDate = $("#renewal_date").val(); // crent Date
            var chequee = ($('#payment_mode').val() == 1) ? true : false;
            const paymentMode = $("#payment_mode").val();
            $.post("{!! route('branch.investment.renewlimit') !!}",
                {
                    'amount': cValue,
                    'account_number': mId,
                    'renewPlan': renewPlan,
        ...
        $('#renewal-form').validate({ // initialize the plugin
            rules: {
                ...
                'amount[0]': 'required',
                'daily_no_of_accounts': { required: true, number: true, max: 22, min: 1 },
                'rdfrd_no_of_accounts': { required: true, number: true, max: 15, min: 1 },
        ...
        $(document).on('change', '.account-number', function () {
            ...
            //     return false;
            // }
             if (paymentMode == '' && renewPlan == 2 ) {
                $(this).val('');
                swal("Warning!", 'Please select a payment Mode!!', "warning");
                return false;
            }
            // if (company_id == '') {
            //     $(this).val('');
            //     swal("Warning!", 'Please select a company first!!', "warning");
            //     return false;
            // }
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('investment.renewplan') !!}",
                dataType: 'JSON',
                data: { 'account_number': accountNumber, 'renewPlan': renewPlan, 'renewPlanId': renewPlanId, 'renewalDate': renewalDate, /*'company_id': company_id ,*/'chequee':chequee},
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                },
                success: function (response) {
                    // console.log(renewPlan == 2 && response.investment[0].transaction_status, paymentVal);
                    if (response.resCount > 0 && response.minutes > 5) {
                        ...
                        if (renewPlan == 2 && response.investment[0].ssb.transaction_status == '0' && paymentVal == "0") {
                            swal("Warning!", "Account Number is InActive!!", "warning");
                            $('.account-number-' + attVal + '').val('');
                            return false;
                        }
                        ...
                        $.each(response.investment, function (key, value) {
                            ...
                            if (renewPlan == 2) {
                                ...
                                $('#transaction_status').val(value.transaction_status);
                            }
                    ...
                    if (response.cheques != "") {
                        let chequeArray = response.cheques ;
                      let   html = `<option value="">Select cheque number</option>`;
                        chequeArray.forEach(element => {
                            html += `<option value="${element.id}">${element.cheque_no}</option>`;
                        });
                        $('#cheque_id').html(html);
                    }
        ...
        $(document).on('change', '#associate_code,#rdfrd_associate_code', function () {
            ...
            $.ajax({
                type: "POST",
                url: "{!! route('investment.getcollectorassociate') !!}",
                dataType: 'JSON',
                data: { 'code': code },
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        ...
        $('.daily-renew-input-number').on('change', '.renew-amount', function () {
            ...
            var cValue = $(this).val();
            var floatInteger = (depositeAmount > 0) ? (cValue / depositeAmount) : 0;
            ...
            $('#total_amount').val(sum);
            chequerenewlimit(cValue,mId,$(this));
        });
            ...
            if (numberFields <= fieldLimit) {
                for (var x = 1; x <= numberFields; x++) {
                    if (renewPlan == 0) {
                         ...
                         */
                        var list_fieldHTML =
                        `<tr class="tr_class" id="row_${x}">
                            <td>
                                <input type="text" data-val="${x}" name="account_number[${x}]" class="form-control removeSpaceInput account-number account-number-${x}" autocomplete="off" required>
                                <input type="hidden" name="investment_plan[${x}]" class="col-lg-0 investment-plan-${x}">
                                <input type="hidden" name="investment_id[${x}]" class="col-lg-0 investment-id-${x}">
                                <input type="hidden" name="investment_member_id[${x}]" class="col-lg-0 investment-member-id-${x}">
                                <input type="hidden" name="investment_tenure[${x}]" class="col-lg-0 investment-tenure-${x}">
                                <input type="hidden" name="investment_member_phone_no[${x}]" class="col-lg-0 investment_member_phone_no-${x}">
                            </td>
                            <td>
                                <input type="text" data-val="${x}" name="name[${x}]" class="form-control name-${x}" readonly="" tabIndex="-1">
                            </td>
                            <!--<td>
                                <input type="text" data-val="${x}" name="deno_amount[${x}]" class="form-control deno_amount-${x}" readonly="" tabIndex="-1">
                            </td>-->
                            <td>
                                <div class="col-lg-12">
                                    <div class="rupee-img"></div>
                                    <input type="text" data-val="${x}" name="amount[${x}]" class="form-control rupee-txt renew-amount renew-amount-${x}" required>
                                </div>
                                <input type="hidden" data-val="${x}" name="deo_amount[${x}]" class="col-lg-0 form-control rupee-txt deo-amount-${x}" readonly="" tabIndex="-1">
                                <input type="hidden" name="hidden_due_amount[${x}]" class="col-lg-0 hidden-due-amount-${x}">
                                <input type="hidden" name="hidden_deposite_amount[${x}]" class="col-lg-0 hidden-deposite-amount-${x}">
                            </td>
                            <td>
                                <input type="text" data-val="${x}" name="acoount_associate_name[${x}]" class="form-control associate-name-${x}" readonly="" tabIndex="-1">
                                <a href="javascript:void(0);" class="remove-daily-renew-button" tabIndex="-1">Remove</a>
                            </td>
                        </tr>`; //New input field html
                        $('.' + appendClass + '').append(list_fieldHTML); //Add field html
                    } else if (renewPlan == 1) {
                        // last updated by sourab on 22-03-24 as per discussed with sachin sir.
                        var list_fieldHTML =`<tr class="tr_class" id="row_${x}"><td><input type="text" data-val="${x}" name="account_number[${x}]" class="form-control account-number account-number-${x}" autocomplete="off" required><input type="hidden" name="investment_plan[${x}]" class="col-lg-0 investment-plan-${x}"><input type="hidden" name="investment_id[${x}]" class="col-lg-0 investment-id-${x}"><input type="hidden" name="investment_member_id[${x}]" class="col-lg-0 investment-member-id-${x}"><input type="hidden" name="investment_tenure[${x}]" class="col-lg-0 investment-tenure-${x}"><input type="hidden" name="investment_member_phone_no[${x}]" class="col-lg-0 investment_member_phone_no-${x}"></td><td><input type="text" data-val="${x}" name="name[${x}]" class="form-control name-${x}" readonly="" tabIndex="-1"></td><!--<td><input type="text" data-val="${x}" name="deno_amount[${x}]" class="form-control deno_amount-${x}" readonly="" tabIndex="-1"></td>--><td><div class="col-lg-12"><div class="rupee-img"></div><input type="text" data-val="${x}" name="amount[${x}]" class="form-control rupee-txt rdfrd-renew-amount rdfrd-renew-amount-${x}" required></div><input type="hidden" data-val="${x}" name="deo_amount[${x}]" class="col-lg-0 form-control rupee-txt deo-amount-${x}" readonly="" tabIndex="-1"><input type="hidden" name="hidden_due_amount[${x}]" class="col-lg-0 hidden-due-amount-${x}"><input type="hidden" name="hidden_deposite_amount[${x}]" class="col-lg-0 hidden-deposite-amount-${x}"></td><td><input type="text" data-val="${x}" name="acoount_associate_name[${x}]" class="form-control associate-name-${x}" readonly="" tabIndex="-1"><a href="javascript:void(0);" class="remove-rdfrd-renew-button" tabIndex="-1">Remove</a></td></tr>`; //New input field html
                        $('.' + appendClass + '').append(list_fieldHTML); //Add field html
                    }
                }
            }
        ...
        $(document).on('change', '#payment_mode', function () {
            ...
            $('.daily-no-of-accounts').attr('readonly', false);
            $('.rdfrd-no-of-accounts').attr('readonly', false);
            const renewPlan = $("#renewplan option:selected").val();
            if(renewPlan == 2)
            {
                $('.account-number').val('');
                $('.name-0').removeAttr("readonly").val('');
                $('.saving-amount-0 ').val('');
                $('.saving-total-amount-0').val('');
            }
            if (paymentMode == 'cash' && renewPlan == 2) {
                const transaaction_status = $('#transaction_status').val();
                if (transaaction_status == '0') {
                    swal('alert', 'SSB Account Inactive', 'alert');
                }
            }
            $('.cash').hide();
