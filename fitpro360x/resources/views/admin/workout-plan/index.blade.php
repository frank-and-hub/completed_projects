...
<script>
    ...
    $(document).on('click', '.delete-workoutPlan', function () {
       ...
       // deleteModal = new bootstrap.Modal(document.getElementById('deleteworkoutPlanModal'));
        // deleteModal.show();
        $.ajax({
            url: "{{ route('admin.check_workout_plan_status', '') }}/" + workoutPlanId,
            type: 'GET',
            success: function(response) {
                if (response.status === 'error' && response.can_delete === false) {
                    // Workout plan is assigned to users - cannot delete
            ...
            error: function(xhr) {
                var errorMessage = 'Error checking workout plan status.';
