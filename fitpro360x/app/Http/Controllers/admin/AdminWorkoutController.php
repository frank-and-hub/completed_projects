...
class AdminWorkoutController extends Controller
{
    ...
    public function workoutPlansDelete($id)
    {
        try {
            ...
            if ($userEntryExists) {
                return response()->json([
                    'success' => false,
                    'message' => 'Cannot delete this workout plan. It is currently assigned to one or more users on App.'
                ], 400);
            }
            ...
            // Delete admin workout settings
            AdminWorkoutSettings::where('workout_program_id', $id)->delete();

            // Delete admin meal entries
            Admin_Meal_Entries::where('workout_program_id', $id)->delete();
    ...
    public function checkWorkoutPlanStatus($id = null)
    {
        if (!$id) {
            return response()->json([
                'status' => 'error',
                'message' => 'No Workout Plan ID provided'
            ], 400);
        }

        $workoutPlan = Workout_Programs::find($id);
        ...
        // Check if this workout plan is assigned to any users
        $assignedUsers = UserWorkoutPlan::where('workout_program_id', $id)
            ->with('user:id,email')
            ->get();
        ...
        if ($assignedUsers->isNotEmpty()) {
            ...
            return response()->json([
                ...
                'active' => $workoutPlan->status ?? true
            ], 403);
        }
        ...
        return response()->json([
            'status' => 'success',
            'can_delete' => true,
            'message' => 'This workout plan can be safely deleted.',
            'active' => $workoutPlan->status ?? true
        ]);
    }
