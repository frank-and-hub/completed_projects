...
class SubscriptionController extends BaseApiController
{
    ...
    public function getUserSubscriptions(Request $request)
    {
        ...
        // if ($subscription->status === 'expired' && !$subscription->is_expire_email_sent) {
        //     Mail::to($user->email)->send(new SubscriptionExpireMail($user, $subscription->expires_at));
        //     $subscription->is_expire_email_sent = true;
        //     $subscription->save();

        //     Log::info("Expired subscription email sent to user ID {$user->id}");
        // }
        /* $expirestatus = UserSubscription::where('user_id', $user->id)
            ->whereNull('deleted_at')
            ->orderByDesc('id')
            ->where('status', '!=', 'active')
            ->where('status', '=', 'active')
        ...
        if (
            ...
            ($expirestatus->is_expire_email_sent === null)
        ) {
            if (!empty($user->email)) {
                try {
                    Mail::to($user->email)->send(new SubscriptionExpireMail($user, $expirestatus->expires_at));

                    $expirestatus->is_expire_email_sent != 1;
                    $expirestatus->save();
