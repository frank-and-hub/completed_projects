            ...
            <div class="card">
                <div class="card-body">
                    @if (condication(auth()->user(), '3', 'add'))
                        <a href="{{ route('admin.home.banner.add') }}">
                            <h6 style="float:right;" class="card-title">Add New</h6>
                        </a>
                    @endif
                    <h6 class="card-title">Banners List</h6>
                            ...
                            <tbody>
                                @foreach ($banners as $key => $value)
                                    <tr>
                                        ...
                                        <td>{{ $value->title }}</td>
                                        <td><img src="{{ asset('uploads/' . $value->image) }}" alt="Blog Image"
                                                class="mb-2" style="max-width: 200px;"></td>
                                        <td>{{ $value->created_at->format('d-m-Y') }}</td>


                                        <td>
                                            @if (condication(auth()->user(), '3', 'edit'))
                                                <a href="{{ route('admin.home.banner.edit', $value->id) }}"  class="btn btn-none bg-none border-0" >
                                                    <i class="fas fa-edit"></i> <!-- Edit icon -->
                                                </a> |
                                                @endif @if (condication(auth()->user(), '3', 'view'))
                                                    <a href="{{ route('admin.home.banner.view', $value->id) }}"  class="btn btn-none bg-none border-0" >
                                                        <i class="fas fa-eye"></i> <!-- View icon -->
                                                    </a> |
                                                    @endif @if (condication(auth()->user(), '3', 'delete'))
                                        ...
                                        <div class="modal-content">
                                          ...
                                          <div class="modal-footer">
                                            <!--<button type="button" class="btn btn-primary" data-dismiss="modal">-->
                                                <a href="{{ route('admin.home.banner.delete', $value->id) }}" class="text-white btn btn-primary">
                                                    Yes
                                                </a>
                                            <!--</button>-->
