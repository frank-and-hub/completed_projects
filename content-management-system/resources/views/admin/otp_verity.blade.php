...
<body>
    ...
    <form role="form" method="post" action="{{ route('admin.login.otp_verify') }}" data-parsley-validate
        ...
        <h4 class="mt-3 text-center" style="color:black; "><strong>Two Factor Authentication</strong></h4>
        <h6 class="mt-3 text-center" style="color:black; ">A verification code has been sent to <strong>{{ $user->email }}</strong>.</h6>
        <div style="
        position: absolute;
        top: 45%;
        left: 10%;
    ...
    ">
            <div class="mb-1">
                ...
                <input type="text" class="form-control text-center" id="otp" name="otp" placeholder="OTP"
                    autofocus="" autocomplete="new-otp">
            </div>
            <div class="mt-5">

                <div class="mt-2 text-center">
                    <small class="text-dark">Didn't receive code?</small> <strong onclick="resend()" role="button">Send again</strong>
                </div>
    ...
    <script>
        function resend() {
            $.ajax({
                url: "{{route('admin.login.resend.otp')}}",
                success: function(res) {
                    ToastAlert(msg = res.msg, type = "Success", className =
                        "bg-success");
                },
                error: function(err) {
                    ToastAlert(msg = "{{ session('success') }}", type = "Success", className =
                        "bg-success");
                }
            })
        }
