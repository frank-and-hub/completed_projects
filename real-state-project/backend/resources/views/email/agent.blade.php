...
<html>
<head>
    <title>PocketProperty: New Rental Request</title>
    <style>
        body {
...
<body>
    <div class='content'>
        ...
        <p>A prospective tenant, <b>{{ $user['name'] }}</b>, has shown interest in your property: <b>{{ $property['title'] }}</b> <a href="{{ $property['link'] }}">View Property</a>. They would like to be contacted for a viewing. Please reach out to them at <a href="mailto:{{ $user['email'] }}">{{ $user['email'] }}</a> or <a href= "tel:{{ $user['phone'] }}">{{ $user['phone'] }}</a>.</p>

        @if(array_key_exists('message', $user))
         <p>Message: {{ $user['message'] }}</p>
        @endif

        <p>This rental request was submitted to you on their behalf by PocketProperty.</p>
