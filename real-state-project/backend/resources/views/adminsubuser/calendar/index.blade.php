    ...
    @push('custom-script')
        ...
        <script type="text/javascript">
            document.addEventListener('DOMContentLoaded', function () {
                const ajax_url = `{{ route('adminSubUser.calendar.index') }}`;
                    ...
                    eventClassNames: function (info) {
                        ...
                        if (kinds.length) {
                            result = (result && kinds.indexOf(info.event.extendedProps.kind) >= 0) ||
                                info.event.extendedProps.kind == 'holiday';
                        }
                ...
                var calendarTable = $('#calendarTable').DataTable({
                    ...
                    columns: [
                         {
                            ...
                            render: function (data, type, row, meta) {
                                return data ? (data.length > 30 ?
                                    `<span class="short-text">${data.substring(0, 30)}...</span><span class="full-text" style="display:none; line-height:18px;">${data}</span><span class="view-more" style="color: blue; cursor: pointer;">View More</span>` :
                                    data) : '';
                            }
                        }, {
                            data: 'description',
                    ...
                    drawCallback: function (settings, json) {
                        if ((role != 'agency')) {
                            calendarTable.column(5).visible(false);
                        }
