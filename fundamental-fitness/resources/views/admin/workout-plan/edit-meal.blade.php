...
@section('admin-title', 'Edit Meal Plan')
...
@section('content')
    <div class="container-fluid d-flex justify-content-center align-items-center" style="min-height: 90vh; background: #f5f5f5;">
        <div class="card shadow-lg p-4" style="border-radius: 18px; width: 100%; background: #fff;">
            ...
            <form action="{{ route('admin.updateWorkoutMeal', $programId) }}" method="POST">
                @csrf
                @method('POST')
                ...
                <div id="meal-sections-wrapper">
                    ...
                    @foreach ($requiredDietPreferences as $dietPreference)
                        ...
                        <div class="meal-section-group mb-4 p-3" style="background: #fcf6f5; border-radius: 12px;">
                            ...
                            <input type="hidden" name="meal[{{ $currentIndex }}][diet_preference]" value="{{ implode(',', $targetDietIds) }}">

                            <div class="meal-section mb-4">
                                <label class="fw-bold">Breakfast</label>
                                <select class="form-control meal_title" name="meal[{{ $currentIndex }}][breakfast][]" multiple>
                                    @foreach ($breakfastMeals as $mealId)
                                        @if (isset($allMeals[$mealId]))
                                            <option value="{{ $mealId }}" selected>{{ $allMeals[$mealId]->title }}</option>
                                        @endif
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="meal-section mb-4">
                                ...
                                <select class="form-control meal_title" name="meal[{{ $currentIndex }}][lunch][]" multiple>
                                    @foreach ($lunchMeals as $mealId)
                                        @if (isset($allMeals[$mealId]))
                                            <option value="{{ $mealId }}" selected>{{ $allMeals[$mealId]->title }}</option>
                                        @endif
                                    @endforeach
                            ...
                            <div class="meal-section mb-4">
                                ...
                                <select class="form-control meal_title" name="meal[{{ $currentIndex }}][dinner][]" multiple>
                                    @foreach ($dinnerMeals as $mealId)
                                        @if (isset($allMeals[$mealId]))
                                            <option value="{{ $mealId }}" selected>{{ $allMeals[$mealId]->title }}</option>
                                        @endif
                                    @endforeach
        ...
        $(document).ready(function() {
            ...
            @foreach($requiredDietPreferences as $index => $dietPreference)
                ...
                @if(!empty(array_filter($dietPrefIds)))
                    loadMealsForDietPreference({{ $index }}, [@foreach($dietPrefIds as $id){{ $id }}@if(!$loop->last), @endif @endforeach]);
                @endif
            @endforeach
